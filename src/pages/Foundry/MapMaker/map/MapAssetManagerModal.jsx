...
export default function MapAssetManagerModal({
  ...
  const handleSelectAssetForEdit = (asset, kind) => {
    ...
    if (kind === 'terrains') {
      setTerrainForm({
        ...
        label: asset.isCustom ? asset.label : `${asset.label} (Copy)`,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/80 backdrop-blur-md p-4 pt-4 sm:pt-6 overflow-y-auto select-none">
      <div className="bg-[#161b22] border border-[#0D5C63] rounded-xl w-full max-w-6xl h-[88vh] flex flex-col shadow-2xl overflow-hidden text-slate-100 font-sans">
        
        {/* Header */}
        <div className="px-6 py-3.5 bg-[#0d1117] border-b border-[#0D5C63]/60 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-2xl">📦</span>
            <div>
              <h2 className="text-lg font-bold text-white tracking-wider uppercase">Mapmaker Asset & Texture Catalog Manager</h2>
              <p className="text-xs text-cyan-400 font-mono">Organize preset catalog, import custom image textures & sprites, manage categories</p>
            </div>
          </div>
        ...
        <div className="px-6 py-2.5 bg-[#161b22] border-b border-[#0D5C63]/40 flex flex-wrap items-center justify-between gap-3">
          ...
          {/* Main Category Tabs */}
          <div className="flex flex-wrap items-center gap-1.5 bg-[#0d1117] p-1.5 rounded-full border border-[#0D5C63]/40 shrink-0">
            <button
              onClick={() => {
                setActiveTab('terrains');
                resetToNewForm('terrains');
              }}
              className={`px-3.5 py-1.5 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-tight sm:tracking-wider transition-all flex items-center gap-1.5 shrink-0 whitespace-nowrap border ${
                activeTab === 'terrains'
                  ? 'bg-cyan-950/80 text-[#22d3ee] border-[#22d3ee]/80 shadow-[0_0_12px_rgba(34,211,238,0.25)] ring-1 ring-cyan-500/30'
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              className="bg-[#0d1117] border border-[#0D5C63]/50 text-xs text-white px-2.5 py-1 rounded w-40 outline-none focus:border-[#22d3ee]"
            />
          </div>
        </div>

        {/* Content Layout: Left List + Right Editor */}
        <div className="flex-1 flex overflow-hidden">
          
          {/* Left Panel: Catalog List */}
          <div className="w-1/2 border-r border-[#0D5C63]/40 flex flex-col bg-[#0d1117]/50 p-4 gap-3 overflow-hidden">
            <div className="flex items-center justify-between">
              <span className="text-xs uppercase font-bold text-[#22d3ee] tracking-wider">
                {activeTab === 'terrains' ? `Terrains Catalog (${filteredTerrains.length})` : `Objects Catalog (${filteredObjects.length})`}
              </span>
              <button
                onClick={() => resetToNewForm(activeTab)}
                className="px-3 py-1 bg-cyan-600 hover:bg-cyan-500 text-white rounded text-xs font-bold uppercase tracking-wider flex items-center gap-1 shadow-md transition-all"
              >
                  ...
                  filteredTerrains.map(t => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        {t.textureUrl ? (
                          <div className="w-8 h-8 rounded border border-cyan-500/60 overflow-hidden shrink-0 bg-[#0d1117]">
                            <img src={t.textureUrl} alt={t.label} className="w-full h-full object-cover" />
                          </div>
                        ) : (
                          <span
                            className="w-5 h-5 rounded-full border border-white/40 shadow shrink-0"
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {t.isCustom && (
                              <span className="text-[9px] bg-cyan-950 text-cyan-300 border border-cyan-800 px-1.5 py-0.2 rounded font-mono uppercase">
                                Custom
                              </span>
                            )}
                          </div>
                ...
                filteredObjects.length === 0 ? (
                  ...
                  filteredObjects.map(o => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        {o.imageUrl ? (
                          <div className="w-8 h-8 rounded border border-cyan-500/60 overflow-hidden shrink-0 bg-[#0d1117] p-0.5">
                            <img src={o.imageUrl} alt={o.label} className="w-full h-full object-contain" />
                          </div>
                        ) : (
                          <span
                            className="w-5 h-5 rounded border border-white/40 shadow shrink-0"
          ...
          <div className="w-1/2 p-5 overflow-y-auto flex flex-col bg-[#161b22]">
            <div className="flex items-center justify-between mb-4 border-b border-[#0D5C63]/40 pb-2">
              <h3 className="text-sm font-bold uppercase text-[#22d3ee] tracking-wider flex items-center gap-2">
                <span>{isEditingCustom ? '✏️ Edit Custom Asset' : editingAsset ? '📋 Customize Base Asset' : '✨ Create New Asset'}</span>
              </h3>
              {isEditingCustom && (
                <button
                  onClick={handleDeleteSelected}
                  className="px-2.5 py-1 bg-red-950 hover:bg-red-900 text-red-300 border border-red-800 rounded text-[11px] font-bold uppercase transition-colors"
                >
                  🗑️ Delete Asset
                </button>
              )}
            ...
            {/* TERRAIN FORM */}
            {activeTab === 'terrains' && (
              <form onSubmit={handleSaveTerrainSubmit} className="flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-300">Terrain Name *</label>
                  <input
                    type="text"
                    required
                    placeholder="e.g., Crystalline Void Matrix"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold text-slate-300">Scale</label>
                    <select
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-slate-300">Custom Texture Image File (PNG/JPG/SVG)</label>
                  <div className="flex items-center gap-3">
                    <button
                      type="button"
                      onClick={() => singleFileInputRef.current?.click()}
                      className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-[#0D5C63]/60 rounded text-xs font-bold uppercase"
                    >
                      📁 Upload Image File...
                    </button>
                    <input
                    ...
                    {terrainForm.textureUrl && (
                      <span className="text-[10px] text-emerald-400 font-mono">✓ Custom Texture Loaded</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-300">Engine / Game Mechanics Properties</label>
                  <input
                    type="text"
                ...
                <div className="pt-3 border-t border-[#0D5C63]/40 flex items-center justify-end gap-3">
                  ...
                  >
                    {isEditingCustom ? 'Save Changes' : 'Create Custom Terrain'}
                  </button>
                </div>
              </form>
            )}
            ...
            {/* OBJECT FORM */}
            {activeTab === 'objects' && (
              <form onSubmit={handleSaveObjectSubmit} className="flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-300">Object Name *</label>
                  <input
                    type="text"
                    required
                    placeholder="e.g., Tactical Command Outpost"
                ...
                {/* Custom Object Image Uploader */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-slate-300">Custom Sprite / Image File (PNG/JPG/SVG)</label>
                  <div className="flex items-center gap-3">
                    <button
                      type="button"
                      onClick={() => singleFileInputRef.current?.click()}
                    ...
                    />
                    {objectForm.imageUrl && (
                      <span className="text-[10px] text-emerald-400 font-mono">✓ Custom Image Sprite Loaded</span>
                    )}
                  </div>
                </div>
                ...
                <div className="pt-3 border-t border-[#0D5C63]/40 flex items-center justify-end gap-3">
                  ...
                  <button
                    ...
                    className="px-5 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded text-xs font-bold uppercase tracking-wider shadow-lg transition-all"
                  >
                    {isEditingCustom ? 'Save Changes' : 'Create Custom Object'}
                  </button>
                </div>
              </form>
            )}
