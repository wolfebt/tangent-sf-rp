---
id: occupation-builder
name: Builder
category: occupations
skill_points: 20
professional_skills:
  - Any Vocation Skills
archetypes:
  - Starship Engineer
  - Cybernetic Technician
  - Nanotechnology Specialist
  - Habitat Architect
  - Weaponsmith
  - Robotics Engineer
  - Matter Manipulator
traits:
  - trait-builder-adaptability
  - trait-builder-attention-to-detail
  - trait-builder-attention-to-safety
  - trait-builder-business-acumen
  - trait-builder-creativity
  - trait-builder-environmental-awareness
  - trait-builder-mecha-physics
  - trait-builder-machine-affinity
  - trait-builder-patience
  - trait-builder-persistence
  - trait-builder-problem-solving-skills
  - trait-builder-teamwork
  - trait-builder-technical-knowledge
  - trait-builder-technological-aptitude
  - trait-builder-time-management
description: >-
  A builder character would be responsible for creating, repairing, and
  modifying various items, equipment, and structures. This character would have
  a diverse set of skills and knowledge in various fields, such as engineering,
  materials science, and technology. They would need to be able to understand
  and work with a variety of materials, and be able to design and build
  structures that are both functional and aesthetically pleasing. They would
  also need to be able to troubleshoot problems and make repairs when necessary.
  Builder characters are often essential to the success of any group or
  community. They can provide the tools and structures that are necessary for
  survival, and they can help to improve the quality of life for everyone.
costs:
  bp: 0
  credits: 0
  nodes: 0
  sockets: 0
  strain: 0
  focus: 0
  ap: 0
modifiers: []
modifications: []
critical_details:
  score: ''
  effect: []
  success_effect: []
  failure_effect: []
sockets:
  max: 0
  used: 0
  tier: Socket
  allocated: []
---

# Builder Occupation

**Category**: Occupations  
**Starting Skill Points**: 20 SP (allocated to Professional Skills, max Rank 11, recommended Rank 6)  
**Recommended Features**: Skill (1 BP Discount)  
**Starting Traits**: 2 Free selections (Additional traits cost 1 CP each)

## Description
A builder character would be responsible for creating, repairing, and modifying various items, equipment, and structures. This character would have a diverse set of skills and knowledge in various fields, such as engineering, materials science, and technology. They would need to be able to understand and work with a variety of materials, and be able to design and build structures that are both functional and aesthetically pleasing. They would also need to be able to troubleshoot problems and make repairs when necessary. Builder characters are often essential to the success of any group or community. They can provide the tools and structures that are necessary for survival, and they can help to improve the quality of life for everyone.

## Professional Skills
- Any Vocation Skills

## Typical Archetypes
- Starship Engineer
- Cybernetic Technician
- Nanotechnology Specialist
- Habitat Architect
- Weaponsmith
- Robotics Engineer
- Matter Manipulator

## Available Traits
- `trait-builder-adaptability`
- `trait-builder-attention-to-detail`
- `trait-builder-attention-to-safety`
- `trait-builder-business-acumen`
- `trait-builder-creativity`
- `trait-builder-environmental-awareness`
- `trait-builder-mecha-physics`
- `trait-builder-machine-affinity`
- `trait-builder-patience`
- `trait-builder-persistence`
- `trait-builder-problem-solving-skills`
- `trait-builder-teamwork`
- `trait-builder-technical-knowledge`
- `trait-builder-technological-aptitude`
- `trait-builder-time-management`


## Extended Lore & Background

A builder character would be responsible for creating, repairing, and modifying various items, equipment, and structures. This character would have a diverse set of skills and knowledge in various fields, such as engineering, materials science, and technology. They would need to be able to understand and work with a variety of materials, and be able to design and build structures that are both functional and aesthetically pleasing. They would also need to be able to troubleshoot problems and make repairs when necessary. Builder characters are often essential to the success of any group or community. They can provide the tools and structures that are necessary for survival, and they can help to improve the quality of life for everyone.

Any Vocation Skills

Skill

Starship Engineer  
Cybernetic Technician  
Nanotechnology Specialist  
Habitat Architect  
Weaponsmith  
Robotics Engineer  
Matter Manipulator

Ability to adjust their approach based on the specific needs of each project  
*\+2 to a specific Skill Check, may change assigned Skill daily*

Meticulous and detail-oriented  
*Mastercrafting Checks made at Advantage*

Awareness of safety protocols and ensuring that their creations are safe to use  
*Prototyping Checks gain a \+4 bonus*

Ability to handle financial and administrative aspects of their profession  
*\+2 Administration Knowledge*

Creative mindset and ability to think outside the box  
*\+1 Karma*

Awareness of environmental factors that could impact construction  
*\+2 Alertness*

Comprehensive understanding of physics  
*\+2 to Engineering*

Natural aptitude for working with machinery and tools  
*Additional \+1 from Equipment Bonus and \+2 Technology (Knowledge)*

Patience and willingness to put in the necessary time and effort  
*Meticulous skill check bonus is increased by 1 per stage*

Ability to persevere through challenges  
*\+1 Willpower Checks*

Strong analytical and problem-solving skills  
*\+2 Logic Checks*

Ability to work well with others  
*\+1 to Aid*

Deep understanding of the technical aspects of their craft, including knowledge of materials, tools, and techniques  
*\+2 Vocation Skill*

Natural aptitude for understanding and operating advanced technology  
*\+2 Technology Knowledge*

Excellent time management skills  
*Reduce Rush modifiers to Skill checks by 2*

