---
id: "archetype-field-medic"
name: "The Field Medic"
sphere: "Sentinels (The Stabilizers)"
category: "archetypes"
bp_chassis: 80
core_concept: "Healer / Light Infantry"
primary_attribute: "Wisdom"
secondary_attribute: "Agility"
recommended_occupations: ["Soldier","Specialist"]
recommended_origins: ["Militaristic","Colony","Spacer"]
recommended_factions: ["Coalition (Combat Medics)","Auluran (Healer Caste)","Outworlds (Sawbones)"]
essential_skills: ["Medicine","Athletics","Combat (Rifle/Pistol)"]
signature_features: ["Field Triage","Combat Medic"]
tactical_role: "Keep the tank alive, revive downed allies, provide light fire support."
quote: "\"I decide who walks away from this. Don't make me regret choosing you.\""
description: "Provides emergency medical care under fire. Proficient in both combat and medicine."
---

# The Field Medic

**Sphere**: Sentinels (The Stabilizers)  
**Core Concept**: Healer / Light Infantry  
**Chassis Allocation**: 80 BP (+3 Primary, +2 Secondary, Essential Skills & Signature Features)  
**Primary Attribute**: Wisdom  
**Secondary Attribute**: Agility  

## Overview
Provides emergency medical care under fire. Proficient in both combat and medicine.

> *You hold the line between life and death. When the armor fails and the screaming starts, you are the calm in the chaos. You possess the steady hands to stitch a wound while dodging plasma fire.*

> _"I decide who walks away from this. Don't make me regret choosing you."_

## Tactical Role
Keep the tank alive, revive downed allies, provide light fire support.

## Essential Skills
- Medicine
- Athletics
- Combat (Rifle/Pistol)

## Signature Features
- Field Triage
- Combat Medic

## Recommendations
- **Occupations**: Soldier, Specialist
- **Origins**: Militaristic, Colony, Spacer
- **Factions**: Coalition (Combat Medics), Auluran (Healer Caste), Outworlds (Sawbones)
