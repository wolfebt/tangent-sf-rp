---
id: "archetype-philosopher"
name: "The Philosopher"
sphere: "Visionaries (The Idealists)"
category: "archetypes"
bp_chassis: 80
core_concept: "Support / Lore / Mental Tank"
primary_attribute: "Intellect"
secondary_attribute: "Wisdom"
recommended_occupations: ["Scholar","Adept"]
recommended_origins: ["Enlightened","Research","Urban"]
recommended_factions: ["Alterian Enclave (Sages)","Mekan (Code Adherents)","Ascendancy"]
essential_skills: ["Academics","Insight","Diplomacy","Knowledge (History)"]
signature_features: ["Metacognition","Enlightened Mind"]
tactical_role: "Resisting mental intrusion, providing ethical guidance, identifying historical/philosophical context."
quote: "\"To know the universe, one must first know the silence between the thoughts.\""
description: "The seeker of deeper meaning who challenges the assumptions of society and explores the fundamental nature of existence."
---

# The Philosopher

**Sphere**: Visionaries (The Idealists)  
**Core Concept**: Support / Lore / Mental Tank  
**Chassis Allocation**: 80 BP (+3 Primary, +2 Secondary, Essential Skills & Signature Features)  
**Primary Attribute**: Intellect  
**Secondary Attribute**: Wisdom  

## Overview
The seeker of deeper meaning who challenges the assumptions of society and explores the fundamental nature of existence.

> *You think where others merely act. You analyze the moral and existential implications of every decision, providing the ethical framework that keeps the group—and civilization—from losing its soul.*

> _"To know the universe, one must first know the silence between the thoughts."_

## Tactical Role
Resisting mental intrusion, providing ethical guidance, identifying historical/philosophical context.

## Essential Skills
- Academics
- Insight
- Diplomacy
- Knowledge (History)

## Signature Features
- Metacognition
- Enlightened Mind

## Recommendations
- **Occupations**: Scholar, Adept
- **Origins**: Enlightened, Research, Urban
- **Factions**: Alterian Enclave (Sages), Mekan (Code Adherents), Ascendancy
