---
id: "archetype-saboteur"
name: "The Saboteur"
sphere: "Operatives (The Artisans)"
category: "archetypes"
bp_chassis: 80
core_concept: "Stealth / Tech / Debuff"
primary_attribute: "Intellect"
secondary_attribute: "Agility"
recommended_occupations: ["Agent","Specialist"]
recommended_origins: ["Militaristic","Urban","Industrial"]
recommended_factions: ["Coalition (Resistance)","Syndicate (Corporate Sabotage)"]
essential_skills: ["Stealth","Acrobatics","Knowledge (Technology)","Vocation (Demolitionist)"]
signature_features: ["Demolitionist","Infiltrator"]
tactical_role: "Disabling enemy infrastructure, setting traps, causing distractions."
quote: "\"A little chaos goes a long way.\""
description: "A skilled operative who infiltrates to destroy or disrupt."
---

# The Saboteur

**Sphere**: Operatives (The Artisans)  
**Core Concept**: Stealth / Tech / Debuff  
**Chassis Allocation**: 80 BP (+3 Primary, +2 Secondary, Essential Skills & Signature Features)  
**Primary Attribute**: Intellect  
**Secondary Attribute**: Agility  

## Overview
A skilled operative who infiltrates to destroy or disrupt.

> *You break things. Not randomly, but surgically. You know exactly where to place the charge or cut the wire to bring the whole system crashing down.*

> _"A little chaos goes a long way."_

## Tactical Role
Disabling enemy infrastructure, setting traps, causing distractions.

## Essential Skills
- Stealth
- Acrobatics
- Knowledge (Technology)
- Vocation (Demolitionist)

## Signature Features
- Demolitionist
- Infiltrator

## Recommendations
- **Occupations**: Agent, Specialist
- **Origins**: Militaristic, Urban, Industrial
- **Factions**: Coalition (Resistance), Syndicate (Corporate Sabotage)
