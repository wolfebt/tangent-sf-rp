---
id: "doc-operator-1-07-skills"
name: "1.07 SKILLS — SKILLS"
category: "operator_rule"
parent: "1.00 OPERATOR CORE RULES"
order: 90
perspective: "operator"
entry_type: "Core Rule"
tl: 3
ml: 0
cost: 0
tags: ["operator","core-rules","1.07 skills","mechanics"]
updatedAt: "2026-09-12T08:23:46.544Z"
costs:
  bp: 0
  credits: 0
  nodes: 0
  sockets: 0
  strain: 0
  focus: 0
  ap: 0
modifiers: []
modifications: []
critical_details:
  score: ''
  effect: []
  success_effect: []
  failure_effect: []
sockets:
  max: 0
  used: 0
  tier: Socket
  allocated: []
---

# **SKILLS**

# **SKILLS**

Skills represent a character's training and expertise in specific areas. They are categorized into Physical, Mental, Social, Metaphysic, and Combat groups. During character creation, players receive Skill Points from their chosen species, origin, and faction, which they can allocate to improve their skills. Skill Checks determine success in actions related to the corresponding skill, and various modifiers can influence the outcome.

## 

# **SKILL POINTS**

Skill types are:

* **PHYSICAL SKILLS**  
* **MENTAL SKILLS** (Knowledges and Vocations)  
* **SOCIAL SKILLS** (Manipulation and Expression)  
* **METAFOCUS SKILLS** (Attune, Discipline Skills and Special Abilities)  
* **COMBAT SKILLS** (Archaic, Modern and Advanced).

Build Points (BP) are the currency used in character creation. All skills cost 1 BP per rank regardless of other character aspects. Players must decide how to spend their limited BP to improve the skills most important for their character concept. For example, a character focused on diplomacy might spend more BP on Social skills, while a character focused on combat might spend more on Combat skills. Characters may receive Skill Points from Species as shown, indicating widely accessible training in the field. All characters receive Skill points from the Faction, Origin and Occupation they have chosen as listed, from the various types of education and training, they will only receive one allotment of Skills from each category. Technology & Metascience Levels will also grant the character skills based on the society's advancements. Tech Levels grant skills in focused areas for particular societies with low levels of advancements, or in general granting the bonus to wherever is needed. The more advanced Meta Levels will grant skills in Attune, Disciplines and Invocations show there understanding of Metasciences such as Magic and Psychic phenomenon.

## 

# **SKILL CATEGORIES & MASTER LIST**

## **I. PHYSICAL SKILLS**

*Primary Attribute: Agility or Strength*

| SKILL NAME | DESCRIPTION |
| ----- | ----- |
| **Acrobatics** | Balance, tumbling, escaping restraints, and aerial maneuvers. |
| **Athletics** | Running, climbing, swimming, jumping, and physical endurance. |
| **Piloting** | Operating vehicles, starships, mecha, and mount-riding. |
| **Stealth** | Hiding, moving silently, and camouflaging oneself. |

### 

## **II. MENTAL SKILLS**

*Primary Attribute: Intellect or Wisdom*

| SKILL NAME | DESCRIPTION |
| ----- | ----- |
| **Alertness** | Environmental perception. Spotting hidden items, hearing noises, and avoiding surprise. |
| **Academics** | Research, general education, and the ability to learn new information. |

#### 

### **KNOWLEDGE SKILLS**

*Specific fields of study and understanding.*

| SKILL NAME | DESCRIPTION |
| ----- | ----- |
| **Appraisal** | Determining value and authenticity. |
| **Business** | Understanding of markets, corporate structures, and finance. |
| **Computers** | Hacking, programming, and system operation. |
| **Culture** | Etiquette, customs, and xenology. |
| **History** | Knowledge of past events and civilizations. |
| **Investigation** | Forensics and deductive reasoning. |
| **Language** | Fluency in alien or ancient tongues. |
| **Logistics** | Supply chain management, distribution networks, and resource allocation. |
| **Medicine** | First aid, surgery, and biology. |
| **Metaphysics** | Study of fundamental reality, the nature of existence, and metaphysical phenomena |
| **Nature** | Flora, fauna, and geology. |
| **Navigation** | Astrogation and pathfinding. |
| **Nobility** | Lineages, heraldry, and politics. |
| **Physics** | Understanding of forces and energy. |
| **Religion** | Myths, deities, and rituals. |
| **Science** | Chemistry, biology, and laboratory procedure. |
| **Survival** | Tracking, foraging, and enduring elements. |
| **Tactics** | Strategy and battlefield command. |
| **Technology** | Engineering, repair, and mechanics. |
| **Trade** | Commerce of supply and demand. |

#### 

### **VOCATION SKILLS**

*Professional trades and crafting.*

| SKILL NAME | DESCRIPTION |
| ----- | ----- |
| **Administrator** | Bureaucracy and management. |
| **Alchemist** | Brewing potions and chemical compounds. |
| **Ambassador** | Official negotiation and representation. |
| **Architect** | Building design and structural engineering. |
| **Archivist** | Librarian and record keeping. |
| **Armorer** | Crafting and repairing armor. |
| **Artist** | Painting, sculpture, and visual arts. |
| **Artificer** | Creating magical or high-tech artifacts. |
| **Broker** | Trading, stocks, and mercantilism. |
| **Celebrity** | Public relations and fame management. |
| **Constable** | Law enforcement protocols. |
| **Courtesan** | Seduction and companionship. |
| **Culinarian** | Cooking and food preparation. |
| **Demolitionist** | Explosives handling and disposal. |
| **Electrician** | Wiring and power systems. |
| **Engineer** | Complex machinery and structural work. |
| **Farmer** | Cultivation of crops, livestock management, and agricultural production. |
| **Groundskeeper** | Landscaping and maintenance. |
| **Handler** | Animal training and care. |
| **Laborer** | Heavy lifting and construction. |
| **Mechanic** | Vehicle and machine repair. |
| **Researcher** | Data analysis and academic study. |
| **Salvager** | Recovering scrap and value from debris. |
| **Soldier** | Military protocol and drilling. |
| **Tailor** | Clothing design and repair. |
| **Transporter** | Logistics and cargo management. |
| **Weaponsmith** | Crafting and repairing weaponry. |

### 

## **III. SOCIAL SKILLS**

*Primary Attribute: Charisma or Wisdom*

#### 

### **MANIPULATION**

*Interpersonal influence and reading others.*

| SKILL NAME | DESCRIPTION |
| ----- | ----- |
| **Insight** | Social perception. Reading emotions, detecting lies, and empathy (formerly Insight). |
| **Barter** | Negotiating prices and goods. |
| **Bluff** | Deception, lying, and disguising intent. |
| **Diplomacy** | Persuasion, negotiation, and etiquette. |
| **Intimidate** | Coercion through fear or physical threat. |
| **Leadership** | Commanding, inspiring, and coordinating group efforts. |
| **Streetwise** | Gathering info, finding black markets, and urban survival. |

#### 

### **EXPRESSION**

*Performance and artistic projection.*

| SKILL NAME | DESCRIPTION |
| ----- | ----- |
| **Acting** | Embodying roles and emotions. |
| **Comedy** | Entertainment through humor. |
| **Dancing** | Rhythmic movement. |
| **Disguise** | Physical alteration of appearance. |
| **Keyboard** | Piano, synth, and terminal interfaces. |
| **Legerdemain** | Sleight of hand and trickery. |
| **Oratory** | Public speaking and speeches. |
| **Percussion** | Drums and rhythm instruments. |
| **Singing** | Vocal performance. |
| **String** | Guitars, violins, and alien equivalents. |
| **Style** | Fashion and personal presentation. |
| **Wind** | Flutes, trumpets, and alien equivalents. |

### 

## **IV. COMBAT SKILLS**

*Primary Attribute: Agility or Strength*

#### 

### **ARCHAIC**

*Traditional and physical combat.*

| SKILL NAME | DESCRIPTION |
| ----- | ----- |
| **Defense** | Parrying, dodging, and using shields. Added to Defense Score. |
| **Melee** | Swords, axes, hammers, and energized melee weapons. |
| **Ranged** | Bows, crossbows, and thrown weapons. |
| **Unarmed** | Brawling, martial arts, and natural weapons. |

#### 

### **MODERN**

*Ballistic and chemical propulsion.*

| SKILL NAME | DESCRIPTION |
| ----- | ----- |
| **Ballistic** | Pistols, rifles, SMGs, and kinetic firearms. |
| **Heavy Weapons** | Rocket launchers, grenades, and vehicle-mounted guns. |

#### 

### **ADVANCED**

*High-tech directed energy.*

| SKILL NAME | DESCRIPTION |
| ----- | ----- |
| **Energy** | Lasers, plasma rifles, and particle beams. |
| **Heavy Energy** | Ship cannons, plasma mortars, and singularity projectors. |

### 

## **V. METAFOCUS SKILLS (Optional)**

*Requires "Awakened" Feature.*

| SKILL NAME | DESCRIPTION |
| ----- | ----- |
| **Attune** | Channeling raw metaphysical energy. Used for casting checks. |
| **Discipline** | Specific schools of magic/psionics (e.g., Telepathy, Entropy). |

## 

## **SKILL SOURCES**

### **SPECIES**

The number of Skill Points a character receives during character creation can vary depending on the species they choose. This reflects the diversity of educational systems and cultural values placed on learning and skill development across different species in the game world.  
Some species, like humans, might have a relatively standardized education system, granting a fixed number of Skill Points to all characters of that species. Others, like the Alterians, might have a more focused education system where individual aptitude and achievements are shown with focused scores and specializations. Still others, like the Dwerg, might have a strong cultural emphasis on certain skills, granting bonus points in those areas specifically, but for the entire Species.  
This variability adds another layer of customization to character creation, allowing players to choose a species that aligns with their desired skill set and playstyle. It also encourages players to think about the cultural and educational background of their chosen species, enriching the role-playing experience and adding depth to the game world.

### 

### **FACTION**

Faction represents a group or organization they are aligned with. This affiliation not only shapes their worldview and goals but also provides them with certain advantages in the form of Skill Points and Features.  
Upon choosing a Faction during character creation, you receive 20 Skill Points that can be allocated towards skills that are suggested or encouraged by that Faction. This allows you to tailor your character's abilities to better fit the Faction's focus and expertise. For example, a character joining a militaristic faction might receive suggested skills like Firearms, Melee Weapons, and Tactics, while a character joining a scientific faction might receive suggested skills like Science, Technology, and Medicine.

### 

### **ORIGIN**

Origin refers to a character's upbringing and background. It represents the foundation upon which their life experiences and worldview are built. During character creation, players can choose from a variety of Origins, each offering unique benefits in the form of Skill Points and Origin Traits.  
Each Origin grants 20 Skill Points that can be allocated towards skills that are relevant to the character's background. For example, a character with a "Nomad" origin might put points into Survival, Animal Handling, and Perception, while a character with an "Aristocrat" origin might invest in Etiquette, Persuasion, and History. This allows players to tailor their character's abilities to reflect their upbringing and experiences.

### 

### **OCCUPATION**

Occupation represents their chosen profession or career. This not only defines their area of expertise but also provides them with specific benefits during character creation.  
When you choose an Occupation for your character, you receive 25 Skill Points that must be allocated towards skills directly related to that profession. This ensures your character has a solid foundation in the skills necessary for their chosen career. For example, a character who selects the "Soldier" occupation might allocate their Skill Points towards Firearms, Melee Weapons, Tactics, and Survival. A character who chooses the "Scholar" occupation might focus on Science, History, Research, and Languages.  
This system allows you to tailor your character's abilities to their profession, making them more effective and believable in their chosen field. It also encourages you to think about the specific skills and knowledge that would be required for your character's Occupation, adding depth and realism to their background and capabilities.

## 

## **SKILL RANKS**

In Tangent, a character's Skill Rank reflects their level of expertise and competence in a particular skill. It's a numerical representation of their training, experience, and/or natural aptitude in that area.  
Cost  
Each Skill Rank costs 1 Build Point (BP) to acquire during character creation, regardless of the type of skill. This means that players need to carefully consider which skills are most important for their character concept and allocate their limited BP accordingly.  
Maximum Starting Rank  
The maximum starting Skill Rank is 11, which corresponds to the "Expert" level of proficiency. Skills may be increased by 1 point per experience reward or session.

## 

## **SKILL MODIFIERS**

Various factors can influence the outcome of a skill check, making it more or less likely for a character to succeed in their attempted action. These modifiers can come from both the character's inherent abilities and the external circumstances of the situation.

### **Character's Ability Score**

The Ability Score that governs the skill being used is a primary modifier. A higher Ability Score provides a larger bonus to the skill check, reflecting the character's natural aptitude and talent in that area. For example, a character with a high Strength score would have a bonus to their Athletics skill checks, while a character with a high Intellect score would have a bonus to their Investigation skill checks.

### **Skill Rank**

The character's Skill Rank also directly modifies the skill check. A higher Skill Rank indicates greater expertise and training, resulting in a larger bonus to the roll. This reflects the character's investment in developing their skills and their experience in using them effectively.

### **Difficulty of the Task**

The Game Master (GM) determines the difficulty of the task being attempted, which is often represented by a Difficulty Class (DC) or a Challenge Rating (CR). More challenging tasks have higher DCs or CRs, requiring higher skill check results to succeed. This ensures that characters face appropriate challenges based on their abilities and the circumstances of the situation.

### **Competence**

This modifier represents the advantage gained from having assistance or guidance from a knowledgeable resource in the skill being used. This could be a mentor, a skilled teammate, or even access to helpful resources like databases, manuals or tutorials. The GM determines the specific bonus based on the helper's expertise and the level of assistance provided.

### **Environmental**

Various environmental factors can also modify skill checks. For example, poor lighting might impose a penalty on a Perception check, while strong winds might make it more difficult to hit a target with a ranged weapon. These modifiers reflect the realistic challenges that characters would face in different environments and situations.

* **Terrain:** Difficult terrain, obstacles, or hazardous conditions can make a task more challenging.  
* **Weather:** Extreme weather, such as storms, fog, or extreme temperatures, can hinder visibility or make it harder to perform certain actions.  
* **Lighting:** Poor lighting conditions can make it difficult to see and increase the chance of failure.  
* **Other Factors:** The GM may introduce other environmental modifiers based on the specific situation, such as background noise, distractions, or unexpected events.

### **Gear**

This modifier reflects the advantage provided by using the right tools or equipment for the job. Having access to specialized gear, high-quality materials, or advanced technology can significantly improve a character's chances of success. The GM determines the bonus based on the quality and relevance of the gear to the task at hand.

### **Luck**

This modifier represents the unpredictable element of chance that can sometimes influence outcomes. It can be a result of random events, fortunate coincidences, or even the use of Karma Points (a game mechanic that allows players to spend points to influence rolls). The GM might award a luck bonus for a particularly clever or daring approach, or impose a penalty for reckless or careless actions.

### **Features**

Some characters might have special abilities, features, or traits that grant bonuses or penalties to specific skill checks. These can represent innate talents, learned techniques, or even cybernetic or magical enhancements. For example, a character with the "Night Vision" feature would have a bonus to their Perception checks in low-light conditions.

### **Meta**

This modifier covers the influence of supernatural or extraordinary abilities on a skill check. Including the use of magic, psychic powers, or other special abilities can enhance or hinder a skill check, or the intervention of divine or otherworldly forces in the character's favor. The GM determines the specific bonus based on the nature of the mythical intervention and its impact on the skill check.

### **Other Situational Factors**

The GM may apply additional modifiers based on the specific circumstances of the situation. This could include factors like the character's emotional state, distractions or interruptions, or the presence of allies or enemies. These modifiers help to create a more dynamic and immersive experience, where the outcome of a skill check is influenced by a variety of factors beyond just the character's raw abilities.  
Important Note

A character can only benefit from one Competence bonus, one Environmental bonus, and so on, even if multiple sources of that modifier type are present. This prevents characters from becoming too powerful by accumulating excessive bonuses from similar sources.

By considering all of these factors, the GM can determine the final modifiers that apply to a skill check, ensuring that the outcome is fair, challenging, and reflective of the character's abilities and the situation at hand. This creates a more engaging and dynamic gameplay experience, where success is not guaranteed but is earned through a combination of skill, strategy, and a bit of luck.

### 

### **Time Modifiers**

The time spent on a task directly impacts the outcome.

* **Rushed:** Taking \-25% time incurs **\-2**; \-50% incurs **\-4**; \-75% incurs **\-8**.  
* **Meticulous:** Taking \+50% time grants **\+2**; \+100% time grants **\+4**; \+200% time grants **\+6**.

---

# **SKILL GROUPS & TITLES**

## **OVERVIEW**

Skills are categorized into five groups: Physical Skills, Mental Skills (further divided into Knowledge and Vocation categories), Social Skills (including Manipulation and Expression categories), Metafocus Skills (encompassing Metafocus Disciplines, Attune and Special Abilities), and Combat Skills (unarmed combat, defense and all weapons training).  
Depending on the Skill Rank, characters achieve specific Titles. These titles serve to highlight the character's proficiency and offer additional mechanical advantages.

## 

## **UNIFIED SKILL RANK PROGRESSION**

| RANK RANGE | TIER TITLE | DESCRIPTION |
| :---: | ----- | ----- |
| **0** | Untrained | No formal training. Relies on raw Attribute. |
| **1 \- 5** | Novice | Apprentice or Hobbyist standard. |
| **6 \- 10** | Trained | Professional standard. Can earn a living. |
| **11 \- 15** | Expert | Veteran or Specialist. Renowned locally. |
| **16 \- 20** | Master | One of the best in the sector. |
| **21 \- 25** | Grand Master | Legendary. Understanding transcends logic. |
| **26+** | Pinnacle | Demigod-like perfection. |

## 

## **TIER DESCRIPTIONS**

### **Untrained (Skill Rank 0\)**

An untrained character might be clumsy and ineffective in combat, lacking the precision, timing, and tactical awareness to utilize weapons or defend themselves effectively.

* **Narrative:** Represents a starting point. Highlights the importance of skill development and encourages creative problem solving despite lack of expertise.

### 

### **Novice (Skill Rank 1-5)**

Characters at the Novice Rank have begun their training. They understand the basics but lack the muscle memory or deep theoretical knowledge of a professional. They are "Studied" or "Apprentices."

### **Trained (Skill Rank 6-10)**

Characters at the Trained Rank are valuable assets in any team or organization. They possess the knowledge, experience, and reliability to contribute effectively to a wide range of tasks.

* **Status:** Professional. They can earn a living wage with this skill.  
* **Benefit:** Access to standard "Trained Only" uses of skills (e.g., picking complex locks, disarming explosives).

### 

### **Expert (Skill Rank 11-15)**

Characters who reach the Expert Rank are specialists. They are often sought after for their specific knowledge. They understand the nuance of the skill beyond the textbook.

* **Skillful Professional:** Reduced penalty for Rushing. Experts can work rapidly without sacrificing too much accuracy.  
* **Meticulous Bonus:** Increases to \+3 (per stage) when taking time.

### 

### **Master (Skill Rank 16-20)**

Masters have dedicated their lives to the skill.

* **Meticulous Work:** One of the key benefits of being a Master is the increased bonus for meticulous work. When a Master takes their time, they receive an even greater bonus, reflecting the improved quality of their work.  
* **Bonus Increase:** Meticulous bonus increases by 50% (to 3, 6, 9 or 12).

### 

### **Grand Master (Skill Rank 21-25)**

A Grand Master represents an exceptional Rank of skill mastery, surpassing even the impressive capabilities of a Master. These individuals are true virtuosos, possessing an intuitive understanding of their craft.

* **Rushed Work:** A Grand Master has a decreased penalty for rushed work. A Grand Masters confidence is unwavering even when in a hurry.  
* **Bonus Increase:** Rushed penalty reduced by half (to \-1, \-2 or \-4).

### 

### **Pinnacle (Skill Rank 26+)**

**Virtuoso.**

* **The Golden Rule:** A Pinnacle character may choose to **Take 10** in *any* situation, including combat or high-stress scenarios.  
* **Narrative:** Their mastery is so complete that failure is almost unthinkable. They perform with effortless grace, precision, and creativity, achieving results that others can only dream of.

# **\- PHYSICAL SKILLS**

# **PHYSICAL SKILLS**

These skills rely on the character's bodily fitness, coordination, and agility.

## 

## **Acrobatics**

* **Ability:** Agility  
* **Description:** Acrobatics is the skill of performing feats of gymnastics, extraordinary balance, and dance (and related performance arts), and breaking falls. It is used whenever a character attempts to make a physical maneuver that requires agility, balance, and coordination. This can include things like jumping over obstacles, swinging from ropes, or performing aerial maneuvers. The ability to perform feats of gymnastics, balance, tumbling, and breaking falls. It covers keeping one's footing on slippery surfaces or tightropes and maneuvering through threatened areas without being hit.  
* **Mechanics:**  
  * *Balance:* DC 10 (narrow surface) to DC 20+ (razor edge).  
  * *Tumble:* DC 15 to move through an enemy's threatened area without provoking an attack.  
  * *Break Fall:* Reduce falling damage (DC 15 ignores first 10ft).  
* **Specialties:**   
  * **Tumbling:** Executing offensive or defensive rolls across hard or hazardous terrain.  
  * **Tightrope & Ledge Balancing:** Retaining equilibrium along narrow beams, girders, and razor edges.  
  * **Parkour & Free-Running:** Traversing complex vertical and urban environments at full speed.  
  * **Breakfalling:** Mitigating kinetic shock and impact damage from catastrophic falls.  
  * **Zero-G Maneuvering:** Propelling, rotating, and anchoring oneself in microgravity or vacuum.  
  * **Escapology:** Slipping bonds, manacles, energy cuffs, and narrow biological entrapments.  
  * **Wall-Running:** Using kinetic momentum to traverse perpendicular surfaces briefly.  
  * **Aerial Trapeze & Swing:** Calculating and grabbing ropes, cables, and swinging hazards in motion.  
  * **Dive & Roll:** Clearing sudden ground-level traps, blast radii, or closing bulkheads.  
  * **Kip-Up & Rapid Recovery:** Snapping instantly to one's feet from a prone state without defensive penalties.

## 

## **Athletics**

* **Ability:** Strength  
* **Description:** Athletics is the skill of a character's general physical capabilities – running, jumping, climbing, swimming, and other broadly physical activities. It reflects a character's overall fitness and physical prowess. Athletics is used whenever a character attempts to perform a physical feat that requires strength and endurance. A measure of general physical prowess. It encompasses running, jumping, climbing, swimming, and lifting heavy objects.  
* **Mechanics:**  
  * *Climb:* DC 10 (rope with knots) to DC 25 (sheer surface). Speed is usually half base speed.  
  * *Jump:* DC equals the distance in feet (running start).  
  * *Swim:* DC 10 (calm water) to DC 20 (stormy water). Failure by 5 or more risks drowning.  
* **Specialties:**   
  * **Sprinting:** Short-burst explosive acceleration over open terrain.  
  * **Endurance Running:** Long-distance marches and prolonged pursuits without fatigue.  
  * **Vertical & Long Jumping:** Clearing horizontal chasms and scaling sheer heights by leaping.  
  * **Rock & Hull Climbing:** Scaling sheer cliffs, metal lattice bulkheads, and exterior starship hulls.  
  * **Swimming & Diving:** Navigating high-viscosity liquids, aquatic currents, and submerged operations.  
  * **Powerlifting & Hauling:** Lifting, pressing, and dragging massive bulk, collapsed beams, and heavy machinery.  
  * **Forceful Throwing:** Hurling objects, grenades, or improvised bludgeons with maximum velocity.  
  * **Grapple Wrestling:** Overpowering and pinning opponents through superior bodily mass and leverage.  
  * **High-G Conditioning:** Resisting the physical and circulatory strain of high-gravity worlds and high-G acceleration.  
  * **Breaching & Forcing:** Kicking in heavy security doors, forcing airlocks, and smashing barriers.

## 

## **Piloting**

* **Ability:** Agility (Reaction) or Intellect (Navigation/Systems)  
* **Description:** Piloting is the skill of operating mecha, vehicles of various types, such as aircraft, spacecraft, or boats. It reflects a character's ability to navigate, steer, and control various vehicles. Piloting is used whenever a character attempts to pilot a vehicle, whether in combat or during a peaceful journey. This can include things like flying a spaceship through an asteroid field, navigating a boat through a storm, or driving a car through a high-speed chase.   
* **Mechanics:**  
  * *Normal Operation:* No roll required for standard travel.  
  * *Stunts/Combat:* DC depends on maneuver difficulty (DC 15 for sharp turns at speed, DC 25 for "threading the needle").  
* **Specialties:**   
  * **Wheeled & Tracked Vehicles:** Driving rovers, combat buggies, tanks, and mobile command platforms.  
  * **Hovercraft & Grav-Speeders:** Operating repulsorlift bikes, grav-skimmers, and low-altitude platforms.  
  * **Atmospheric Aircraft:** Flying atmospheric jets, rotorcraft, gunships, and gliders.  
  * **Sub-Orbital Shuttles:** Piloting orbital dropships, landers, and transit shuttles through atmospheric entry.  
  * **Starfighters & Dogfighting:** High-speed space combat, interceptor maneuvers, and close-quarters dogfights.  
  * **Capital Ships & Freighters:** Navigating massive multi-deck vessels, bulk freighters, and naval cruisers.  
  * **Mecha & Walkers:** Piloting bipedal or multipedal combat chassis and industrial bipedal rigs.  
  * **Watercraft & Submersibles:** Commanding surface naval boats, aquatic hydrofoils, and deep-sea exploration pods.  
  * **EVA Thruster Packs:** Precision thruster control during spacewalks and exterior repairs.  
  * **Combat Stunts & Evasion:** Threading asteroid clusters, evading surface-to-air missiles, and extreme defensive barrel rolls.

## 

## **Stealth**

* **Ability:** Agility  
* **Description:** Stealth is the skill of moving quietly and remaining hidden from view. It reflects a character's ability to avoid detection and move silently. Stealth is used whenever a character attempts to sneak past guards, hide from enemies, or remain undetected while spying. This can include things like sneaking through a dark alley, hiding behind a tree, or crawling through a ventilation shaft. The ability to move silently and remain unseen. This is an opposed check against an observer's *Alertness*.  
* **Mechanics:**  
  * *Hide:* Your check becomes the DC for the enemy's Perception/Alertness check. Requires cover or concealment.  
  * *Move Silently:* Move at half speed without penalty. Moving faster imposes penalties (-5 for full speed).  
* **Specialties:**   
  * **Silent Movement:** Walking, running, and shifting weight across creaky, dry, or metal grating without noise.  
  * **Camouflage & Concealment:** Blending into natural foliage, rubble, or static shadows using environmental gear.  
  * **Urban Infiltration:** Skulking through corporate arcologies, maintenance ducts, alleyways, and populated zones.  
  * **Wilderness Stalking:** Approaching targets through alien jungles, tundras, or rocky outcroppings without disturbing wildlife.  
  * **Sensor Evasion:** Avoiding infrared scanners, thermal cameras, motion trackers, and bio-scanners.  
  * **Tailing & Shadowing:** Following marks through crowded spaceports or urban avenues without alerting them.  
  * **Crowd Blending:** Disappearing into pedestrian traffic and public plazas via nondescript body language.  
  * **Vibration & Scent Masking:** Evading tracking beasts and species relying on non-visual olfactory or acoustic senses.  
  * **Quick-Cover Snapping:** Darting between cover zones while security patrols or camera sweeps cycle past.  
  * **Infiltration Rigging:** Placing acoustic and optical cloaks over gear to silence rattling holsters and gear packs.

# **\- MENTAL SKILLS**

# **MENTAL SKILLS**

These skills utilize the character's mind, education, and sensory acuity.

## 

## **ALERTNESS**

* **Ability:** Perception (Intellect \+ Wisdom)  
* **Description:** Alertness is the skill of being aware of one's surroundings and noticing changes or potential threats in the environment. It reflects a character's ability to pay attention to details and react quickly to unexpected situations. Alertness is used whenever a character needs to be aware of their surroundings, whether it's to avoid danger or to gather information. This can include things like noticing a hidden door, detecting a trap, or identifying a potential ambush.  
* **Mechanics:**  
  * *Spot/Listen:* Opposed by Stealth.  
  * *Search:* DC 10 (easy) to DC 30 (microscopic detail).  
* **Specialties:**   
  * **Visual Spotting:** Identifying hidden snipers, optical glints, camouflaged personnel, and concealed traps.  
  * **Auditory Listening:** Eavesdropping through bulkheads, picking out hushed footsteps, and identifying weapon charging hums.  
  * **Ambush Detection:** Sensing structural irregularities, kill-zone geometry, and unnatural environmental stillness.  
  * **Concealed Object Search:** Uncovering secret compartments, hidden weapon holsters, false deckplates, and covert bugs.  
  * **Scent & Pheromone Recognition:** Identifying chemical signatures, ozone leaks, and tracking alien scents.  
  * **Sensor Feed Monitoring:** Catching blips, radar ghosting, and telemetry spikes on passive scanner consoles.  
  * **Danger Sense & Intuition:** Reading subtle atmospheric pressure changes and gut feelings before violence erupts.  
  * **Counter-Surveillance Awareness:** Noticing when you are being tailed, scanned, or watched from high vantage points.  
  * **Micro-Detail Inspection:** Spotting minute hairline fractures in bulkheads, micro-cameras, and hairline tripwires.  
  * **Motion Tracking:** Tracking high-speed peripheral movement across low-light and visual-clutter environments.

## 

## **ACADEMICS**

* **Ability:** Intellect  
* **Description:** Academics is the skill of knowledge acquisition and analysis. It reflects a character's ability to research and understand complex topics. Academics is used whenever a character needs to research a topic, analyze data, or draw conclusions based on evidence. This can include things like researching a new technology, analyzing an artwork or deciphering old texts.   
* **Mechanics:**  
  * *Research:* DC 15 to find obscure information in a library/database.  
* **Specialties:**   
  * **Research Methodology:** Efficiently querying galactic libraries, university databases, and scholarly compendiums.  
  * **Critical Epistemology:** Evaluating the validity, systemic bias, and objective truth of complex source materials.  
  * **Data Synthesis:** Cross-referencing disparate academic fields to extrapolate working theories and hypotheses.  
  * **Scholarly Archiving:** Classifying, indexing, and citing academic literature according to galactic standards.  
  * **Pedagogy & Instruction:** Formulating structured curricula to teach complex theoretical concepts to novices.  
  * **Technical Writing:** Drafting comprehensive peer-reviewed papers, grant proposals, and white papers.  
  * **Abstract Logic:** Solving complex mathematical logic gates, philosophical proofs, and systemic dilemmas.  
  * **Linguistic Philology:** Tracing the roots, evolution, and semantic mutations of languages across galactic eras.  
  * **Comparative Sociology:** Analyzing societal development structures across disparate humanoid and alien cultures.  
  * **Scholastic Peer Review:** Detecting fabrication, confirmation bias, and flawed datasets in published scientific reports.

## 

## **KNOWLEDGE (Category)**

* **Ability:** Intellect  
* **Mechanics:**  
  * *Common Knowledge:* DC 10\.  
  * *Obscure Fact:* DC 20\.  
  * *Lost/Forbidden Knowledge:* DC 30+.  
* **Skill Lists:**

### 

  ### **APPRAISAL**

* **Ability:** Intellect  
* **Description:** The skill of determining the value, authenticity, and origin of items. It allows a character to identify rare artifacts, detect forgeries, and estimate the market price of goods.  
* **Mechanics:**  
  * **Value Item:** DC 10 (Common) to DC 30 (Unique/Alien).  
  * **Detect Forgery:** Opposed check vs. Forgery (Bluff/Craft).  
  * **Identify Properties:** DC 15+ to determine what a specific device or artifact does.  
* **Specialties:**   
  * **Antiquities & Relics:** Dating and pricing ancient artifacts, dynastic treasures, and pre-collapse relics.  
    * **Gems & Exotic Minerals:** Assessing crystalline purity, carats, cut, and value of rare planetary ores.  
    * **High-Tech Hardware:** Evaluating internal wear, chip fabrication specs, and market value of modern computing cores.  
    * **Xenotech Artifacts:** Identifying the rarity, function, and black-market pricing of unknown alien devices.  
    * **Fine Art & Holography:** Spotting stylistic authenticity, provenance, and valuations in visual art and media.  
    * **Weapons & Armor Valuation:** Pricing arms, determining custom modifications, and grading combat durability.  
    * **Counterfeit & Forgery Detection:** Spotting faux stamps, reproduction alloys, doctored serials, and artificial aging.  
    * **Raw Industrial Commodities:** Estimating bulk container loads of refined fuels, hyper-alloys, and construction composites.  
    * **Metaphysical Artifacts:** Valuing enchanted talismans, psi-receptive matrices, and mana-infused items.  
    * **Salvage Bulk Valuation:** Estimating the scrap yield and recovery profit of derelict ship hulls and shattered stations.

  ### 

  ### **BUSINESS**

* **Ability:** Intellect  
* **Description:** The skill of understanding markets, corporate structures, economics, and business operations. It allows a character to analyze market trends, understand financial systems, and manage business ventures.  
* **Mechanics:**  
  * *Analyze Market:* DC 15 to predict trends or find investment opportunities.  
  * *Corporate Insight:* DC 20 to understand internal company politics or organizational structures.  
  * *Negotiation:* DC 15+ to facilitate business deals.  
* **Specialties:**   
  * **Corporate Law & Governance:** Understanding regulatory filings, compliance liabilities, and corporate charters.  
    * **Market Trend Analysis:** Forecasting supply, demand, and economic fluctuations across star sectors.  
    * **Investment & Portfolio Management:** Allocating financial capital across ventures, equities, and resource bonds.  
    * **Mergers & Acquisitions:** Conducting corporate takeovers, buyout evaluations, and shell company absorptions.  
    * **Venture Capitalism:** Spotting promising tech startups and high-yield frontier resource ventures.  
    * **Fiscal Auditing:** Tracing hidden accounting ledgers, corporate embezzlements, and balance sheet discrepancies.  
    * **Contract Architecture:** Drafting binding corporate NDAs, licensing agreements, and commercial partnerships.  
    * **Marketing & Public Relations:** Designing campaign rollouts and brand identity to maximize sector-wide sales.  
    * **Risk Management:** Mitigating operational losses due to sector wars, market crashes, and supply disruptions.  
    * **Corporate Shell Networks:** Establishing holding syndicates to protect personal assets from tax levies and seizures.

  ### **COMPUTERS**

* **Ability:** Intellect  
* **Description:** Computer is the skill of understanding and using computer technology. It reflects a character's ability to operate and program computers, and to understand the underlying principles of computer science. Computer is used whenever a character needs to use a computer to perform a task, whether it's to hack into a system or to analyze data. This can include things like programming, hacking, and data analysis.  
* **Mechanics:**  
  * **Operate Remote System:** DC 10 to DC 25 depending on security.  
  * **Hacking:** DC is determined by the target system's firewall/security rating (Security Level \+ 10).  
  * **Programming:** DC 15 (Simple Script) to DC 30+ (AI architecture).  
* **Specialties:**   
  * **Network Intrusion (Hacking):** Cracking firewalls, breaching intrusion detection systems, and gaining administrative root.  
    * **Cyber-Defense & Hardening:** Patching zero-day vulnerabilities, setting up honeypots, and countering rival hackers.  
    * **Scripting & Process Automation:** Writing rapid macros and terminal scripts to automate complex digital procedures.  
    * **AI Architecture & Training:** Developing, modifying, and debugging synthetic neural networks and heuristic models.  
    * **Data Mining & Parsing:** Sifting petabytes of corporate and planetary databases to extract target information.  
    * **Firmware & Low-Level Code:** Interfacing with raw machine code, BIOS chips, and low-level kernel drivers.  
    * **Cryptographic Decryption:** Brute-forcing and mathematically unraveling complex data encryption keys.  
    * **Neural Interface Protocols:** Configuring direct cybernetic brain-machine links and wetware jacks.  
    * **Distributed Cloud Systems:** Managing planetary data farms, decentralized storage hubs, and quantum cloud relays.  
    * **Counter-Forensic Scrubbing:** Erasing digital fingerprints, bounce-routing terminal traces, and wiping system logs.

  ### 

  ### **CULTURE**

* **Ability:** Intellect  
* **Description:** Culture is the skill of understanding and appreciating different cultures. It reflects a character's ability to understand the customs, beliefs, and practices of different groups of people. Culture is used whenever a character needs to interact with people from different cultures, whether it's to negotiate a deal or to avoid offending someone. This can include things like understanding social norms, speaking different languages, and recognizing cultural symbols.   
* **Mechanics:**  
  * **Etiquette Knowledge:** DC 10 (Common custom) to DC 25 (Obscure ritual).  
  * **Identify Origin:** DC 15 to identify a person's origin based on dress or behavior.  
* **Specialties:**   
  * **Impyrium High Society:** Navigating the subtle etiquette, titles, and ancestral hierarchies of the imperial core.  
    * **Dynasty Lineages:** Understanding the honor codes, draconic influences, and clan hierarchies of the Dynasty.  
    * **Underworld Subcultures:** Knowing street cant, gang boundary markers, and honor-among-thieves traditions.  
    * **Alien Xenology (Auluran/Kizin/Dwerg):** Understanding the behavioral norms, biology, and taboos of major species.  
    * **Frontier & Nomad Customs:** Grasping the survival rituals, barter courtesies, and rites of deep-space void-clans.  
    * **Corporate Megacorp Culture:** Knowing executive ranks, intra-office politics, and corporate loyalty dogma.  
    * **Planetary Folkways:** Adapting quickly to localized, isolated religious and traditional practices of frontier worlds.  
    * **Diplomatic Hospitality Protocols:** Understanding the sacred guest rites, banquet behaviors, and host responsibilities.  
    * **Taboos & Insults:** Recognizing subtle physical gestures, eye contact rules, or color choices that spark lethal offense.  
    * **Galactic Pop Trends:** Tracking the fads, underground music scenes, and holistic lifestyle shifts of urban youths.

  ### 

  ### **HISTORY**

* **Ability:** Intellect  
* **Description:** History is the skill of understanding and interpreting historical events. It reflects a character's ability to understand the past and how it has shaped the present. History is used whenever a character needs to understand the context of a situation, whether it's to solve a mystery or to plan a strategy. This can include things like researching historical events, analyzing historical documents, and interpreting historical artifacts.   
* **Mechanics:**  
  * **Recall Event:** DC 10 (Recent/Famous) to DC 30 (Pre-history/Lost).  
  * **Legend Lore:** DC 20 to know rumors or legends about a specific place or item.  
* **Specialties:**   
  * **Ancient Civilizations:** Studying the structures, fall, and artifacts of long-dead interstellar empires.  
    * **Galactic Wars & Campaigns:** Recalling operational movements, treaties, and turning points of sector-wide conflicts.  
    * **Planetary Colonization:** Knowing the initial settlement waves, generational ships, and terraforming histories of worlds.  
    * **Technological Evolution:** Understanding the timeline of hyperdrive breakthroughs, laser discoveries, and AI bans.  
    * **Political Revolutions:** Analyzing past civil uprisings, overthrow of despots, and governmental transitions.  
    * **House & Dynasty Lineages:** Tracing historical royal successions, usurpations, and aristocratic marriages.  
    * **Archaeological Context:** Dating architectural ruins, stratigraphy, and pre-collapse relic sites.  
    * **Religious Schisms:** Knowing the theological splits, crusades, and doctrinal disputes that formed current faiths.  
    * **Pre-Void Collapse Lore:** Recalling historical records and legends preceding the great cosmological cataclysms.  
    * **Economic Booms & Panics:** Studying historical trade collapses, embargo famines, and planetary bankruptcies.

  ### 

  ### **INVESTIGATION**

* **Ability:** Intellect (sometimes Perception)  
* **Description:** The application of deductive reasoning, forensics, and information gathering to solve mysteries. It differs from Alertness (which is spotting things) by focusing on *analyzing* what is found.  
* **Mechanics:**  
  * **Analyze Clue:** DC 15 to determine how a clue fits into the larger picture.  
  * **Search/Forensics:** DC 15+ to find microscopic evidence or wiped data.  
* **Specialties:**   
  * **Crime Scene Reconstruction:** Determining angles of fire, point of entry, sequence of events, and struggles.  
    * **Forensic Ballistics & Trauma:** Matching projectile deformation, burn marks, and wound tracks to weapons.  
    * **Digital Profiling & Tracing:** Piecing together target routines, communication meta-logs, and travel manifests.  
    * **Evidence Preservation:** Collecting, bagging, and securing micro-traces, hairs, fibers, and chemical residue cleanly.  
    * **Interrogation Deduction:** Spotting factual contradictions, nervous tics, and narrative inconsistencies during questioning.  
    * **Financial Paper Trails:** Unraveling money laundering webs, hidden ledger transfers, and payment drops.  
    * **Physical Pattern Tracking:** Connecting serial crimes, matching operational signatures across star systems.  
    * **Cryptanalysis:** Solving handwritten ciphers, hidden dead-drop marks, and coded personal journals.  
    * **Surveillance Audio/Video Analysis:** Cleaning up audio feeds, enhancing reflection data, and mapping blind spots.  
    * **Cold Case Deduction:** Evaluating historical evidence files to surface overlooked facts or corrupt cover-ups.

### 

  ### **LANGUAGE**

* **Ability:** Intellect  
* **Description:** Language is the skill of understanding and speaking different languages. It reflects a character's ability to communicate with people from different cultures and backgrounds. Language is used whenever a character needs to communicate with someone who speaks a different language, whether it's to negotiate a deal or to gather information. This can include things like speaking, reading, and writing different languages.   
    
* Specializations for this skill can include specific languages.  
  **Notable Languages:**  
  Common or Trade \- Known by most, mix of various languages and dialects  
  Draconic \- Tongue of Dragons, Draconic and Draconians, common in the Dynasty  
  Premian \- Spoken by Native Premians and common in the Syndicate  
  Tech \- Pidgin and Jargon common to scientist and technicians  
  Fae \- A shared semi-mystical tongue of beings and spirits of nature  
  High Tongue \- Impyrium, Ascendancy and some old Dynasty worlds  
  Sign \- Hand gesture communication  
  ***Note** \- Regional and World variants are very common but are usually based on a listed language (for most communication a check is generally not needed, indicating fluency in chosen language \- whether spoken, written or any other context of that dialect. Rare dialects and old forms of the language will require checks to understand and communicate).*  
* **Mechanics:**  
  * **Decipher Script:** DC 20 (Simple code/Unknown dialect) to DC 30+ (Ancient/Alien cipher).  
  * **Fluency:** Ranks determine fluency (1-2: Broken, 3-5: Fluent, 6+: Native/Technical).  
* **Specialties:**   
  * **Trade Common Fluency:** Reading, writing, and speaking the primary universal interstellar commerce tongue.  
    * **Draconic Dialects:** Fluency in the guttural, complex structural syntax of the Dynasty worlds.  
    * **Premian Tongue:** Conversational and formal mastery of the native Premian languages of the Syndicate.  
    * **Tech-Jargon & Machine Cant:** Interpreting compressed technical codes, system shorthand, and technician argot.  
    * **Ancient & Dead Scripts:** Translating petroglyphs, ancestral tablets, and forgotten pre-space scripts.  
    * **Fae & Metamystic Dialects:** Deciphering the symbolic, layered semantics of mystical and natural entities.  
    * **Imperial High Tongue:** Conversing in the formal, flowery, and strictly regimented language of the Impyrium court.  
    * **Tactical Sign Language:** Communicating complex tactical orders silently via standard military hand signals.  
    * **Xeno-Linguistic Decryption:** Mapping unknown alien phonetic frameworks using mathematical syntax models.  
    * **Lip Reading:** Discerning spoken words visually across silent rooms, surveillance video, or soundproof glass.

  ### 

### **LOGISTICS**

* **Ability:** Intellect  
* **Description:** The skill of managing the flow of resources, supply chains, and large-scale operations. It reflects a character's ability to plan efficient movement, optimize inventory levels, and coordinate resource allocation. Logistics is used whenever a character needs to organize complex logistical operations, whether for military, industrial, or commercial purposes.  
* **Mechanics:**  
  * *Supply Chain Management:* DC 15 to establish efficient supply routes or storage solutions.  
  * *Resource Allocation:* DC 20 to optimize resource distribution during high-demand or crisis scenarios.  
  * *Transport Planning:* DC 15+ to coordinate complex multi-modal shipments or fleet movements.  
* **Specialties:**   
  * **Interstellar Supply Chains:** Organizing transport schedules across jump gates and hyperspace lanes.  
    * **Fleet Fuel & Resource Management:** Calculating propellant reserves, reactor fuel burn, and life support replenishment.  
    * **Warehouse & Depot Operations:** Optimizing cargo space, autonomous pallet storage, and loading bay throughput.  
    * **Wartime Requisitioning:** Sourcing, prioritizing, and deploying arms, ammunition, and rations to active frontlines.  
    * **Hazardous Cargo Transport:** Managing temperature, shielding, and containment protocols for volatile shipments.  
    * **Just-In-Time Distribution:** Ensuring critical materials arrive exactly when required to minimize storage overhead.  
    * **Cargo Manifest Auditing:** Detecting missing crates, manifest skimming, unauthorized stowaways, and smuggled tonnage.  
    * **Emergency Relief Staging:** Rapidly deploying modular hospitals, rations, and water purification to disaster worlds.  
    * **Multi-Modal Route Optimization:** Seamlessly integrating orbital drop, rail, atmospheric shuttle, and ground hauling.  
    * **Disassembly & Staging:** Breaking down massive industrial machinery into shippable container modules.

  ### 

  ### **MEDICINE**

* **Ability:** Wisdom (Intellect for surgery/diagnosis)  
* **Description:** Medicine is the skill of understanding and treating medical conditions. It reflects a character's ability to diagnose and treat illnesses and injuries. Medicine is used whenever a character needs to heal themselves or others, whether it's to save a life or to prevent an infection. This can include things like diagnosing illnesses, performing surgeries, and administering medication.   
* **Mechanics:**  
  * **First Aid:** DC 15 to stabilize a dying character.  
  * **Long-term Care:** DC 15 to double natural healing rates for a patient.  
  * **Surgery/Treat Disease:** DC varies by severity (DC 15 to DC 30).  
* **Specialties:**   
  * **Combat Trauma & Triage:** Stabilizing life-threatening wounds, stopping arterial bleeds, and patching sucking chests.  
    * **Field Surgery:** Performing amputations, extracting shrapnel, and suturing organs under battlefield conditions.  
    * **Xenobiology Treatment:** Diagnosing and treating disparate alien biologies, organs, and foreign blood chemistries.  
    * **Pharmacology & Toxicology:** Synthesizing antidotes, administering pain management drugs, and treating poisons.  
    * **Cybernetic Implantation:** Surgical installation, calibration, and rejection management of bionic limbs and organs.  
    * **Infectious Diseases & Pathology:** Identifying bio-warfare vectors, alien spores, viruses, and designing quarantines.  
    * **Intensive Care Life-Support:** Managing coma patients, critical stasis preservation, and multi-system organ failure.  
    * **Gene Therapy:** Repairing radiation-induced DNA breaks and treating genetic degenerative diseases.  
    * **Psychiatry & Neural Trauma:** Treating combat shock, chemical psychosis, and neural interface feedback loops.  
    * **Forensic Autopsy:** Determining precise cause, timeline, and physical weapon signatures on deceased bodies.

  ### **METAPHYSICS**

* **Ability:** Intellect  
* **Description:** Metaphysics is the skill of understanding the fundamental nature of reality, existence, and the underlying principles of the universe. It reflects a character's ability to grasp abstract concepts, perceive patterns in the fabric of existence, and understand the limits of physical laws. Metaphysics is used whenever a character needs to comprehend anomalies, navigate realities, or understand the nature of psionic or magical energy.  
* **Mechanics:**  
  * *Identify Pattern:* DC 15 to recognize repeating cosmological or mystical patterns.  
  * *Analyze Reality:* DC 20 to determine if a location is subject to unusual physical or planar laws.  
  * *Theorize:* DC 25 to formulate a hypothesis about a high-level cosmic event.  
* **Specialties:**   
  * **Planar Mechanics:** Charting the geometry, boundaries, and bleed-through zones of alternate dimensions.  
    * **Cosmological Anomaly Analysis:** Diagnosing spatial rifts, localized reality tears, and planar convergence points.  
    * **Psionic Field Theory:** Mapping neural energy waves, ESP transmission conduits, and psi-receptive matter.  
    * **Magitech Integration Theory:** Designing circuit schematics that marry micro-electronics with arcane Ley matrices.  
    * **Reality Anchoring:** Calculating stabilization thresholds to prevent reality dissolution by chaotic forces.  
    * **Entropy & Void Dynamics:** Modeling decay radiation, Void incursions, and vacuum energy extractions.  
    * **Ley Line & Node Cartography:** Locating and calculating the energetic output of planetary meta-currents.  
    * **Temporal Disruption Theory:** Modeling localized chrono-stasis fields, causality loops, and paradox vectors.  
    * **Aura Diagnostics:** Analyzing the metaphysical luminescence and spiritual health of awakened entities.  
    * **Metascience Epistemology:** Formulating repeatable, scientific test standards for otherwise mystical occurrences.

  ### 

  ### **NATURE**

* **Ability:** Intellect (or Wisdom for practical application)  
* **Description:** Nature is the skill of understanding and interacting with the natural world. It reflects a character's ability to survive in the wilderness and understand the ecology of different environments. Nature is used whenever a character needs to navigate through the wilderness, whether it's to hunt for food or to avoid danger. This can include things like tracking animals, identifying plants, and surviving in extreme environments.   
* **Mechanics:**  
  * **Identify Creature:** DC 10 \+ Creature's Rarity/CR.  
  * **Foraging/Remedies:** DC 15 to find herbal antidotes or food.  
* **Specialties:**   
  * **Alien Botany:** Identifying toxic, medicinal, and nutritional properties of wild planetary plants.  
    * **Zoology & Beast Behavior:** Predicting predator attack vectors, nesting grounds, and pack dynamics.  
    * **Geology & Speleology:** Analyzing rock strata, seismic fault lines, mineral stability, and cavern layouts.  
    * **Meteorology & Weather Forecasting:** Predicting dangerous ionic storms, hurricanes, acid rain, and sudden blizzards.  
    * **Marine & Aquatic Ecology:** Navigating coastal shoals, identifying predatory sea life, and assessing tides.  
    * **Toxicology Sourcing:** Extracting potent venom, paralyzing secretions, and irritants from wild creatures.  
    * **Biome Tracking:** Identifying disturbed ground, broken twigs, scat, and animal territory scent posts.  
    * **Ecosystem Modeling:** Assessing ecological collapses, invasive species introductions, and bio-cycles.  
    * **Spelunking & Underground Survival:** Finding breathable air pockets, navigating cave-ins, and finding subterranean water.  
    * **Micro-Ecology & Spores:** Detecting airborne fungal infestations, micro-parasites, and spore clouds.

  ### 

  ### **NAVIGATION**

* **Ability:** Intellect (Astrogation) or Wisdom (Survival)  
* **Description:** Navigation is the skill of understanding and using navigational tools. It reflects a character's ability to navigate through different environments, whether it's on land, sea, or air. Navigation is used whenever a character needs to travel to a specific location, whether it's to explore a new area or to deliver a package. This can include things like using a map and compass, piloting a ship, or flying a plane.  
* **Mechanics:**  
  * **Plot Course:** DC 10 (Familiar) to DC 25 (Uncharted/Hazardous).  
  * **Astrogation:** Calculating FTL jumps (DC varies by distance and route stability).  
* **Specialties:**   
  * **Astrogation & FTL Calculations:** Plotting jump-vectors through hyperspace, avoiding gravity wells and stars.  
    * **Planetary Orienteering:** Traversing wilderness using compasses, star arrays, natural landmarks, and elevation maps.  
    * **Cartography & Mapmaking:** Drafting detailed topographical, nautical, and three-dimensional celestial maps.  
    * **Dead Reckoning:** Calculating current position based strictly on velocity, time, and previous heading.  
    * **Subterranean Navigation:** Mapping complex cavern networks, mine tunnels, and under-city catacombs.  
    * **Drift & Anomaly Routing:** Navigating around radiation belts, ion storms, asteroid fields, and spatial debris.  
    * **Orbital Mechanics:** Calculating re-entry angles, geosynchronous orbits, and slingshot escape velocities.  
    * **Nautical & Marine Navigation:** Charting ocean currents, reefs, depth soundings, and dead-water channels.  
    * **Sensor-Blind Navigation:** Flying or driving purely on inertial instruments when sensors are completely jammed.  
    * **Waystation & Gate Wayfinding:** Knowing established sector beacon networks, gate corridors, and toll routes.

  ### 

  ### **NOBILITY**

* **Ability:** Intellect  
* **Description:** Nobility is the skill of understanding and interacting with the aristocracy. It reflects a character's ability to navigate a society's social hierarchy and interact with people of high status. Nobility is used whenever a character needs to interact with the aristocracy, whether it's to gain political power or to secure a marriage alliance. This can include things like understanding court etiquette, negotiating with nobles, and identifying noble titles.   
* **Mechanics:**  
  * **Identify Heraldry:** DC 10 (Major House) to DC 25 (Obscure lineage).  
  * **Political Insight:** DC 15 to understand current political intrigues.  
* **Specialties:**   
  * **Imperial Dynasties:** Tracing succession charts, legitimate heirs, and marriage covenants of Great Houses.  
    * **Heraldry & Insignia:** Identifying house crests, livery patterns, honor banners, and personal sigils.  
    * **Aristocratic Etiquette:** Knowing correct forms of address, seating arrangements, and bow depths for high nobility.  
    * **Feudal Law & Land Rights:** Interpreting ancestral planetary charters, fief titles, and aristocratic privileges.  
    * **Court Intrigue & Factionalism:** Identifying secret alliances, covert blood feuds, and patronage webs among nobility.  
    * **Chivalric Orders:** Knowing the codes, members, sworn oaths, and tournament rights of titled knightly groups.  
    * **Diplomatic Immunity Protocols:** Invoking or circumventing noble sovereign immunity and peerage protections.  
    * **Precedence Arbitration:** Resolving violent squabbles between visiting lords over status, rank, and parade priority.  
    * **Royal Matchmaking:** Calculating political and financial yields of proposed inter-house dynastic marriages.  
    * **Private Royal Treasuries:** Understanding the asset holdings, private guards, and sovereign wealth funds of dukes and kings.

  ### 

  ### **PHYSICS**

* **Ability:** Intellect  
* **Description:** Physics is the skill of understanding and applying the laws of physics. It reflects a character's ability to understand the physical world and use that knowledge to solve problems. Physics is used whenever a character needs to understand the behavior of physical objects, whether it's to design a machine or to solve a puzzle. This can include things like calculating trajectories, designing experiments, and understanding the properties of matter and energy.   
* **Mechanics:**  
  * **Solve Problem:** DC 15 to DC 30 depending on complexity.  
  * **Analyze Anomaly:** DC 20+ to understand spatial distortions or energy fields.  
* **Specialties:**   
  * **Astrophysics:** Modeling stellar life cycles, black hole event horizons, neutron stars, and accretion disks.  
    * **Quantum Mechanics:** Calculating particle superposition, quantum entanglement communications, and tunneling.  
    * **Relativistic Mechanics:** Accounting for time dilation, high-velocity mass expansion, and sub-light acceleration.  
    * **Plasma & High-Energy Dynamics:** Regulating magnetic confinement bottles and thermonuclear energy streams.  
    * **Gravitational Field Physics:** Calculating grav-plate fields, tidal forces, and artificial gravity generator grids.  
    * **Ballistics Trajectory:** Calculating complex gravitational-assist projectile arcs across atmospheric wind shears.  
    * **Fluid Dynamics:** Modeling atmospheric drag on hull profiles and turbulent flows through turbines.  
    * **Thermodynamics:** Designing heat-sink radiation fins, thermal dissipation grids, and cooling loops.  
    * **Subatomic Particle Acceleration:** Understanding antimatter creation, particle beam collation, and cyclotron arrays.  
    * **Dimensional Warp Mechanics:** Modeling spatial curvature, Alcubierre bubble metrics, and metric tension.

  ### 

  ### **RELIGION**

* **Ability:** Intellect  
* **Description:** Religion is the skill of understanding and practicing religious beliefs. It reflects a character's ability to understand the beliefs and practices of different religions, and to practice their own religion. Religion is used whenever a character needs to interact with people of different faiths, whether it's to negotiate a peace treaty or to perform a religious ceremony. This can include things like understanding religious texts, performing religious rituals, and interpreting religious symbols.  
* **Mechanics:**  
  * **Identify Symbol/Rite:** DC 10 (Common) to DC 25 (Secret Cult).  
  * **Theological Debate:** Supported by Diplomacy checks.  
* **Specialties:**   
  * **Pantheons & Cosmic Deities:** Recalling the domains, alignments, and mythologies of recognized sector gods.  
    * **Sectarian Cults & Heresies:** Identifying outlawed apocalyptic sects, blood cults, and rogue pseudo-religious factions.  
    * **Liturgical Rites & Rituals:** Conducting, interpreting, or disrupting formal consecrations, ordinations, and purifications.  
    * **Scriptural Hermeneutics:** Quoting chapter and verse from holy texts to win theological arguments or sway monks.  
    * **Funerary Rites & Afterlife Lore:** Knowing the death rites, ossuary designs, and spirit-warding traditions of cultures.  
    * **Sacred Symbolism:** Decoding holy icons, divine glyphs, sacred geometries, and ceremonial vestment colors.  
    * **Ascendant & Precursor Worship:** Understanding sects that deify ancient alien architects or god-machines.  
    * **Prophetic Traditions:** Interpreting ancient religious prophecies, apocalyptic calendars, and omens.  
    * **Ecclesiastical Law:** Navigating church property rights, clerical trials, and sanctuary protections.  
    * **Exorcism & Cleansing:** Identifying canonical rites to purge metaphysical corruption or unclean entities from holy ground.

  ### 

  ### **SCIENCE**

* **Ability:** Intellect  
* **Description:** Science is the skill of understanding and applying the scientific method. It reflects a character's ability to understand the natural world and use that knowledge to solve problems. Science is used whenever a character needs to understand the behavior of natural phenomena, whether it's to design an experiment or to solve a mystery. This can include things like designing experiments, analyzing data, and interpreting scientific theories.   
* **Mechanics:**  
  * **Lab Work:** DC 15 to perform experiments.  
  * **Identify Substance:** DC 15 to DC 25\.  
* **Specialties:**   
  * **Inorganic Chemistry:** Synthesizing acids, industrial catalysts, synthetic polymers, and inert shielding reagents.  
    * **Biochemistry:** Analyzing cellular metabolism, amino acid structures, and enzymatic reaction triggers.  
    * **Genetics & Cloning:** Splicing DNA, sequencing alien genomes, and managing cellular degradation in clones.  
    * **Material Science & Metallurgy:** Testing tensile strength, molecular lattice bonding, and alloy fatigue.  
    * **Nanotechnology:** Programming molecular assemblers, managing gray-goo dampeners, and repairing nano-swarms.  
    * **Laboratory Procedure:** Operating centrifuges, mass spectrometers, clean-room environments, and electron microscopes.  
    * **Atmospheric Chemistry:** Analyzing toxic air balances, terraforming gas mixtures, and greenhouse gas saturation.  
    * **Radiological Science:** Measuring rems, half-lives, radioactive isotope decay, and managing fission shielding.  
    * **Synthetic Biology:** Designing artificial bacteria to consume plastics, scrub hydrocarbons, or excrete fuels.  
    * **Applied Spectroscopy:** Analyzing distant planetary atmospheres and compound solutions using optical light bands.

  ### 

  ### **SURVIVAL**

* **Ability:** Wisdom  
* **Description:** Survival is the skill of surviving in extreme environments. It reflects a character's ability to find food, water, and shelter in the wilderness, and to avoid danger. Survival is used whenever a character needs to survive in the wilderness, whether it's to explore a new area or to escape from danger. This can include things like finding food and water, building shelter, and avoiding predators.   
* **Mechanics:**  
  * **Track:** DC varies by surface (Soft ground DC 10, Hard floor DC 20+).  
  * **Survival:** DC 15 to provide food/water for self; \+2 DC per additional person.  
  * **Endure Elements:** Grants bonuses to Fortitude saves against weather.  
* **Specialties:**   
  * **Foraging:** Sourcing edible roots, insects, fruits, and clean sustenance in untamed wilderness.  
    * **Water Procurement:** Locating hidden springs, building solar condensation stills, and testing water toxicity.  
    * **Extreme Cold (Arctic):** Building snow shelters, preventing frostbite, and enduring sub-zero blizzards.  
    * **Arid Wasteland (Desert):** Conserving bodily moisture, traveling by night, and finding shaded oases.  
    * **Jungle & Marshland:** Evading quicksand, combating rot, avoiding parasitic leeches, and crossing bogs.  
    * **Emergency Shelter Construction:** Rigging lean-tos, debris huts, windbreaks, and insulated thermal bivouacs.  
    * **Zero-G & Vacuum Survival:** Patching suit punctures, surviving depressurization, and conserving suit oxygen.  
    * **Subterranean Survival:** Navigating lightless deep caves, avoiding bad air pockets, and scaling internal chasms.  
    * **Fire-Building:** Starting cooking and survival fires using flint, friction, or battery sparks in rain and wind.  
    * **Wilderness Traps & Snares:** Constructing deadfalls, snare wires, and pit traps to capture wild game for food.

  ### 

  ### **TACTICS**

* **Ability:** Intellect  
* **Description:** Knowledge of strategy, warfare, and small unit operations. It allows a character to predict enemy movements and plan effective ambushes.  
* **Mechanics:**  
  * **Predict Strategy:** Opposed check vs. Enemy Tactics/Bluff.  
  * **Combat Bonus:** A successful check (DC 15+) prior to combat might grant the team a \+1 Initiative or Attack bonus for the first round (GM discretion).  
* **Specialties:**   
  * **Close-Quarters Battle (CQB):** Clearing rooms, stacking doors, leapfrogging down corridors, and fatal funnel control.  
    * **Squad Fireteam Maneuvers:** Coordinating bounding overwatch, flanking suppression, and tactical withdrawals.  
    * **Guerrilla & Asymmetric Warfare:** Setting up hit-and-run ambushes, sabotage supply raids, and partisan insurgencies.  
    * **Ship-to-Ship Fleet Tactics:** Coordinating broadsides, torpedo spreads, screen escorts, and crossfire brackets.  
    * **Siegecraft & Fortifications:** Establishing defensive kill zones, interlocking fire fields, and breaching fortified walls.  
    * **Boarding Actions:** Breaching airlocks, fighting in tight starship corridors, and securing bridge/engineering hubs.  
    * **Ambush Planning:** Selecting choke points, camouflage perimeters, and timed kill boxes along transit routes.  
    * **Sniper/Spotter Protocols:** Calculating windage, ballistic drop, concealment blinds, and designated marksman security.  
    * **Electronic Warfare Coordination:** Synchronizing jamming bursts with kinetic assaults to deafen enemy comms.  
    * **Counter-Battery Warfare:** Calculating enemy indirect fire origins from impact craters to direct counter-strikes.

  ### 

  ### **TECHNOLOGY**

* **Ability:** Intellect  
* **Description:** Technology is the skill of understanding and using advanced technology. It reflects a character's ability to understand the underlying principles of technology and general concept of its usage.   
* **Mechanics:**  
  * **Repair:** DC 15 (Simple fix) to DC 30 (Rebuild from scrap).  
  * **Disable Device:** DC depends on device complexity/security rating.  
  * **Identify Tech:** DC 10 \+ Tech Level difference.  
* **Specialties:**   
  * **Archaic Technology:** Restoring and fabricating muscle-powered, spring, and clockwork mechanical systems.  
    * **Advanced Directed Energy Systems:** Calibrating laser lenses, tuning particle coils, and replacing cooling magnets.  
    * **Xenotechnology Diagnostics:** Assessing and coaxing function out of inscrutable, non-human technical artifacts.  
    * **Robotics & Automata:** Wiring logic boards, replacing servo actuators, and troubleshooting utility droids.  
    * **Weapons Technology:** Repairing weapon feed systems, accurizing barrels, and modifying firing pins.  
    * **Starship Subsystems:** Diagnosing life support scrubbers, grav-plating, and airlock pressure valves.  
    * **Sensor & Scanner Systems:** Calibrating ladar arrays, tuning thermal scanners, and repairing multi-spectrum probes.  
    * **Power Generators & Fuel Cells:** Servicing hydrogen fuel cells, compact fusion reactors, and battery banks.  
    * **Security Hardware:** Disabling magnetic mag-locks, biometric readers, and laser tripwire systems mechanically.  
    * **Emergency Field-Stripping:** Disassembling, cleaning, and unjamming complex machinery while pinned under fire.

### **TRADE**

* **Ability:** Intellect  
* **Description:** The skill of understanding and navigating the complexities of commercial exchange. It reflects a character"s ability to identify market needs, navigate trade routes, understand commercial laws, and manage mercantile relationships. Trade is used whenever a character needs to buy, sell, or transport goods. This can include things like negotiating contracts, assessing commodity values, and understanding trade regulations.  
* **Mechanics:**  
  * *Analyze Market:* DC 15 to identify trade opportunities, demand, or supply shortages.  
  * *Trade Law:* DC 20 to understand tariffs, trade agreements, or restricted goods in a sector.  
* **Specialties:**   
  * **Commodity Arbitrage:** Buying low in resource-rich frontier sectors and selling high in industrialized core worlds.  
    * **Tariff & Customs Evasion:** Structuring cargo routing to legally minimize planetary import taxes and duties.  
    * **Black Market Smuggling Networks:** Identifying covert delivery corridors, illicit fences, and bribe payoffs.  
    * **Bulk Wholesale Brokering:** Negotiating multi-kiloton contracts for raw durasteel, food grains, and reactor fuel.  
    * **Planetary Export Licensing:** Securing legal monopolies, trade charters, and merchant fleet landing passes.  
    * **Supply Shortage Exploitation:** Reading war and famine indicators to position medical and ammo cargo ahead of demand.  
    * **Currency & Credit Speculation:** Trading local planetary scrip against standard credits to capture exchange spreads.  
    * **Merchant Marine Regulations:** Navigating international interstellar laws regarding distress signals and salvaging rights.  
    * **Consignment Sales:** Negotiating display, sale percentages, and escrow protection for third-party luxury cargo.  
    * **Counter-Embargo Smuggling:** Slipping essential food, fuels, and machine components past military naval blockades.

## 

## 

## **VOCATION (Category)**

* **Ability:** Varies (Usually Int, Wis, or Agi depending on the job)  
* **Description:** Professional skills used to earn a living or perform complex crafting/technical tasks.  
* **Mechanics:** Checks determine the quality of work created or the success of a professional task.  
  * *Earn Income:* Check result determines weekly wealth generation.  
  * *Craft/Repair:* DC determines complexity of the item.  
* **Skill List & Specialties (Examples):**

### 

  ### **ADMINISTRATOR**

* **Ability:** Intellect or Charisma  
* **Description:** Administrator is the skill of managing and organizing people and resources. It reflects a character's ability to lead and coordinate groups of people, and to manage budgets and schedules. Administrator is used whenever a character needs to manage a team, whether it's to complete a project or to run a business. This can include things like delegating tasks, setting goals, and managing finances.   
* **Mechanics:**  
  * *Bureaucracy:* DC 15 to locate specific files or get a permit approved quickly.  
  * *Logistics:* DC 20 to organize a complex supply chain or fleet movement.  
* **Specialties:**   
  * **Civil Bureaucracy:** Fast-tracking applications, navigating red tape, and processing government licenses.  
    * **Colony Management:** Allocating water, housing, power, and labor to maintain frontier outpost stability.  
    * **Corporate Personnel Management:** Organizing department heads, setting labor quotas, and resolving union strikes.  
    * **Budgetary Auditing:** Slashing redundant expenditures and balancing municipal or corporate operational budgets.  
    * **Military Staff Coordination:** Managing quartermaster ledgers, assignment orders, and barracks protocols.  
    * **Public Regulatory Compliance:** Passing environmental inspections, building codes, and workplace safety checks.  
    * **Crisis Management Protocols:** Mobilizing emergency services, setting curfews, and organizing disaster zoning.  
    * **Records Governance:** Systematically filing, sealing, and purging organizational archives and personnel records.  
    * **Contract Administration:** Ensuring contractors, suppliers, and third-party vendors meet project milestones.  
    * **Inter-Department Mediation:** Resolving political gridlock and resource tug-of-wars between rival ministry heads.

  ### 

  ### **ALCHEMIST**

* **Ability:** Intellect  
* **Description:** Alchemist is the skill of understanding and creating potions and other magical substances. It reflects a character's ability to mix and manipulate chemicals and magical ingredients. Alchemist is used whenever a character needs to create a potion or other magical substance, whether it's to heal an injury or to create a powerful spell. This can include things like mixing ingredients, analyzing substances, and creating magical effects.   
* **Mechanics:**  
  * *Identify Substance:* DC 15 to identify a chemical or potion.  
  * *Craft Compound:* DC varies (Acid DC 15, Panacea DC 25). Failure by 5 or more may cause an explosion or exposure.  
* **Specialties:**   
  * **Combat Stimulants:** Synthesizing adrenal boosters, coagulants, and reflex-accelerating injections.  
    * **Panaceas & Antitoxins:** Compounding broad-spectrum antivenoms and toxin-neutralizing serums.  
    * **Corrosive Acids & Caustics:** Formulating industrial acids capable of melting hull plating and security deadbolts.  
    * **Incendiary Gels:** Brewing sticky, pyrophoric chemical pastes that ignite on contact with air.  
    * **Metaphysical Potions:** Distilling liquid mana, psionic stability elixirs, and reality-anchoring tonics.  
    * **Paralytics & Sleeping Agents:** Extracting and refining fast-acting knockout vapors and tranquilizing neurotoxins.  
    * **Biological Preservatives:** Mixing preservation fluids to prevent organ decay and specimen degradation.  
    * **Smoke & Gas Compounds:** Creating thick visual-obscuring smoke, tear gas, and suffocating choking dust.  
    * **Reagent Purification:** Refining raw minerals, alien glands, and planetary botanicals into pure alchemical bases.  
    * **Volatile Reactions Handling:** Stabilizing explosive compounds during laboratory synthesis to prevent accidental detonations.

  ### 

  ### **AMBASSADOR**

* **Ability:** Charisma  
* **Description:** Ambassador is the skill of negotiating and representing one's country or organization. It reflects a character's ability to communicate effectively with people from different cultures and backgrounds, negotiate treaties, and represent one's country or organization in a foreign land. Ambassador is used whenever a character needs to promote their country's interests and maintain good relations with other countries. This can include things like negotiating trade agreements, resolving disputes, and building alliances.   
* **Mechanics:**  
  * *Protocol:* DC 15 to know the correct etiquette for a specific alien court.  
  * *Negotiate Treaty:* Opposed check vs. rival Ambassador.  
* **Specialties:**   
  * **First Contact Protocols:** Establishing non-hostile communications with uncontacted or newly awakened species.  
    * **Bilateral Treaty Negotiation:** Drafting peace accords, borders, and non-aggression treaties between factions.  
    * **Extradition & Prisoner Exchanges:** Negotiating the repatriation of captured spies, soldiers, and political dissidents.  
    * **Trade Coalition Representation:** Advocating for planetary interests in interstellar trade summits and unions.  
    * **Hostage & Ceasefire Mediation:** Brokering emergency stand-downs between warring armies under active crossfire.  
    * **Diplomatic Immunity Defense:** Invoking extraterritoriality and sovereign status to escape local legal harassment.  
    * **Cultural Liaison:** Educating state delegations on delicate social norms to prevent accidental declarations of war.  
    * **Imperial Court Protocol:** Presenting credentials and conducting official state audiences before emperors and kings.  
    * **Sanction & Embargo Negotiation:** Convincing neutral factions to cut off trade, fuel, and arms to enemy systems.  
    * **Alliance Building:** Assembling multi-system coalitions through shared security and mutual defense pledges.

  ### 

  ### **ARCHITECT**

* **Ability:** Intellect  
* **Description:** Architect is the skill of designing and building structures. It reflects a character's ability to create and design buildings and other structures, design blueprints, create models, and oversee construction. Architect is used whenever a character needs to create a structure, whether it's a house, a castle, or a city. This can include things like designing layouts, creating blueprints, and managing construction crews.   
* **Mechanics:**  
  * *Design:* Create blueprints for a structure (DC 15 for a house, DC 30 for a space elevator).  
  * *Assess Weakness:* DC 20 to find a structural weak point in a building (grants bonus to Demolitionist).  
* **Specialties:**   
  * **Arcology Design:** Engineering self-contained mega-structures housing millions with integrated air and waste loops.  
    * **Starship Naval Architecture:** Designing structural bulkheads, internal frame load paths, and compartmental layouts.  
    * **Bunkers & Fortifications:** Calculating reinforced blast wall thickness, subterranean depths, and bunker hardening.  
    * **Orbital Station Design:** Designing modular docking rings, centrifuge spin hubs, and microgravity stress joints.  
    * **Hostile-Environment Habitats:** Designing dome colonies resilient to magma, crushing oceanic depths, or toxic air.  
    * **Acoustic & Sightline Layouts:** Structuring amphitheaters, auditoriums, and security plazas for optimal line-of-sight.  
    * **Structural Weakness Assessment:** Identifying load-bearing pillars, shear points, and design flaws in enemy installations.  
    * **Modular Prefab Structures:** Creating rapidly deployable, flat-pack buildings for frontier forward bases.  
    * **Urban Transit Design:** Integrating subways, skybridges, and vehicle traffic arteries into city blueprints.  
    * **Historic Restoration Architecture:** Reconstructing ancient ruins and historic structures using original techniques.

  ### 

  ### **ARCHIVIST**

* **Ability:** Intellect  
* **Description:** Archivist is the skill of preserving and organizing historical documents and artifacts. It reflects a character's ability to collect and preserve historical records and artifacts, catalog artifacts, preserve documents, and research historical events. Archivist is used whenever a character needs to preserve and make accessible historical records for future generations. This can include things like cataloging artifacts, preserving documents, and researching historical events.   
* **Mechanics:**  
  * *Deep Search:* DC 20 to find a specific, non-digitized record or deleted file fragment.  
  * *Preservation:* DC 15 to restore damaged books or data drives.  
* **Specialties:**   
  * **Ancient Tome Preservation:** Restoring, sealing, and handling fragile parchment, papyrus, and crystalline plates.  
    * **Data Recovery & File Reconstruction:** Extracting readable data from melted drives, corrupted memory, and shattered chips.  
    * **Genealogical Indexing:** Tracing sprawling historical lineage charts and imperial ancestry rolls across centuries.  
    * **Classified Document Redaction:** Scrubbing, sealing, and securely compartmentalizing state and corporate secrets.  
    * **Library Taxonomy & Systems:** Designing efficient searchable classification systems for planetary-scale data hubs.  
    * **Holographic & Optical Media Care:** Repairing and reading degraded microfilms, magnetic reels, and optical cubes.  
    * **Museum Conservation:** Stabilizing archaeological artifacts against moisture, oxidation, and atmospheric decay.  
    * **Ancient Script Transliteration:** Converting forgotten scripts into modern readable linguistic formats without data loss.  
    * **Historical Provenance Authentication:** Verifying the unbroken chain of custody and authenticity of old historical records.  
    * **Deep Index Searching:** Rapidly locating specific micro-records within massive, million-year-old civil repositories.

  ### 

  ### **ARMORER**

* **Ability:** Intellect (for design) or Agility (for fine detail)  
* **Description:** Armorer is the skill of creating and repairing different types of armor. It reflects a character's ability to forge metal, repair damaged armor, and create new designs. Armorer is used whenever a character needs to ensure that armor is functional and effective. This can include things like repairing damaged armor to creating a new suit.   
* **Mechanics:**  
  * *Repair Armor:* Restore armor points (DC 15 \+ Tech Level).  
  * *Craft Armor:* Create new armor (Time required varies).  
  * *Fit Armor:* DC 15 to resize armor for a different species.  
* **Specialties:**   
  * **Archaic Plate & Mail:** Forging, quenching, and strapping tempered steel and bronze personal armor.  
    * **Ballistic Weaves & Flak Suits:** Stitching layered kevlar, carbon-nanotube fabrics, and impact-dispersion vests.  
    * **Composite Ceramic Plating:** Baking and inserting high-density boron-carbide strike plates into tactical rigs.  
    * **Powered Armor Exoskeletons:** Servicing hydraulic actuators, frame braces, and artificial myomer musculature.  
    * **Energy Shield Harnesses:** Tuning personal deflector coils, field harmonizers, and capacitor discharge belts.  
    * **Hazardous Environment Seals:** Fitting airtight collars, hermetic seals, and thermal linings on void suits.  
    * **Xenotech Shell Integration:** Splicing chitinous alien hides and bio-shells into functional composite armor.  
    * **Stealth Coating Application:** Applying radar-absorbent paint, thermal-masking fabrics, and matte anti-glare dyes.  
    * **Custom Armor Tailoring:** Resizing captured or off-the-shelf armor to perfectly fit unique alien physiologies.  
    * **Rapid Battle-Damage Patching:** Hammering out dents, welding cracked plate, and cold-patching suit punctures.

  ### 

  ### **ARTIST**

* **Ability:** Charisma (Expression) or Wisdom (Insight) or Agility (Fine Motor)  
* **Description:** Artist is the skill of creating art in various forms, such as painting, sculpture, or music. It reflects a character's ability to paint, sculpt, or compose music. Artist is used whenever a character needs to create works of art that are aesthetically pleasing and meaningful. This can include things like painting portraits, sculpting statues, or composing music.   
* **Mechanics:**  
  * *Create Work:* Check result determines the value and quality of the piece.  
  * *Forgery:* Opposed by Appraisal.  
* **Specialties:**   
  * **Canvas & Mural Painting:** Mastering pigments, oil glazes, brushwork, and monumental wall fresco execution.  
    * **Digital & Concept Illustration:** Speed-painting character concepts, environments, and schematics via digital pads.  
    * **Hard-Light & Holographic Art:** Sculpting interactive light displays, laser murals, and volumetric art.  
    * **Stone & Metal Sculpting:** Chiseling marble, casting bronze, and welding scrap steel into expressive sculptural forms.  
    * **Tattooing & Bio-Art:** Inking intricate skin tattoos, dermal scarring, and implanting aesthetic bioluminescent dyes.  
    * **Prop & Stage Design:** Crafting visual sets, theatrical props, and physical backdrops for entertainment productions.  
    * **Artistic Forgery:** Replicating the brushstrokes, aged cracking, and pigment blends of master artists.  
    * **Graphic Iconography & Logos:** Designing striking corporate branding, faction banners, and military unit crests.  
    * **Calligraphy & Illumination:** Hand-inking illuminated manuscripts, ornate royal proclamations, and gilded letters.  
    * **Ceramics & Glasswork:** Throwing functional pottery, firing porcelain, and blowing blown-glass decorative forms.

  ### 

  ### **ARTIFICER**

* **Ability:** Intellect or Wisdom  
* **Description:** Artificer is the skill of creating metaphysical empowered items, such as devices, potions, or enchanted weapons. It reflects a character's ability to mix special ingredients, empower weapons, and create magical effects. Artificer is used whenever a character needs to create magical or psionic items that are functional and effective. This can include things like mixing magical ingredients, enchanting weapons, and creating magical effects. The specific craft of combining technology with metaphysical (magical/psionic) energies. This is the skill used to create "enchanted" or "psi-active" items.  
* **Mechanics:**  
  * *Imprint Item:* Bind a spell or psionic effect to an object (DC 20 \+ Spell Level).  
  * *Analyze Artifact:* Identify the properties of a magitech item.  
* **Specialties:**   
  * **Focusing Crystals:** Cutting, polishing, and aligning quartz and alien crystals to channel raw meta-currents.  
    * **Psionic Amplifiers:** Building cranial headbands and helm arrays that boost telepathic and telekinetic range.  
    * **Metaphysical Weapon Imbuement:** Inscribing runic channels onto blades and barrels to conduct elemental magic.  
    * **Magitech Circuitry:** Etching silver and cold-iron traces into printed circuit boards to process arcane power.  
    * **Golem & Construct Forging:** Animating mechanized automata using bound elemental spirits or artificial souls.  
    * **Talisman & Amulet Crafting:** Creating defensive jewelry that stores single-charge warding or protective spells.  
    * **Dimensional Containers:** Crafting bags, crates, and pockets that utilize folded space to hold massive volume.  
    * **Wands & Focusing Rods:** Turning dense woods, conductive alloys, and monster bones into directional casting implements.  
    * **Etheric Batteries:** Fabricating capacitors that store ambient mana, void discharge, or psychic residue for later use.  
    * **Artifact Diagnostics:** Examining ancient, unknown magitech machinery to deduce activation keys and hazards.

  ### 

  ### **BROKER**

* **Ability:** Wisdom or Charisma  
* **Description:** Broker is the skill of negotiating and managing financial transactions. It reflects a character's ability to negotiate contracts, manage investments, and analyze financial data. Broker is used whenever a character needs to ensure that financial transactions are profitable and beneficial for their clients. This can include things like negotiating deals, managing investments, and analyzing market data.  
* **Mechanics:**  
  * *Appraise Market:* DC 15 to know if a commodity is selling above or below average value in this system.  
  * *Secure Deal:* DC 20 to find a buyer for illegal or rare goods.  
* **Specialties:**   
  * **Black Market Contraband:** Connecting sellers of illicit narcotics, military tech, and unvetted chips with buyers.  
    * **Securities & Equities:** Brokering large blocks of corporate stock, municipal bonds, and venture equities.  
    * **Starship Sales & Transfers:** Arranging ownership transfers, escrows, and surveys for used civilian and naval craft.  
    * **Mercenary Contracts:** Matching corporate and planetary clients with private military outfits and bounty hunters.  
    * **Real Estate & Land Rights:** Brokering purchases of commercial arcology blocks, private moons, and farming lands.  
    * **Military Arms Brokering:** Negotiating bulk surplus sales of firearms, munitions, and combat armor to third parties.  
    * **Mining Claims & Concessions:** Trading mineral survey data and staking rights for asteroid belts and virgin planets.  
    * **Information & Secrets Trading:** Brokering blackmail, corporate trade secrets, and military troop manifests for cash.  
    * **Intellectual Property Licensing:** Trading patents, pharmaceutical formulas, and software source codes.  
    * **High-Value Luxury Commodities:** Finding private collectors for vintage wines, rare furs, and exotic alien pets.

  ### 

  ### **CELEBRITY**

* **Ability:** Charisma  
* **Description:** Celebrity is the skill of managing one's public image and interacting with fans and the media. It reflects a character's ability to manage social interactions, give interviews, and attend public events. Celebrity is used whenever a character needs to maintain a positive public image and promote their brand.  
* **Mechanics:**  
  * *Work the Crowd:* DC 15 to gain access to a VIP area based on recognition.  
  * *Spin:* DC 20 to mitigate a scandal or bad reputation event.  
* **Specialties:**   
  * **Holonet Streaming & Broadcast:** Presenting engaging video streams, entertaining live feeds, and hosting shows.  
    * **Public Relations Spin:** Turning scandalous mistakes, arrests, or failures into sympathetic PR victories.  
    * **Brand Endorsements:** Leveraging star power to sell products, gear lines, and lifestyle concepts for credits.  
    * **Red Carpet & Gala Presence:** Navigating high-profile parties, dominating camera angles, and dazzling crowds.  
    * **Fanbase Mobilization:** Directing millions of loyal fans to boycott corporations, vote, or crowdfund projects.  
    * **Paparazzi Evasion:** Slipping away from aggressive media cameras, hover-drones, and tabloid trackers.  
    * **Persona Fabrication:** Developing and maintaining a consistent, highly marketed public persona or alter ego.  
    * **Talk Show Charm:** Dominating live interview formats, deflecting awkward queries, and landing memorable soundbites.  
    * **Charity Gala Organizing:** Hosting multi-million-credit charity events to scrub reputations and curry public goodwill.  
    * **VIP Access & Entourage Control:** Using celebrity status to walk past security bouncers and enter exclusive VIP spaces.

  ### 

  ### **CONSTABLE**

* **Ability:** Wisdom or Intellect  
* **Description:** Constable is the skill of enforcing the law and maintaining order. It reflects a character's ability to investigate crimes, make arrests, and protect citizens. Constable is used whenever a character needs to ensure that laws are upheld and that citizens are safe. This can include things like investigating crimes, making arrests, and protecting citizens.   
* **Mechanics:**  
  * *Procedure:* DC 15 to know the jurisdiction and rights in a specific sector.  
  * *Collect Evidence:* DC 15 to properly bag and tag evidence without contaminating it.  
* **Specialties:**   
  * **Patrol Procedures:** Walking beats, spotting suspicious street activity, and projecting an authoritative presence.  
    * **Riot & Crowd Control:** Deploying shield walls, tear gas lines, and batons to break up violent civil mobs.  
    * **Suspect Apprehension:** Tacking down, tackling, and applying restraint cuffs to non-compliant suspects safely.  
    * **Evidence Chain of Custody:** Documenting, bagging, and securing legal evidence to ensure courtroom admissibility.  
    * **Tactical Breaching:** Stacking outside narcotics dens, blowing hinge bolts, and executing high-threat entries.  
    * **Interrogation & Field Questioning:** Interviewing street witnesses and suspects to glean immediate tactical leads.  
    * **Jurisdictional Sector Law:** Knowing legal boundaries, warrant requirements, and search rights in a given sector.  
    * **Traffic & Checkpoint Control:** Setting up roadblock barricades, running license scans, and spotting drug mules.  
    * **Hostage Crisis First-Response:** Containing active shooter situations and securing perimeters until negotiators arrive.  
    * **Undercover Vice Operations:** Infiltrating low-level illegal gambling dens, chop shops, and contraband rings.

  ### 

  ### **COURTESAN**

* **Ability:** Charisma  
* **Description:** Courtesan is the skill of entertaining and pleasing others. It reflects a character's ability to dance, sing, or be intimate in a professional manner. Courtesan is used whenever a character needs to provide entertainment and pleasure to their clients. Professional companionship, high-society socialization, and the art of conversation and entertainment. Often overlaps with spying.  
* **Mechanics:**  
  * *Seduction/Charm:* Opposed by Wisdom (Insight).  
  * *Gather Rumors:* DC 15 to pick up sensitive information during social interactions.  
* **Specialties:**   
  * **High-Society Etiquette:** Effortlessly fitting into noble galas, elite banquets, and decadent private revues.  
    * **Pillow Talk & Eavesdropping:** Coaxing military secrets, executive passcodes, and hidden plots from wealthy patrons.  
    * **Art of Witty Conversation:** Entertaining bored aristocrats and corporate moguls with captivating verbal banter.  
    * **Sensual Seduction:** Reading physical attraction cues to make individuals feel uniquely desired and disarmed.  
    * **Discreet Companionship:** Maintaining total professional confidentiality, inspiring profound emotional trust.  
    * **Flattery & Ego Stroking:** Inflating fragile egos to lower defenses, win favors, and secure extravagant gifts.  
    * **Poise & Posture Mastery:** Projecting absolute grace, physical beauty, and captivating movement across any room.  
    * **Social Shielding:** Gracefully diverting abrasive guests away from vulnerable patrons during awkward social scenes.  
    * **Subtle Intoxication Management:** Feigning drunkenness while keeping a razor-sharp, sober mind during late-night revels.  
    * **Patron Psychology:** Evaluating client emotional vulnerabilities to serve as confidant, therapist, and advisor.

  ### 

  ### **CULINARIAN**

* **Ability:** Wisdom or Agility  
* **Description:** Culinarian is the skill of cooking and preparing food. It reflects a character's ability to prepare ingredients, cook meals, and present food. Culinarian is used whenever a character needs to create delicious and nutritious meals. This can include things like preparing ingredients, cooking meals, and presenting food.   
* **Mechanics:**  
  * *Prepare Meal:* DC 10 (Standard) to DC 30 (Banquet). High success grants temporary morale bonuses to those who eat.  
  * *Identify Ingredients:* DC 15 to taste a poison or strange alien spice.  
* **Specialties:**   
  * **Xenofood Neutralization:** Processing, cooking, and detoxifying strange alien flora and fauna for human consumption.  
    * **Haute Cuisine & Fine Dining:** Preparing multi-course, visually stunning gourmet meals for high nobility and executives.  
    * **Field Ration Optimization:** Transforming bland nutrient paste and dried field rations into palatable, morale-boosting meals.  
    * **Baking & Pastry Arts:** Crafting delicate breads, pastries, sugar art, and complex doughs with precise chemistry.  
    * **Brewing, Fermenting & Distilling:** Crafting fine wines, ales, spirits, and fermented probiotics from local crops.  
    * **Food Preservation & Curing:** Smoking, salting, pickling, and freeze-drying meats and produce for deep-space voyages.  
    * **Butchery & Dressing:** Carving wild carcasses, jointing meat cleanly, and extracting edible cuts without waste.  
    * **Flavor Balancing & Spicing:** Blending rare off-world herbs, spices, and seasonings to elevate common dishes.  
    * **Toxin & Poison Tasting:** Identifying trace amounts of cyanide, arsenic, or spoiled proteins in served dishes by scent/taste.  
    * **Banquet & Mass Catering:** Managing industrial kitchens to feed thousands of starship crewmen or soldiers efficiently.

  ### 

  ### **DEMOLITIONIST**

* **Ability:** Intellect  
* **Description:** Demolitionist is the skill of creating and using explosives. It reflects a character's ability to create bombs, set charges, and detonate explosives for various purposes. Demolitionist is used whenever a character needs to ensure that explosives are used safely and effectively. This can include things like creating bombs, disarming bombs, setting charges, and detonating explosives for various purposes.   
* **Mechanics:**  
  * *Set Charge:* DC 15 for simple breach, DC 25 to collapse a structure without damaging surroundings.  
  * *Defuse:* DC depends on the bomb's complexity (usually DC 20+). Failure by 5+ triggers the device.  
* **Specialties:**   
  * **Structural Controlled Implosion:** Calculating load points to drop high-rise buildings and towers within their footprints.  
    * **Combat Breaching Charges:** Setting linear shaped charges to blow open reinforced steel doors without killing the stack.  
    * **EOD Bomb Defusal:** Tracing circuitry, bypassing anti-tamper switches, and disarming unexploded bombs under pressure.  
    * **Commercial Mining Blasting:** Drilling and packing dynamite or plas-charges into rock faces to maximize ore fracture.  
    * **Shaped-Charge Armor Penetration:** Rigging concentrated directional blasts to punch clean holes through bunker armor.  
    * **Improvised Explosive Devices (IEDs):** Fashioning functional mines and bombs from fertilizer, cleaning agents, and scrap.  
    * **Remote & Timed Detonators:** Wiring radio triggers, laser tripwires, timers, and pressure plates with fail-safes.  
    * **Underwater & Vacuum Demolitions:** Accounting for hydro-shockwaves and explosive dissipation in the cold vacuum of space.  
    * **Blast Mitigation & Containment:** Sandbagging, venting, and rigging blast curtains to direct explosions away from allies.  
    * **Munitions Disposal:** Safely burning off degraded explosives, unstable gel-packs, and leaking chemical shells.

  ### 

  ### **ELECTRICIAN**

* **Ability:** Intellect or Agility  
* **Description:** Electrician is the skill of understanding and working with electrical systems. It reflects a character's ability to install and repair electrical systems, such as wiring, circuits, outlets, switches, and lighting fixtures. Electrician is used whenever a character needs to ensure that electrical systems are safe and functional. This can include things like installing and repairing electrical systems, such as wiring, circuits, outlets, switches, and lighting fixtures. Wiring, circuitry, and power systems. Distinct from Computers (software) and Mechanic (moving parts).  
* **Mechanics:**  
  * *Bypass Security:* DC 20 to hotwire an electronic lock.  
  * *Repair Circuit:* DC 15 to fix a fried panel.  
  * *Restore Power:* DC 20 to reroute power in a damaged ship.  
* **Specialties:**   
  * **High-Voltage Distribution:** Laying heavy-duty power lines, substations, and industrial 10,000-volt transformers.  
    * **Micro-Electronics & PCBs:** Soldering surface-mount resistors, micro-chips, and delicate data bus relays.  
    * **Ship Power Bus Routing:** Balancing primary and secondary power conduits across starship decks during combat.  
    * **Backup Generators & Uninterruptible Power:** Installing and servicing diesel, turbine, and battery-bank backup arrays.  
    * **Circuit Breakers & Fuses:** Diagnosing electrical overloads, short-circuits, and replacing blown primary bus fuses.  
    * **Cyberware Interface Wiring:** Terminating neural-wire harness plugs and biocompatible conductive ports cleanly.  
    * **Lock Relay Bypassing:** Hotwiring electronic door strikes and shorting solenoid lock tumblers with jumper wires.  
    * **EMP Hardening:** Installing Faraday shielding, surge arresters, and shielded conduits to resist electromagnetic shocks.  
    * **Fiber-Optic & Data Cabling:** Splicing glass optical strands and routing interference-free high-speed data trunks.  
    * **Solar & Renewable Arrays:** Wiring photovoltaic solar panels, thermal collectors, and inverters into local grids.

  ### 

  ### **ENGINEER**

* **Ability:** Intellect  
* **Description:** Engineering is the skill of using scientific principles to design and build mecha, automata (simple robotics) and similar structures and mechanisms. It reflects a character's ability to apply mathematical and scientific concepts to create new products or improve existing ones. Engineers work in various industries, including mechanical, electrical, computer and architectural skills. Engineering is using various components together to craft something larger and more complex, whereas the components themselves are covered by particular skills such as Mechanical or Electrical.  
* **Mechanics:**  
  * *Diagnose Issue:* DC 15 to identify why the Hyperdrive isn't engaging.  
  * *Boost System:* DC 20 to temporarily increase engine output or shield strength.  
* **Specialties:**   
  * **Starship FTL Propulsion:** Servicing warp field coils, hyperdrive motivators, and dimensional jump engines.  
    * **Sublight Thruster Assemblies:** Maintaining ion drives, fusion torches, and chemical reaction thrusters.  
    * **Life Support Systems:** Designing and overhauling oxygen scrubbers, hydroponic air loops, and waste recyclers.  
    * **Civil Infrastructure:** Engineering water aqueducts, sewage treatment plants, suspension bridges, and dams.  
    * **Hydraulics & Pneumatics:** Designing high-pressure fluid valves, mechanical pistons, and pneumatic seals.  
    * **Thermal Dissipation Systems:** Engineering reactor cooling loops, cryo-jacket cooling, and radiator fin arrays.  
    * **Industrial Automation & Assembly:** Setting up automated robotic car lines, conveyor feeds, and stamping presses.  
    * **Structural Stress Diagnostics:** Using ultrasound and load sensors to spot metal fatigue and impending frame collapse.  
    * **Power Plant Core Management:** Regulating magnetic containment in fusion reactors and antimatter reaction chambers.  
    * **Heavy Crane & Rigging Mechanics:** Calculating counterweights, crane moments, and structural winching limits for massive loads.

  ### 

### **FARMER**

* **Ability:** Wisdom or Constitution  
* **Description:** Farmer is the skill of cultivating crops and raising livestock. It reflects a character's ability to manage soil health, plant and harvest crops, and tend to animals. Farmer is used whenever a character needs to produce food, manage agricultural operations, or work with livestock. This can include things like preparing fields, planting seeds, and ensuring animal health.  
* **Mechanics:**  
  * *Crop Management:* DC 15 to ensure a successful harvest despite poor conditions.  
  * *Animal Husbandry:* DC 15 to breed or treat livestock.  
  * *Soil Analysis:* DC 20 to determine optimal crops for a specific environment.  
* **Specialties:**   
  * **Hydroponics & Aeroponics:** Managing vertical water-nutrient tanks, pH balances, and LED grow-light cycles.  
  * **Broad-Acre Grain Farming:** Operating harvesters, planting vast cereal fields, and timing seasonal crop rotations.  
  * **Livestock Husbandry:** Breeding, grazing, birthing, and caring for domestic cattle, poultry, and alien beasts.  
  * **Soil Chemistry & Fertilization:** Analyzing nitrogen, phosphorus, and organic loam balances to maximize crop yield.  
  * **Agricultural Pest & Fungus Control:** Spraying organic pesticides, deploying weed controls, and countering alien locusts.  
  * **Irrigation & Water Rights:** Digging drainage canals, operating drip-feed lines, and managing agricultural aquifers.  
  * **Alien Crop Domestication:** Cross-breeding native wild plants to engineer edible, high-calorie domestic strains.  
  * **Greenhouse Environmental Control:** Tuning humidity, atmospheric carbon, and thermal ceilings in sealed domes.  
  * **Veterinary Care:** Treating domestic herd infections, setting broken animal limbs, and inoculating against rot.  
  * **Harvest Preservation & Silos:** Drying, fumigating, and storing tons of grain against mildew, rot, and rodent pests.

### 

  ### **GROUNDSKEEPER**

* **Ability:** Wisdom or Constitution  
* **Description:** Groundskeeper is the skill of managing and maintaining outdoor spaces, such as parks or gardens. It reflects a character's ability to mow lawns, trim hedges, plant flowers, and maintain other outdoor features. Groundskeepers are responsible for ensuring that outdoor spaces are safe, functional, and aesthetically pleasing. This can include things like mowing lawns, trimming hedges, and planting flowers.  
* **Mechanics:**  
  * *Cultivate:* DC 15 to grow specific plants in a hostile environment.  
  * *Terraform:* DC 25 to operate large scale atmosphere processors.  
* **Specialties:**   
  * **Landscape Gardening:** Designing flower beds, trimming hedges, laying sod, and styling ornamental gardens.  
    * **Arboriculture & Tree Surgery:** Felling dangerous deadwood, pruning high branches, and treating tree infections.  
    * **Estate Hardscaping:** Laying stone pathways, building retaining walls, patio pavings, and ornamental fountains.  
    * **Turf & Heavy Lawn Machinery:** Operating commercial ride-on mowers, aerators, dethatchers, and turf rollers.  
    * **Invasive Flora Eradication:** Identifying and systematically poisoning or uprooting choking alien weeds and vines.  
    * **Drainage & Erosion Control:** Installing French drains, grading soil swales, and planting grass to prevent mudslides.  
    * **Greenhouse Maintenance:** Managing glass hygiene, misting systems, and soil heating cables for rare shrubs.  
    * **Park & Plaza Sanitation:** Clearing public leaves, emptying waste receptacles, and pressure-washing stone avenues.  
    * **Soil & Mulch Composting:** Managing thermal compost piles, shredding yard waste, and top-dressing garden beds.  
    * **Planetary Terraforming Planting:** Seeding hardy lichen and scrub bushes across barren rocky ground to build topsoil.

  ### 

  ### **HANDLER**

* **Ability:** Charisma or Wisdom  
* **Description:** Training and controlling animals or beasts.  
* **Mechanics:**  
  * *Train:* Long-term check to teach a creature a trick (DC 15-25).  
  * *Control:* DC 15 to calm a spooked animal; DC 20 to command an attacking beast.  
* **Specialties:**   
  * **Riding Mount Training:** Breaking, saddling, and training equines, alien lizards, and quadrupeds for riding.  
    * **Combat Attack Training:** Teaching canines and alien predators to bite on command, hold targets, and ignore gunfire.  
    * **Beast Calming & De-Escalation:** Approaching spooked, aggressive, or wounded wild beasts without triggering an attack.  
    * **Scent & Tracking Hounds:** Directing tracking packs to follow faint prey scent trails over difficult wilderness.  
    * **Working Draft Animals:** Harnessing oxen and pack beasts to pull heavy wagons, plows, and artillery pieces.  
    * **Exotic Alien Pet Taming:** Conditioning unpredictable wild fauna to live peacefully in domestic environments.  
    * **Avian & Flying Mount Handling:** Training hawks, flying reptiles, or gliders for scouting, hunting, and aerial message delivery.  
    * **Livestock Herding:** Maneuvering cattle and sheep herds across open ranges using whistles, mounts, and whips.  
    * **Behavioral Obedience:** Teaching complex domestic commands, tricks, and retrieval habits to working creatures.  
    * **Zoological Caging & Transport:** Safely securing, crating, tranquilizing, and moving massive apex predators.

  ### **LABORER**

* **Ability:** Strength or Constitution  
* **Description:** Laborer is the skill of performing manual tasks, such as digging ditches, carrying heavy objects, or building structures. It reflects a character's ability to perform physical labor and complete tasks that require strength and endurance. Workers are responsible for completing manual tasks safely and efficiently. This can include things like digging ditches, carrying heavy objects, and building structures. Laborers will be knowledgeable of various tools and equipment of the trade.  
* **Mechanics:**  
  * *Endurance Work:* DC 15 to work a double shift without fatigue.  
  * *Operate Heavy Machinery:* DC 10 to use forklifts, drills, or exosuits for work (not combat).  
* **Specialties:**   
  * **Dockworking & Stevedoring:** Unloading shipping containers, rigging cranes, and securing cargo nets on docks.  
    * **Deep Shaft Mining:** Wielding jackhammers, swinging pickaxes, mucking ore, and timbering mine shafts.  
    * **Heavy Demolition Work:** Smashing concrete with sledgehammers, cutting rebar, and hauling industrial rubble.  
    * **Concrete & Composite Pouring:** Screeding wet concrete slabs, vibrating forms to clear air pockets, and raking gravel.  
    * **Scaffolding Erection:** Clamping metal poles, laying wooden walk-boards, and anchoring safety harnesses on high walls.  
    * **Manual Trenching & Shoveling:** Digging deep pipeline trenches, earthworks, and defense berms with shovels.  
    * **Power Loader & Exosuit Labor:** Operating industrial hydraulic lifter rigs to move multi-ton shipping crates.  
    * **Hazardous Waste Mucking:** Shoveling toxic slag, cleaning out chemical sludge tanks, and handling contaminated soot.  
    * **High-G/Zero-G Physical Toil:** Shifting heavy mass under crushing high gravity or anchoring oneself while handling bulk in zero-G.  
    * **Industrial Site Rigging:** Inspecting steel wire ropes, slinging chains, and directing forklift drivers via hand signs.

  ### 

  ### **MECHANIC**

* **Ability:** Agility or Intellect  
* **Description:** Mechanic is the skill of repairing and maintaining components of machines and vehicles. It reflects a character's ability to diagnose and fix mechanical problems, and to perform routine maintenance tasks. Mechanics are responsible for ensuring machines and vehicles are safe and functional. This can include things like repairing engines, replacing parts, and performing oil changes.   
* **Mechanics:**  
  * *Repair:* DC 15 (Simple part) to DC 30 (Rebuild engine).  
  * *Jury-Rig:* DC 20 to make a temporary fix with inadequate tools (lasts 1d6 hours).  
* **Specialties:**   
  * **Internal Combustion Engines:** Rebuilding piston engines, tuning carburetors, timing camshafts, and replacing spark plugs.  
    * **Turbines & Jet Propulsion:** Balancing compressor blades, repairing fuel injectors, and rebuilding jet burner cans.  
    * **Drive Train & Transmission:** Replacing clutches, rebuilding planetary gearboxes, and balancing drive shafts.  
    * **Pneumatic & Hydraulic Actuators:** Replacing blown hydraulic seals, flushing dirty brake fluid, and rebuilding rams.  
    * **Suspension & Steering Systems:** Aligning tie-rods, replacing heavy coil springs, and servicing ball joints.  
    * **Chassis & Body Fabrication:** Cutting out rusted sheet metal, bending roll cages, and MIG/TIG welding frames.  
    * **Cooling & Radiator Systems:** Flushing clogged water jackets, replacing water pumps, and soldering leaking cores.  
    * **Jury-Rigged Field Repairs:** Patching split hoses with duct tape, bypassing busted valves, and getting rigs running on scrap.  
    * **Machine Shop Lathe Tooling:** Turning metal axles on manual lathes and milling custom replacement brackets from steel billet.  
    * **Heavy Construction Equipment:** Servicing bulldozers, tracked excavators, and industrial rock crushers.

  ### 

  ### **RESEARCHER**

* **Ability:** Intellect  
* **Description:** Researcher is the skill of conducting research and analyzing data. It reflects a character's ability to gather information, analyze it, and draw conclusions. Researchers are responsible for ensuring that information is accurate and reliable. This can include things like conducting surveys, analyzing data, and writing reports.  
* **Mechanics:**  
  * *Conduct Experiment:* DC 20 to test a hypothesis.  
  * *Synthesize Data:* DC 15 to combine multiple clues into a theory.  
* **Specialties:**   
  * **Experimental Design:** Structuring double-blind trials, control groups, and repeatable lab procedures.  
    * **Empirical Data Modeling:** Plotting statistical regressions, identifying standard deviations, and mapping anomalies.  
    * **Field Specimen Collection:** Sampling alien soils, extracting sterile blood samples, and gathering core geological drills.  
    * **Literature Review & Meta-Analysis:** Reviewing thousands of published academic white papers to summarize states of knowledge.  
    * **Hypothesis Testing:** Designing targeted scientific trials to systematically prove or disprove empirical claims.  
    * **Statistical Error Auditing:** Spotting mathematical flukes, sample biases, and p-hacking in submitted studies.  
    * **AI Model Validation:** Testing machine-learning models against sanitized datasets to prevent algorithmic hallucinations.  
    * **Laboratory Safety Management:** Managing biosafety level 4 containment seals, chemical hoods, and emergency eye-washes.  
    * **Grant Proposal Drafting:** Writing persuasive, highly structured technical pitches to secure research funding.  
    * **Peer-Reviewed Publishing:** Drafting concise abstracts, figures, and methodology sections for publication.

  ### 

  ### **SALVAGER**

* **Ability:** Wisdom or Intellect  
* **Description:** Salvager is the skill of salvaging materials or items from wrecked or damaged ships, buildings, or other structures. It reflects a character's ability to identify and extract valuable materials or items from damaged structures. Salvagers are responsible for ensuring that materials or items are extracted safely and efficiently. This can include things like salvaging metal from a shipwreck, or extracting valuable items from a damaged building.   
* **Mechanics:**  
  * *Valuation:* DC 15 to know if a piece of junk is worth credits.  
  * *Extraction:* DC 20 to remove a component without breaking it.  
* **Specialties:**   
  * **Deep Space Derelict Cutting:** Using industrial plasma torches to slice open airless starship hulls and structural spines.  
    * **Structural Stabilization:** Bracing sagging ship decks and collapsed buildings with jacks to prevent crushing collapses.  
    * **Hazardous Reactor Extraction:** Pulling volatile, leaking nuclear or antimatter cores out of wrecks without containment failure.  
    * **Component De-Installation:** Unbolting high-value avionics, computers, and sensors cleanly without torching the circuits.  
    * **Scrap Metal Grading:** Sorting high-value titanium, platinum wire, and hyper-alloys from low-value structural slag iron.  
    * **Underwater Wreck Salvage:** Rigging lift balloons, operating sub-sea cutting torches, and pulling cargo from ocean beds.  
    * **Urban Ruin Scavenging:** Picking clean abandoned skyscrapers and military bases for copper, wires, and functioning tech.  
    * **Rigging & Cargo Extraction:** Winching crushed freight containers out of twisted starship holds using cables.  
    * **Booby-Trap & Hazard Spotting:** Detecting unexploded ordinance, pressurized fuel lines, and live wiring in mangled wrecks.  
    * **Black-Market Salvage Washing:** Grinding off corporate serial numbers, transponder tags, and repainting recovered machinery.

  ### 

  ### **SOLDIER**

* **Ability:** Constitution or Intellect  
* **Description:** Knowledge of military life, hierarchy, logistics, and field procedures. Does *not* replace Combat skills (shooting/hitting), but covers the *job* of being a soldier.  
* **Mechanics:**  
  * *Regulations:* DC 10 to know the rules of engagement.  
  * *Field Maintenance:* DC 15 to clean and maintain gear in the field.  
  * *Drill:* DC 15 to march or coordinate a squad efficiently.  
* **Specialties:**   
  * **Field Gear Maintenance:** Stripping, cleaning, and oiling service rifles, boots, and webbing to military inspection code.  
    * **Military Regulations & Law:** Knowing the rules of engagement, uniform codes, lawful orders, and court-martial laws.  
    * **Drill & Formation Marching:** Moving in tight lock-step formations, reacting instantly to parade and field whistle blasts.  
    * **Entrenchment & Sandbagging:** Digging fighting holes, revetting trench walls, and filling sandbags to build firebases.  
    * **Guard Duty & Perimeter Security:** Standing alert night watches, checking sentry passwords, and challenging intruders.  
    * **Field Bivouac Setup:** Pitching squad tents, setting up perimeter trip-flares, and digging field latrines to hygiene specs.  
    * **Combat Communications Protocol:** Speaking in standardized radio brevity codes, call signs, and reporting coordinates clearly.  
    * **Unit Cohesion & Morale:** Supporting squadmates through combat fatigue, suppression panic, and the horrors of war.  
    * **Escort Formations:** Walking security perimeters around supply convoys, VIP officers, and artillery pieces.  
    * **Bayonet & Close-Quarters Drill:** Executing textbook thrusts, butt-strokes, and rifle parries in tight trench combat.

  ### 

  ### **TAILOR**

* **Ability:** Intelect  
* **Description:** Tailor is the skill of designing and creating clothing items, such as dresses, suits, or light armor or armor dressings. It reflects a character's ability to create clothing that is functional and aesthetically pleasing. Tailors are responsible for ensuring that clothing items fit well and are made from high-quality materials. This can include things like designing patterns, cutting fabric, and sewing clothing items.   
* **Mechanics:**  
  * *Craft Outfit:* DC 15 (Common) to DC 25 (High Fashion/Armored).  
  * *Disguise Prep:* DC 20 to alter clothing to fit a disguise.  
* **Specialties:**   
  * **High-Society Couture:** Designing and hand-stitching bespoke silk suits, evening gowns, and noble court capes.  
    * **Ballistic Fabric Weaving:** Splicing flexible aramid, carbon-weave, and spider-silk fibers into puncture-proof business suits.  
    * **Uniform Tailoring:** Sewing military uniforms, naval tunics, and dress coats with precise ribbon and medal mounts.  
    * **Pattern Drafting & Cutting:** Measuring body proportions to draft custom paper cutting patterns that fit like a second skin.  
    * **Concealed-Carry Alteration:** Modifying suit linings, coat shoulders, and vests with hidden pockets to hide pistols and blades.  
    * **Space Suit Sealing & Repair:** Stitching pressurized canvas layers, vulcanizing rubber joints, and replacing neck rings.  
    * **Leather & Beast-Hide Working:** Curing, tooling, dyeing, and stitching heavy alien hides, boots, belts, and weapon holsters.  
    * **Quick-Change Garment Design:** Creating breakaway stage costumes and reversible coats for undercover field operatives.  
    * **Embroidery & Insignia Handiwork:** Hand-stitching gold-bullion crests, unit shoulder patches, and ornate religious glyphs.  
    * **Garment Mending & Alterations:** Hemming trousers, taking in waists, darning fabric tears, and reinforcing stress seams.

  ### 

  ### **TRANSPORTER**

* **Ability:** Agility or Wisdom  
* **Description:** Transporter is the skill of transporting people or goods from one place to another, such as by driving, flying, or sailing. Transporters are responsible for ensuring people or goods are transported safely and efficiently. The professional moving of goods or people. Includes piloting commercial vehicles, loading cargo efficiently, and route planning.  
* **Mechanics:**  
  * *Load Cargo:* DC 15 to maximize cargo space or secure dangerous goods.  
  * *Route Navigation:* DC 15 to find the fastest/safest route.  
* **Specialties:**   
  * **Long-Haul Space Trucking:** Piloting heavy multi-container cargo freighters across deep-space shipping routes safely.  
    * **Armored Cash Transit:** Driving heavy security vaults and defensive bullion trucks through hostile urban zones.  
    * **Secret Compartment Rigging:** Designing and packing false fuel tanks and hidden hull voids that evade customs scans.  
    * **Cargo Weight Distribution:** Calculating center of gravity and tie-down strap tensions to prevent shifts in heavy seas/G-forces.  
    * **VIP Chauffeur Services:** Driving luxury limousines smoothly while executing evasive anti-ramming driving maneuvers.  
    * **Atmospheric Sky-Bus Transit:** Piloting mass-transit commuter hovercraft and rail cars through dense metropolitan corridors.  
    * **Hazardous Cargo Hauling:** Driving volatile liquid-fuel tankers, radioactive canisters, and bio-hazards without incident.  
    * **Intermodal Freight Transfer:** Operating gantry cranes, rail car switchers, and ramp docks to transfer bulk cargo quickly.  
    * **Smuggling Blockade Runs:** Threading military intercept nets and customs patrol perimeters to deliver illicit contraband.  
    * **Cold-Chain Courier Delivery:** Transporting delicate organs, vaccines, and living samples under strict temperature windows.

  ### 

  ### **WEAPONSMITH**

* **Ability:** Intellect or Strength  
* **Description:** Weaponsmith is the skill of creating and repairing weapons, such as swords, guns, or bows. It reflects a character's ability to create weapons that are functional and effective. Weaponsmiths are responsible for ensuring that weapons are made from high-quality materials and are designed to be effective in combat. This can include things like forging metal, sharpening blades, and repairing damaged weapons.  
* **Mechanics:**  
  * *Repair Weapon:* DC 15 to fix a jammed or broken weapon.  
  * *Craft Weapon:* DC varies by complexity (Sword DC 15, Laser Rifle DC 25).  
  * *Modify:* DC 20 to add a scope, silencer, or increased capacity.  
* **Specialties:**   
  * **Bladed Weapon Forging:** Folding, quenching, grinding, and heat-treating high-carbon steel and monomolecular edges.  
    * **Ballistic Firearm Gunsmithing:** Rifling barrels, tuning gas blocks, polishing feed ramps, and lightening trigger pulls.  
    * **Energy Weapon Optics & Lasers:** Aligning focusing prism lenses, polishing laser apertures, and calibrating beam diodes.  
    * **Heavy Weapon Servicing:** Rebuilding heavy machine gun extractors, mortar bipods, and anti-tank rocket firing pins.  
    * **Accurizing & Scope Fitting:** Bedding actions, mounting telescopic sights, and laser-boresighting long-range precision rifles.  
    * **Ammunition Reloading & Swaging:** Pressing brass cartridges, compounding custom gunpowder loads, and swaging hollow points.  
    * **Weapon Modifications:** Threading barrels for suppressors, mounting underbarrel shotguns, and installing extended magazines.  
    * **Vibro & High-Frequency Blades:** Wiring ultrasonic vibrating motors and balancing resonant tungsten blades.  
    * **Field Cleaning & Unjamming:** Rapidly drilling out squib rounds, clearing mangled brass feeds, and scraping carbon build-up.  
    * **Exotic Material Tempering:** Machining titanium alloys, depleted uranium darts, and plasma-containment coils.

  ### 

# **\- SOCIAL**

# **SOCIAL**

## 

## **EXPRESSION SKILLS**

**Core Mechanic:** **d20 \+ Skill Rank \+ Charisma Mod vs. Challenge Rating (CR)**

* **Performance/Busking:** The result determines the quality of the show and potential tips/wealth earned.  
  * *DC 10:* Amateur performance (Standard success).  
  * *DC 20:* Professional/Memorable performance.  
  * *DC 30:* Legendary performance.  
* **Influence:** You can use these skills to alter the attitude of an NPC (Make them Friendly/Helpful).

### 

  ### **Acting**

* **Ability:** Charisma  
* **Description:** Acting is the skill of portraying a character or role in a performance. It reflects a character's ability to use body language, facial expressions, and tone of voice to convey emotions and communicate effectively. Acting is used whenever a character needs to deceive or persuade others, or to perform in a play or other theatrical production. The ability to embody a character, fake emotions, or improvise a persona. Unlike Disguise (which is visual), Acting is behavioral. It allows a character to pass as a soldier, a doctor, or a beggar through mannerisms and speech patterns alone.  
* **Mechanics:**  
  * *Impersonation:* Opposed check vs. **Insight**.  
  * *Distraction:* Create a scene to draw attention away from allies (DC 15).  
* **Specialties:**   
  * **Voice & Accent Mimicry:** Replicating the specific cadence, regional dialect, and vocal pitch of specific individuals.  
    * **Improvisational Roleplay:** Reacting organically in character when an undercover cover story goes completely off the rails.  
    * **Method Immersion:** Living, thinking, and behaving completely as an assumed identity for months at a time.  
    * **Stage & Holo-Screen Acting:** Delivering powerful, nuanced, and tearful dramatic performances before cameras or live crowds.  
    * **Feigning Injury & Death:** Realistically collapsing, faking seizures, and playing dead to fool passing guards.  
    * **Physical Mannerism Masking:** Changing one's walk, posture, head tilt, and physical gait to match a completely different persona.  
    * **Theatrical Stage Combat:** Executing convincing punches, sword swings, and falls that look lethal without causing harm.  
    * **Dramatic Monologuing:** Holding an entire audience or dining hall spellbound through dramatic pacing and projection.  
    * **Musical Theatre:** Seamlessly integrating song, stage dialogue, and stylized movement into a single performance.  
    * **Comedic Timing:** Delivering humorous acting cues, physical slapstick, and reaction beats on stage with perfection.

  ### 

  ### **Comedy**

* **Ability:** Charisma  
* **Description:** Comedy is the skill of making people laugh or feel amused. It reflects a character's ability to use humor and wit to entertain others. Comedy is used whenever a character needs to lighten the mood or entertain others, or to perform in a comedy show or other comedic production. The art of humor, satire, and timing. Used to entertain, defuse tense situations, or provoke an enemy into making a mistake (Taunt).  
* **Mechanics:**  
  * *Entertain:* Standard performance mechanics.  
  * *Defuse Hostility:* DC 20 to turn a Hostile NPC to Indifferent (if they have a sense of humor).  
  * *Taunt:* Opposed check vs. **Willpower**. Success causes the target to focus on you or suffer a penalty to Logic/Discipline due to anger.  
* **Specialties:**   
  * **Stand-Up Routines:** Delivering written comedic material, setups, and punchlines to pack cantinas and clubs.  
    * **Combat Taunts & Jeering:** Enraging an enemy soldier with sharp, humiliating barbs so they break cover and attack wildly.  
    * **Political Satire & Parody:** Mocking corrupt corporate moguls, planetary governors, and imperial policies through sharp humor.  
    * **Hostility Defusal Through Humor:** Cracking the perfect joke during an armed standoff to disarm tension and get laughs.  
    * **Roasting & Heckler Put-Downs:** Brutally shredding an abrasive heckler or arrogant rival with instant, razor-sharp wit.  
    * **Slapstick & Physical Humor:** Executing hilarious pratfalls, exaggerated double-takes, and physical gags for laughs.  
    * **Dark & Gallows Humor:** Using grim, morbid humor to keep spirits up and relieve stress in horrific combat zones.  
    * **Deadpan Irony:** Delivering absurd statements with an absolute stone face to confuse and amuse listeners.  
    * **Impressionist Parody:** Exaggerating the vocal tics, pomposity, and flaws of public celebrities for comedic effect.  
    * **Crowd Banter & Crowd-Work:** Interacting spontaneously with cantina patrons to turn unexpected events into comedic gold.

  ### 

  ### **Dancing**

* **Ability:** Charisma (Expression) or Agility (Technique)  
* **Description:** Dancing is the skill of moving rhythmically to music. It reflects a character's ability to use body movements to express emotions and communicate effectively. Dancing is used whenever a character needs to entertain others or perform in a dance show or other performance. Rhythmic bodily movement to music. This skill covers everything from high-society ballroom dancing to club dancing or traditional folk dances. It is a display of grace, stamina, and social awareness.  
* **Mechanics:**  
  * *Performance:* Standard performance mechanics.  
  * *Partnering:* DC 15 to expertly lead or follow a partner, granting them a \+2 bonus to their own Social checks during the event.  
* **Specialties:**   
  * **Ballroom & Court Waltz:** Gliding gracefully through formal waltzes and minuet dances at aristocratic high galas.  
    * **Club & Rave Freestyle:** Moving fluidly to pounding electronic music, light-shows, and underground dance beats.  
    * **Zero-G Aerial Dance:** Executing three-dimensional pirouettes, flips, and spins in microgravity chambers.  
    * **Classical Ballet:** Performing disciplined, highly athletic leaps, toe-spins, and technical classical routines.  
    * **Folk & Cultural Dances:** Executing the traditional, historic footwork, line-dances, and circle-dances of worlds.  
    * **Sensual & Seductive Dance:** Performing alluring, captivating movements designed to mesmerize and distract patrons.  
    * **Martial Katas & War Dances:** Executing weapon dances, spear-stepping, and rhythmic intimidations before combat.  
    * **Breakdancing & Urban Acrobatics:** Executing head-spins, ground-freezes, and explosive popping and locking on street corners.  
    * **Choreographed Group Pop:** Synchronizing complex, high-energy group dance routines with background performers.  
    * **Tap & Percussive Footwork:** Creating complex acoustic rhythms and syncopated beats purely through iron-shod shoe taps.

  ### 

  ### **Disguise**

* **Ability:** Charisma (Acting the part) or Intellect (Creating the prop)  
* **Description:** Disguise is the skill of changing one's appearance to look like someone else. It reflects a character's ability to use makeup, costumes, and other techniques to alter their appearance. Disguise is used whenever a character needs to deceive or impersonate someone else, or to hide their true identity. The application of props, makeup, prosthetics, and costumes to change one's physical appearance. While Acting covers behavior, Disguise covers the visual fabrication.  
* **Mechanics:**  
  * *Create Disguise:* The result of your check becomes the **DC** for anyone trying to see through it using **Alertness** or **Investigation**.  
  * *Speed Disguise:* Rushing a disguise (-5 penalty) takes 1d4 rounds. Proper application takes 10-30 minutes.  
* **Specialties:**   
  * **Prosthetic & Latex Application:** Applying false noses, cheekbones, brow ridges, and synthetic skin appliances.  
    * **Cosmetic Makeup & Contouring:** Using shading, highlights, and foundation to drastically alter perceived face shapes.  
    * **Specific Individual Cloning:** Replicating the exact face, wrinkles, scars, and hair of a known living person.  
    * **Cross-Species Camouflage:** Using silicone masks, false ears, and body padding to pass as a different humanoid alien race.  
    * **Quick-Change Techniques:** Shedding an entire outer costume and wig in three seconds inside a crowd or alley.  
    * **Aging & De-Aging Effects:** Simulating liver spots, graying hair, wrinkles, or youthful skin with paints and glues.  
    * **Body Shape Padding:** Wearing fat suits, muscle pads, or slouch braces to alter height, build, and posture completely.  
    * **Cyberware Concealment:** Using synthetic flesh and dermal plating to hide chrome cybernetic limbs and data jacks.  
    * **Camouflage Makeup:** Painting matte disruptive patterns across faces and gear to blend into jungle or night terrain.  
    * **Holographic Disguise Tuning:** Calibrating holographic light-emitters to project a false digital identity shell over gear.

  ### 

  ### **Keyboard**

* **Ability:** Charisma  
* **Description:** Keyboard is the skill of playing a keyboard instrument, such as a piano or synthesizer. It reflects a character's ability to create music using a keyboard instrument. Keyboard is used whenever a character needs to create music or perform in a musical production. Proficiency with instrument interfaces involving keys, buttons, or haptic pads. In a Sci-Fi setting, this ranges from classic pianos to complex holographic synthesizers.  
* **Mechanics:**  
  * *Performance:* Standard performance mechanics.  
  * *Composition:* Write a piece of music (Intellect \+ Keyboard).  
* **Specialties:**   
  * **Acoustic Grand Piano:** Performing complex classical sonatas, jazz improvisations, and concertos on grand pianos.  
    * **Pipe & Cathedral Organ:** Operating multi-manual organ keyboards, foot pedals, and stops in massive echoic halls.  
    * **Electronic Synthesizer Workstations:** Programming oscillators, low-pass filters, and playing electronic synth leads.  
    * **Holographic Light-Keys:** Playing futuristic musical consoles composed of projected, floating hard-light keys.  
    * **Keytar & Stage Performance:** Playing portable keyboard instruments while moving aggressively across live stages.  
    * **Harpsichord & Early Instruments:** Performing delicate, plucking keyboard compositions from archaic eras.  
    * **Haptic Touch-Pads:** Playing responsive, velocity-sensitive silicone pads and matrix controller surfaces.  
    * **Accordion & Squeeze-Box:** Operating bellow-driven reed key boxes while managing rhythmic bass buttons.  
    * **Ambient Soundscape Design:** Generating lush, evolving chordal pads and atmospheric sonic backdrops live.  
    * **Microtonal Synthesizers:** Navigating alien tuning scales and non-standard pitch intervals on custom boards.

  ### 

  ### **Legerdemain**

* **Ability:** Agility (Dexterity) or Charisma (Misdirection)  
* **Description:** Legerdemain is the skill of performing sleight of hand tricks and illusions. It reflects a character's ability to use misdirection and other techniques to deceive others. Legerdemain is used whenever a character needs to deceive or entertain others such as switching items, stealing or performing a stage or table magic show. Sleight of hand, stage magic, and manual dexterity. While often used for entertainment, it is the primary skill for pickpocketing, palming small objects, and concealing weapons.  
* **Mechanics:**  
  * *Pickpocket:* DC 20 to take an item unnoticed (or opposed by Alertness).  
  * *Conceal Item:* DC 15 to hide a small weapon/item on your person (opposed by Search).  
  * *Stage Magic:* Perform a trick to entertain or distract.  
* **Specialties:**   
  * **Pickpocketing:** Slitting purse strings, slipping wallets from coats, and lifting keycards unnoticed.  
    * **Palming & Small Item Concealment:** Hiding daggers, datachips, coins, or vials in the palm or sleeve seamlessly.  
    * **Card & Coin Manipulation:** Executing flawless shuffles, coin-rolls, false cuts, and close-up stage magic tricks.  
    * **Stage Illusions & Escapology:** Slipping out of padlocked chains, straitjackets, and water tanks before an audience.  
    * **Quick-Draw Palming:** Snapping a hidden derringer or holdout blade from a sleeve straight into a firing grip.  
    * **Item Swapping:** Switching an authentic relic or keycard with an exact fake right before an inspector's eyes.  
    * **Shell Game & Three-Card Monte:** Manipulating cups, shells, and cards so fast that onlookers cannot track the prize.  
    * **Wax Impression Key Duplication:** Pressing a guard's physical security key into soft wax in a single casual handshake.  
    * **Distraction Flairs:** Snapping fingers, dropping coins, or generating sparks with one hand while the other steals.  
    * **Planting Evidence:** Slipping hot contraband, tracking bugs, or stolen narcotics into an unsuspecting mark's pocket.

  ### 

  ### **Oratory**

* **Ability:** Charisma  
* **Description:** Oratory is the skill of public speaking. It reflects a character's ability to use language and tone of voice to persuade and inspire others. Oratory is used whenever a character needs to convince a group to take a certain course of action, or to deliver a speech or other public address. The art of public speaking. Used to inspire crowds, deliver sermons, debate in court/senate, or give orders on a battlefield.  
* **Mechanics:**  
  * *Inspire:* DC 15 to grant allies a \+1 morale bonus to Willpower saves for 1 hour.  
  * *Persuade Crowd:* Shift the attitude of a group (opposed by the group's collective Will or a set DC).  
* **Specialties:**   
  * **Political Rally Speeches:** Delivering fiery, populist orations that sweep elections and energize masses of voters.  
    * **Military Pre-Battle Speeches:** Delivering inspiring addresses that harden resolve and banish fear before bloody assaults.  
    * **Religious Sermons & Preaching:** Delivering sermons that move congregations to tears, repentance, and zealotry.  
    * **Courtroom Advocacy:** Presenting closing arguments to juries and judges with compelling narrative power.  
    * **Eulogies & Funerary Orations:** Honoring the deceased with dignified, deeply moving reflections that comfort mourners.  
    * **Crowd Riot Incitement:** Whipping an already angry protest crowd into an uncontrollable, barrier-smashing mob.  
    * **Philosophical Lectures:** Explaining abstract ethical concepts and ideological frameworks to university halls.  
    * **Diplomatic Keynote Addresses:** Addressing global assemblies with statesmanlike poise, vision, and gravitas.  
    * **Acoustic Vocal Projection:** Throwing your voice without microphones to fill cavernous halls and outdoor stadiums.  
    * **Debate Cross-Examination:** Dismantling an opponent's speech line-by-line with sharp rhetorical traps and counter-arguments.

  ### 

  ### **Percussion**

* **Ability:** Charisma  
* **Description:** Percussion is the skill of playing percussion instruments, like drums or cymbals. It reflects a character's ability to create music using percussion instruments. Percussion is used whenever a character needs to create music or perform in a musical production. Proficiency with instruments that are struck, shaken, or scraped. This is often the foundation of musical timing and rhythm.  
* **Mechanics:**  
  * *Performance:* Standard performance mechanics.  
  * *Rhythm:* DC 15 to maintain a beat that helps others coordinate physical tasks (e.g., rowing, marching), granting a \+1 bonus to their endurance-based checks.  
* **Specialties:**   
  * **Acoustic Drum Kits:** Playing intricate rock, jazz, and metal beats using snare, kick, tom-toms, and cymbals.  
    * **Military War Drums & Snares:** Keeping the marching cadence, signaling battle lines, and drumming troops into action.  
    * **Orchestral Timpani & Gongs:** Striking tuned kettle drums and massive orchestral gongs for dramatic climaxes.  
    * **Hand Percussion (Congas/Bongos):** Slapping, popping, and palm-striking skin hand drums with polyrhythmic speed.  
    * **Electronic Drum Pads:** Playing velocity-sensitive rubber pads triggering synthesized samples and sub-bass kicks.  
    * **Tuned Mallet Percussion:** Playing wooden marimbas, steel xylophones, and vibraphones with precision mallets.  
    * **Tribal Ritual Drumming:** Driving deep, hypnotic, relentless drum circles that induce trance states in participants.  
    * **Bodhrán & Frame Drums:** Beating shallow Celtic-style frame drums with double-ended tippers while modulating pitch.  
    * **Cymbals, Triangles & Chimes:** Accenting delicate sonic passages with crashes, rides, sizzling chimes, and iron bells.  
    * **Rhythmic Rowing/Marching Cadence:** Maintaining a continuous, driving beat that coordinates team endurance tasks.

  ### 

  ### **Singing**

* **Ability:** Charisma (Performance) or Constitution (Breath Control)  
* **Description:** Singing is the skill of using one's voice to create music. It reflects a character's ability to use pitch, tone, and rhythm to convey emotions and communicate effectively. Singing is used whenever a character needs to entertain others or perform in a musical production. The use of the voice as an instrument. This creates a powerful connection with audiences without the need for equipment.  
* **Mechanics:**  
  * *Performance:* Standard performance mechanics.  
  * *Project Voice:* DC 15 to be heard clearly over loud noise/combat.  
* **Specialties:**   
  * **Classical Opera:** Projecting high, unamplified vibrato vocals across full orchestras with crystalline resonance.  
    * **Rock & Metal Belting:** Singing powerful, gritty, overdriven lead vocals with intentional rasp and chest-drive.  
    * **Choral & Sacred Hymns:** Blending harmonious soprano, alto, tenor, or bass lines into cathedral choirs.  
    * **Lounge Jazz & Torch Songs:** Singing breathy, smoky, intimate vocal melodies into close microphones in clubs.  
    * **Folk Ballads & Sea Shanties:** Singing narrative tales and rhythmic work songs that crowds can easily chant along with.  
    * **Overtone & Throat Singing:** Producing two distinct pitch frequencies simultaneously through vocal-tract resonance.  
    * **Sub-Harmonic Vocalizations:** Dropping into deep bass registers that rattle ribcages and project primal authority.  
    * **Vocal Whistling & Warbling:** Executing intricate melodic trills and whistle registers mimicking birds or wind.  
    * **Battle Hymns & Cadences:** Projecting rhythmic vocal cadences over gunfire to keep allied morale unshakeable.  
    * **Microtonal Alien Chants:** Navigating vocal melodies that slide between traditional half-step and quarter-step scales.

  ### 

  ### **String**

* **Ability:** Charisma  
* **Description:** String is the skill of playing string instruments like guitar or violin. It reflects a character's ability to create music using strings. String is used whenever a character needs to create music or perform in a musical production.Proficiency with instruments that produce sound via vibrating strings, whether plucked, strummed, or bowed.  
* **Mechanics:**  
  * *Performance:* Standard performance mechanics.  
* **Specialties:**   
  * **Electric Guitar:** Shredding high-gain lead solos, tapping fretboards, and chugging heavy distorted rhythm riffs.  
    * **Acoustic Guitar:** Fingerpicking, flatpicking, and strumming steel-string and nylon acoustic guitars.  
    * **Violin & Fiddle:** Bowing high-speed country fiddle breakdowns and expressive classical violin concertos.  
    * **Cello & Double Bass:** Playing rich, deep, resonant bowed basslines that provide orchestral weight.  
    * **Concert Harp:** Plucking cascading arpeggios and glissandos across large, multi-pedal classical harps.  
    * **Lute & Mandolin:** Plucking rapid, delicate tremolos and Renaissance melodies on doubled-string instruments.  
    * **Electric Bass Guitar:** Locking into the drummer's groove with slap-and-pop basslines and heavy walking rhythms.  
    * **Exotic Alien Zithers:** Plucking flat, resonant soundboards equipped with alien sympathetic drone strings.  
    * **Banjo Playing:** Executing high-speed three-finger Scruggs-style rolls and clawhammer rhythm patterns.  
    * **Bowed Rebec & Archaic Strings:** Playing ancient, gut-stringed instruments with historical bows and rustic tunings.

  ### 

  ### **Style**

* **Ability:** Charisma  
* **Description:** Style is the skill of creating a unique and distinctive appearance. It reflects a character's ability to use clothing, accessories, and other techniques to create a unique and memorable look. Style is used whenever a character needs to make a good impression or to stand out from the crowd. The knowledge of fashion, grooming, and personal aesthetics. It is the ability to make an impression before speaking a word. It involves dressing appropriately for high society or looking intimidating in street armor.  
* **Mechanics:**  
  * *First Impression:* DC 15 to grant a \+2 bonus to the first **Diplomacy** or **Intimidate** check made in an encounter.  
  * *Blend In:* Dress to match a specific social caste or group (opposed by Culture/Streetwise).  
* **Specialties:**   
  * **High-Society Elegance:** Selecting tailored suits, noble sashes, and jewelry that radiate immense wealth and status.  
    * **Streetwear & Cyberpunk Chic:** Styling leather jackets, neon hair, combat boots, and decals for maximum street cred.  
    * **Corporate Executive Polish:** Dressing in crisp, minimalist, intimidating boardroom suits that project pure competence.  
    * **Military & Uniform Crispness:** Ironing razor-sharp uniform creases, polishing boots to mirrors, and aligning ribbons.  
    * **Intimidating Mercenary Look:** Layering battered combat rigs, scars, and holster wear that scream hardened killer.  
    * **Subtle Blend-In (Gray Man):** Selecting ultra-nondescript, beige, forgettable clothing that leaves zero mental impression.  
    * **Avant-Garde Alien Couture:** Pulling off radical alien silhouettes, kinetic fabrics, and exotic off-world color clashes.  
    * **Gala Glamour:** Wearing head-turning, showstopping formal eveningwear that dominates room attention instantly.  
    * **Retro & Nostalgic Vintage:** Rocking period-accurate antique threads, vintage flight jackets, and classic hats.  
    * **Personal Grooming & Scent:** Mastering hair styling, facial hair trimming, and signature scents that leave lasting impressions.

  ### 

  ### **Wind**

* **Ability:** Charisma  
* **Description:** Wind is the skill of playing wind instruments, like a flute or trumpet. It reflects a character's ability to create music using wind instruments. Wind is used whenever a character needs to create music or perform in a musical production. Proficiency with instruments that require breath to play, including woodwinds and brass.  
* **Mechanics:**  
  * *Performance:* Standard performance mechanics.  
* **Specialties:**   
  * **Brass Trumpet & Cornet:** Blasting clear, ringing fanfares, jazzy muted solos, and piercing high-register brass lines.  
    * **Saxophone (Alto/Tenor/Baritone):** Playing soulful, expressive jazz melodies, blues runs, and smooth vibrato riffs.  
    * **Flute & Piccolo:** Flying through rapid, airy classical scales, birdlike trills, and breathy folk melodies.  
    * **Clarinet & Reed Instruments:** Navigating woody, warm, wide-ranging melodies across complex key mechanisms.  
    * **Bagpipes & War Pipes:** Driving continuous double-drone pipes and chanter melodies that carry for miles across fields.  
    * **Harmonica & Blues Harp:** Bending reeds, cross-harping, and chugging train rhythms using cupped hands.  
    * **Trombone & Low Brass:** Moving slide mechanics to execute smooth glissandos, punchy bass lines, and orchestral power.  
    * **Hunting Horns & Bugles:** Sounding natural, valveless brass calls that carry across forests and battlefields.  
    * **Alien Double-Pipes:** Playing exotic instruments requiring circular breathing to produce unceasing melodic loops.  
    * **Didgeridoo & Drone Wind:** Producing deep, guttural, cyclical drone vibrations infused with vocalized animal growls.

## **MANIPULATION**

These skills are used to influence others, hide intentions, or read social situations.

### 

### **Insight**

* **Ability:** Wisdom (or Charisma for active social maneuvering)  
* **Description:** Insight is the skill of understanding and empathizing with others. It reflects a character's ability to comprehend and appropriately react to both broad problems of society and interpersonal struggles. Insight is used whenever a character needs to understand the emotions and feelings of others, whether it's to avoid conflict or to build relationships. This can include things like identifying social cues, taking others' perspectives, demonstrating empathy and compassion, and showing concern for the feelings of others. The skill of understanding and empathizing with others. It reflects a character's ability to comprehend and appropriately react to both broad societal problems and interpersonal struggles. Insight is used to read emotions, detect lies, and understand hidden motives.  
* **Mechanics:**  
  * **Insight:** Opposed check vs. **Bluff**. Success reveals if the target is lying or hiding something.  
  * **Read Situation:** DC 15 to pick up on social dynamics or hidden tension in a room.  
* **Specialties:**  
  * **Lie Detection:** Catching subtle facial tics, vocal pitch spikes, and pupil dilations that expose falsehoods.  
    * **Emotional Empathy:** Sensing buried grief, simmering rage, or sheer panic beneath a calm exterior.  
    * **Cold Reading:** Deducing a stranger's profession, wealth, origin, and marital status from worn clothes and ring marks.  
    * **Motive Deduction:** Figuring out what an individual truly wants—money, vengeance, glory, or safety.  
    * **Social Tension Assessment:** Walking into a crowded cantina or boardroom and instantly sensing imminent violence.  
    * **Coercion & Blackmail Detection:** Spotting when an NPC is speaking under duress, extortion, or psychic domination.  
    * **Hidden Relationship Spotting:** Noticing subtle glances and body orientation that reveal secret lovers or conspirators.  
    * **Mental Instability Profiling:** Recognizing symptoms of paranoia, cyber-psychosis, withdrawal, or chemical imbalance.  
    * **Body Language Reading:** Predicting an opponent's sudden draw, reach for a weapon, or sprint for an exit before it starts.  
    * **Sincerity Gauging:** Determining if a stated apology, peace offer, or surrender is completely authentic or a setup.

  ### 

  ### **Bluff**

* **Ability:** Charisma  
* **Description:** Bluff is the skill of lying convincingly. It reflects a character's ability to deceive others and manipulate situations to their advantage. Bluff is used whenever a character needs to convince someone of something that isn't true, whether it's to avoid danger or to gain information. This can include things like lying about one's identity, pretending to be someone else, or making up a story to cover up a mistake. The ability to lie convincingly, deceive others, and manipulate situations to one’s advantage. It covers everything from little white lies to elaborate cons and feints in combat.  
* **Mechanics:**  
  * **Deceive:** Opposed check vs. **Insight (Sense Motive)**.  
  * **Feint:** Opposed check vs. **Insight** (or Defense) in combat to deny a target their Agility bonus to Defense for the next attack.  
  * **Create Distraction:** DC 15+ to momentarily divert attention (often used to Hide).  
* **Specialties:**  
  * **Fast-Talking:** Overwhelming security guards and dock officials with rapid jargon and manufactured emergencies.  
    * **Tactical Feints:** Misleading an opponent's combat guard with false eye glances, fake lunges, and baiting strikes.  
    * **Impromptu Cover Stories:** Fabricating believable, airtight explanations on the fly when caught red-handed in restricted areas.  
    * **Digital Credential Faking:** Confidently presenting forged ID passes and cloned clearance badges without sweating.  
    * **Poker Face & Emotional Masking:** Keeping an absolute stone-cold, unreadable expression during intense stress and pain.  
    * **Grifts & Confidence Scams:** Gaining the total trust of marks to swindle their wealth, ships, or warehouse access.  
    * **False Identity Impersonation:** Convincingly pretending to be an inspector, naval officer, or physician through tone alone.  
    * **Shifting the Blame:** Planting verbal suspicion onto an innocent bystander or rival to deflect internal investigations.  
    * **Manufactured Rumors:** Spreading false gossip and fabricated leaks through a spaceport to manipulate public behavior.  
    * **Misdirection of Intent:** Convincing a room you are unarmed and terrified while quietly lining up an ambush.

  ### **Barter**

* **Ability:** Charisma (or Wisdom)  
* **Description:** Barter is the skill of trading goods and services without relying solely on currency. It reflects a character's ability to negotiate value, haggle for better prices, and reach mutually beneficial agreements in trade. Barter is used whenever a character needs to acquire items, services, or information through direct exchange. This includes assessing the relative value of goods, identifying market trends, and convincing others of the worth of a trade. The art of negotiation and exchange.  
* **Mechanics:**  
  * **Negotiate:** Opposed check vs. the target's **Diplomacy** or **Insight**.  
  * **Appraise:** DC 15+ to determine the true value or rarity of an object or service.  
* **Specialties:**  
  * **Street Market Haggling:** Whittling down the asking price of street vendors and cantina fences credit by credit.  
    * **Bulk Discount Negotiation:** Securing large percentage-off price breaks when buying weapon crates or fuel tons in bulk.  
    * **Non-Monetary Trade Swapping:** Negotiating equal-value item-for-item swaps when credits are useless or unavailable.  
    * **Margin Squeezing:** Forcing suppliers to drop shipping, packaging, and handling fees from commercial sales.  
    * **Trade Goods Appraisal:** Accurately assessing what your own carried loot is realistically worth to a local buyer.  
    * **Closing the Deal:** Reading the precise moment a buyer or seller is on the fence and locking down the transaction.  
    * **Credit Terms & Financing:** Bargaining low interest rates and extended repayment timelines on borrowed capital.  
    * **Collateral Negotiation:** Convincing loan sharks or merchants to accept ships, deeds, or cyberware as trade collateral.  
    * **Pawn Valuation:** Squeezing the maximum instant credit payout from pawn brokers for stolen or hot goods.  
    * **Surplus Liquidating:** Dumping unwanted bulk inventory quickly onto frontier merchants without taking a total loss.

  ### 

  ### **Diplomacy**

* **Ability:** Charisma  
* **Description:** Diplomacy is the skill of negotiating and persuading others. It reflects a character's ability to communicate effectively and build relationships with others. Diplomacy is used whenever a character needs to convince someone of something that is true, whether it's to gain support or to resolve a conflict. This can include things like negotiating a peace treaty, convincing a king to support a cause, or persuading a merchant to lower their prices. The art of negotiation, persuasion, and maintaining positive relations. It is used to convince others of the truth, gain support, resolve conflicts peacefully, and navigate complex social hierarchies.  
* **Mechanics:**  
  * **Influence Attitude:** Change an NPC's attitude (e.g., from Hostile to Unfriendly, or Indifferent to Friendly). DC depends on the starting attitude (DC 15 for Indifferent, DC 25+ for Hostile).  
  * **Negotiation:** Opposed check vs. the target's **Diplomacy** or **Insight**.  
  * **Gather Information (Formal):** DC 15+ to find information through official channels or high society.  
* **Specialties:**  
  * **Hostility De-escalation:** Talking down armed, angry standoffs and convincing aggressive groups to lower their weapons.  
    * **Formal Treaty Drafting:** Formulating compromise terms, ceasefire articles, and non-aggression treaties between factions.  
    * **Consensus Building:** Uniting bickering political factions or mercenary squads behind a singular operational goal.  
    * **High-Society Etiquette:** Seamlessly observing royal court manners, formal toasts, and elite seating protocols.  
    * **Inter-Faction Mediation:** Serving as an objective neutral arbitrator to settle disputes between rival mining guilds or houses.  
    * **Goodwill Cultivation:** Turning neutral or mildly suspicious planetary leaders into warm, cooperative political allies.  
    * **Official Information Gathering:** Eliciting sensitive political intelligence smoothly during high-level formal state dinners.  
    * **Apology & Restitution Brokerage:** Smoothing over diplomatic insults and international blunders with grace and reparations.  
    * **Alliance Brokering:** Forging binding military and mutual-defense pacts between independent star systems.  
    * **Hostage Negotiation:** Communicating calmly with kidnappers and terrorists to trade concessions for living captives.

  ### 

  ### **Intimidate**

* **Ability:** Charisma (or Strength for physical threats)  
* **Description:** Intimidation is the skill of using fear and threats to control others. It reflects a character's ability to instill fear in others and manipulate situations to their advantage. Intimidate is used whenever a character needs to scare someone into doing something, whether it's to gain information or to avoid danger. This can include things like threatening physical harm, using one's reputation to intimidate others, or using psychological tactics to make someone feel powerless. The skill of coercing others through fear, physical threats, or sheer force of personality. It can be used to interrogate prisoners, demoralize enemies, or force compliance.  
* **Mechanics:**  
  * **Coerce:** Opposed check vs. the target's **Insight** or **Willpower** (Level \+ Wisdom). Success forces the target to act friendly or compliant temporarily.  
  * **Demoralize:** Standard action in combat. Opposed check vs. target's **Insight** or **Willpower**. Success leaves the target **Shaken** (-2 to attacks/checks) for 1 round.  
* **Specialties:**  
  * **Physical Menace:** Towering over targets, cracking knuckles, and using size and muscle mass to compel obedience.  
    * **Interrogation Under Duress:** Breaking the psychological resolve of captured prisoners to extract codes and coordinates.  
    * **Cold Psychological Menace:** Speaking in calm, quiet, matter-of-fact tones that make it clear refusal equals death.  
    * **Combat Demoralization:** Unleashing a terrifying war cry or lethal glare to leave an opponent shaken and hesitation-prone.  
    * **Reputation Posturing:** Using your known notorious body count or terrifying faction affiliation to cow bystanders.  
    * **Veiled Extortion & Threats:** Implying terrible harm to a target's family, business, or ship without stating it outright.  
    * **Stare-Down:** Winning intense eye-contact dominance battles to force a rival to look away and yield ground.  
    * **Interrogative Shock Tactics:** Slamming tables, firing near a target's ear, or breaking fingers to shatter stubborn silences.  
    * **Crowd Dispersal:** Roaring commands that send curious civilian crowds and street onlookers fleeing in absolute terror.  
    * **Exploiting Phobias:** Leveraging a target's known terror of vacuum, drowning, insects, or fire to force immediate compliance.

  ### 

### **Leadership**

* **Ability:** Charisma  
* **Description:** Leadership is the skill of inspiring, organizing, and guiding others toward a common goal. It reflects a character's ability to motivate their allies, maintain morale, and direct the flow of a team or larger group. Leadership is used whenever a character needs to command respect, manage a team, or provide direction in challenging circumstances. It covers leading troops in battle, managing organizational structures, or simply keeping a group focused and cohesive under pressure.  
* **Mechanics:**  
  * *Inspire:* DC 15 to grant allies a \+1 morale bonus to specific checks or saves for a duration.  
  * *Organize:* DC 20 to coordinate a team's efforts for a complex project, reducing time or increasing efficiency.  
  * *Opposed Check:* Used to maintain morale against fear or discord effects.  
* **Specialties:**   
  * **Battlefield Tactical Command:** Issuing clear, authoritative firing and movement orders in the deafening noise of combat.  
    * **Inspiring Speeches:** Delivering electrifying orations that banish fear, rally broken lines, and restore morale.  
    * **Crew Morale Management:** Maintaining high spirits, solving internal grievances, and keeping a starship crew cohesive.  
    * **Crisis Command:** Stepping into sudden chaos—explosions, hull breaches, captain deaths—and taking total control.  
    * **Delegation & Task Assignment:** Assigning the exact right operative to the exact right task to maximize group efficiency.  
    * **Mutiny Suppression:** Quelling brewing insurrections and subduing rebellious crewmen through sheer force of will.  
    * **Rallying Broken Allies:** Snapping panicked or terrified companions out of shock and putting their weapons back in the fight.  
    * **Strategic Vision Planning:** Formulating long-term campaign roadmaps that keep an entire faction or fleet pointed forward.  
    * **Mentorship & Drill:** Training recruits to overcome their personal doubts, forging them into disciplined squad members.  
    * **Personal Accountability:** Taking public responsibility for failures to retain absolute loyalty and trust from subordinates.

  ### **Streetwise**

* **Ability:** Charisma (or Wisdom)  
* **Description:** Streetwise is the skill of navigating the criminal underworld and understanding the social dynamics of the streets. It reflects a character's ability to survive and thrive in dangerous and unpredictable environments. Streetwise is used whenever a character needs to gather information or make connections in the criminal underworld, whether it's to gain access to resources or to avoid danger. This can include things like finding a black market dealer, identifying a gang's territory, or navigating a dangerous neighborhood. Knowledge of the urban environment, criminal underworld, and how to navigate the "streets." It involves gathering information from informal contacts, finding black markets, and avoiding trouble in dangerous neighborhoods.  
* **Mechanics:**  
  * **Gather Information (Informal):** DC 10-20 to pick up rumors, find a specific person, or locate illicit goods. Takes 1d4 hours.  
  * **Urban Survival:** DC 15 to find a safe house or avoid street gangs.  
  * **Network:** locating a fence or fixer.  
* **Specialties:**  
  * **Black Market Fences:** Locating reliable criminal brokers who buy hot cyberware, stolen weapons, and unmarked bullion.  
    * **Underworld Informant Networks:** Knowing which bartenders, street urchins, and cabbies have their ears to the ground.  
    * **Gang Territory & Boundaries:** Recognizing street graffiti, gang colors, and border lines to avoid lethal ambushes.  
    * **Safehouse Identification:** Locating discreet, off-the-books apartments, crash pads, and sewer basements to lay low.  
    * **Criminal Cant & Slang:** Speaking and decoding the shifting street argot, hand signs, and slang of local syndicates.  
    * **Bribing Officials:** Knowing exactly which customs inspectors or beat cops take payoffs, and the going price for silence.  
    * **Loan Shark Navigation:** Negotiating survival terms with underworld loan sharks without losing your kneecaps.  
    * **Spotting Plainclothes Law:** Pointing out disguised undercover detectives and corporate internal security spooks in crowds.  
    * **Illicit Contraband Sourcing:** Procuring illegal combat drugs, untracked blasters, and forged transit papers in alleys.  
    * **Urban Evasion:** Slipping away through winding back-alleys, fire escapes, and sewer drains when syndicates hunt you.

# **\- COMBAT**

# **COMBAT** 

## 

## **ARCHAIC COMBAT SKILLS**

Game Mechanic for Attacks: d20 \+ Skill Rank \+ Ability Modifier vs. Target’s Defense  
Game Mechanic for Defense: Defense Skill Rank is added to the character's Static Defense Score (10 \+ Agility \+ Defense Skill \+ Armor/Size Mods).

### 

### **DEFENSE**

* **Ability:** Agility  
* **Description:** Defense is the art of self-preservation in combat. unlike other combat skills used to inflict damage, this skill measures a character's ability to avoid being hit, deflect blows, or position themselves advantageously to mitigate harm. It represents situational awareness, footwork, and the effective use of protective gear like shields.  
* **Game Mechanics:**  
  * **Passive Defense:** Your Rank in this skill is directly added to your **Defense Score**. (e.g., A character with Agility \+2 and Defense Rank 4 has a base Defense of 16).  
  * **Active Defense (Reaction):** In specific tactical situations (such as the *Total Defense* action), you may roll a Defense check to replace your static score or negate specific maneuvers like Trips or Grapples.  
* **Specialties:**  
  * **Evasion & Footwork:** Dodging, weaving, and ducking under melee swings and incoming kinetic projectiles.  
  * **Solid Shield Blocking:** Intercepting arrows, blades, and blunt trauma directly on a heavy metal or composite shield.  
  * **Energy Shield Deflection:** Angling hand-mounted hard-light or plasma shields to bounce energy bolts harmlessly away.  
  * **Weapon Parrying:** Using your own blade or staff to deflect an incoming melee strike off its attack line.  
  * **Total Defense Stance:** Sacrificing all offensive action to curl behind cover or shields, maximizing survivability.  
  * **Roll & Tuck (Impact Absorption):** Rolling with a direct concussive explosion or heavy blunt hit to cut damage in half.  
  * **Counter-Grapple Framing:** Posting forearms and hips to prevent an opponent from securing a takedown or chokehold.  
  * **High-Cover Bracing:** Maximizing cover benefits behind low concrete barriers, bulkheads, and sandbag walls.  
  * **Ranged Projectile Weaving:** Moving unpredictably in zig-zag patterns to spoil the aim of marksmen downfield.  
  * **Point-Blank Deflection:** Shunting an attacker's drawn gun barrel or blade tip away from your center of mass in CQB.

### 

### **MELEE**

* **Ability:** Strength (standard) or Agility (with specific Features like *Weapon Finesse*)  
* **Description:** This skill covers proficiency with hand-held weaponry designed for close-quarters combat. It encompasses everything from simple clubs to elegant rapiers. It reflects a character’s ability to strike accurately and forcefully with extended reach or heavy implements.  
* **Game Mechanics:**  
  * **Attack Roll:** d20 \+ Melee Rank \+ Strength Mod vs. Target Defense.  
  * **Damage:** Weapon Damage Dice \+ Strength Mod.  
  * **Reach:** Some melee weapons (Polearms) allow attacks from 10ft away.  
* **Specialties:**  
  * **Daggers & Combat Knives:** Swift, lethal slashes, throat cuts, and reverse-grip thrusts inside grappling range.  
  * **One-Handed Swords:** Fencing with rapiers, sabers, and arming swords utilizing precision point-work and lunges.  
  * **Two-Handed Greatswords:** Sweeping massive, heavy blades across wide arcs to cleave multiple targets and break lines.  
  * **Polearms & Spears:** Utilizing superior 10-foot reach, thrusting points, and hooking blades to keep foes at bay.  
  * **Blunt Staves & Bo Rods:** Executing rapid, spinning two-handed bludgeoning combinations, sweeps, and disarms.  
  * **Battleaxes & Cleavers:** Delivering crushing, chopping blows designed to split wooden shields, limbs, and light armor.  
  * **Warhammers & Maces:** Driving blunt armor-piercing kinetic force directly through heavy steel and plate armor.  
  * **Vibro-Blades:** Wielding high-frequency vibrating knives and swords that saw through metal plate effortlessly.  
  * **Energized Melee Weapons:** Slashing with superheated plasma blades and power-axes that melt armor on contact.  
  * **Flails & Flexible Weapons:** Whipping weighted chains and morningstars around an opponent's raised shield to strike.

### 

### **RANGED**

* **Ability:** Agility  
* **Description:** The skill of using muscle-powered projectile weapons or thrown objects to strike targets at a distance. This differs from *Ballistic* (guns) as it relies more on the user's physical trajectory calculation and force (for thrown items) or mechanical tension (bows).  
* **Game Mechanics:**  
  * **Attack Roll:** d20 \+ Ranged Rank \+ Agility Mod vs. Target Defense.  
  * **Damage:** Weapon Damage Dice (Strength mod is usually added to *Thrown* weapons, but not standard Bows unless they are composite/specialized).  
  * **Range Increment:** Each weapon has a range increment; attacking beyond this imposes a cumulative \-2 penalty per increment.  
* **Specialties:**  
  * **Recurve & Hunting Bows:** Drawing, anchoring, and loosing arrows rapidly at close-to-medium forest targets.  
  * **Heavy Longbows:** Drawing massive 100-pound pull bows to punch arrows through metal armor at extreme distances.  
  * **Compound Tech-Bows:** Utilizing modern pulley cams, stabilizer rods, and fiber-optic sights for pinpoint arrow shots.  
  * **Hand Crossbows:** Firing and rapid-cranking compact, concealable crossbows designed for close ambush work.  
  * **Heavy Siege Crossbows:** Operating foot-stirrup, crank-drawn heavy crossbows that hit with devastating kinetic shock.  
  * **Throwing Knives & Spikes:** Hurling balanced steel blades with precision into eyes, necks, and exposed joints.  
  * **Throwing Axes & Tomahawks:** Lobbing heavy, rotating axes that bury deeply into skull bone and wooden barricades.  
  * **Shuriken & Flechettes:** Flumping spreads of star-blades or balanced throwing darts in rapid succession.  
  * **Slings & Staff Slings:** Whirling lead bullets or river stones to hit targets at range with crushing blunt force.  
  * **Boomerangs & Returning Sticks:** Throwing aerodynamic curved timber sticks that strike hard or loop back if missed.

### 

### **UNARMED**

* **Ability:** Strength (for power/grappling) or Agility (for striking speed/precision)  
* **Description:** The ability to fight without artificial weapons, using only the body. This includes brawling, martial arts, wrestling, and the use of natural weaponry for non-human species.  
* **Game Mechanics:**  
  * **Attack Roll:** d20 \+ Unarmed Rank \+ Str/Agi Mod vs. Target Defense.  
  * **Damage:** Typically lower than weapons (1d4 or varies by size/species) \+ Strength Mod.  
  * **Non-Lethal:** Unarmed attacks can often be chosen to deal Stun/Non-Lethal damage without penalty.  
  * **Grapple:** Unarmed skill is used to initiate and maintain holds (Grapple checks).  
* **Specialties:**  
  * **Boxing & Power Punches:** Throwing crisp jabs, hooks, crosses, and uppercuts with weighted hand wraps or bare fists.  
  * **Kicks & Leg Sweeps:** Snapping roundhouse kicks, front teep kicks, low shin strikes, and sweeping enemy legs.  
  * **Joint Locks & Submissions:** Snapping armbars, kimuras, and heel hooks to break enemy joints and force surrenders.  
  * **Grappling & Throws:** Executing hip throws, double-leg takedowns, and suplexes to slam targets into hard deckplates.  
  * **Claws & Talons (Natural):** Slashing and tearing flesh and armor with native species claws, finger spikes, and talons.  
  * **Biting & Fangs (Natural):** Clamping down with powerful reptilian or feline jaws, injecting venom, or tearing throats.  
  * **Horns & Headbutts (Natural):** Driving heavy head-crest horns or hardened skulls straight into an opponent's face.  
  * **Tail Strikes & Sweeps (Natural):** Whipping heavy muscular tails to trip enemies, strike backs, or shatter ribs.  
  * **Chokeholds & Strangles:** Sinking rear-naked chokes, guillotine holds, and crushing windpipes to knock foes unconscious.  
  * **Nerve Strikes & Pressure Points:** Striking vulnerable nerve clusters to temporarily paralyze limbs and drop guards.

## 

## 

## **MODERN COMBAT SKILLS (Combat)**

These skills govern the use of technologically advanced projectile weaponry, from personal firearms to heavy ordinance.

### 

### **BALLISTIC**

* **Ability:** Agility  
* **Description:** The operation of firearms and other handheld projectile weapons. This skill covers maintaining, reloading, and firing weapons that use combustion or magnetic acceleration to launch solid slugs at high velocity.

* **Mechanics:**  
  * **Standard Attack:** **d20 \+ Skill Rank \+ Agility Mod vs. Target Defense**.  
  * **Burst Fire (Short):** Firing a controlled 3-round burst grants a **\+1 Bonus to Strike**2.

  * **Auto-Fire (Long Burst):** Firing longer bursts (10+ rounds) imposes a **\-1 Recoil Penalty** for every 10 rounds fired. However, for every point the attack roll exceeds the target's Defense, one additional round hits (up to the total shots fired). Each extra hit deals **\+1d damage** (if the initial hit penetrated DR)3.

* **Specialties:**   
  * **Semi-Automatic Pistols:** Double-tapping service handguns rapidly, clearing malfunctions, and drawing from holsters.  
  * **Heavy Revolvers:** Firing high-caliber magnum hand cannons with heavy recoil control and precision shot placement.  
  * **Tactical Shotguns:** Pumping or cycling 12-gauge buckshot spreads and armor-piercing slugs in room clearing.  
  * **Submachine Guns (SMGs):** Controlling high cyclic rates of fire, firing controlled bursts from compact machine pistols.  
  * **Assault Rifles & Carbines:** Engaging point and area targets from 50 to 300 meters using single shots and short bursts.  
  * **Designated Marksman Rifles:** Precision shooting at mid-to-long range using magnified optics and match ammunition.  
  * **Sniper Rifles:** Calculating windage, bullet drop, core Coriolis effect, and firing single-shot extreme range kills.  
  * **Automatic Fire Walking:** Firing long full-auto bursts while walking the muzzle stream onto moving enemy lines.  
  * **Kinetic Anti-Materiel Rifles:** Firing massive .50-caliber or 20mm kinetic rounds to destroy engine blocks and light armor.  
  * **Concealed Carry Quick-Draw:** Clearing clothing, drawing a hidden sub-compact pistol, and firing within one second.

### 

### **HEAVY WEAPONS**

* **Ability:** Agility (or Intellect for calculated trajectories/guided systems)  
* **Description:** The use of large-scale weaponry, often requiring two hands, a tripod, or a vehicle mount. This includes machine guns, rocket launchers, and artillery systems.

* **Mechanics:**  
  * **Launched/Thrown Ordinance:** Grenades and missiles use this skill for targeting.

  * **Area Effect (Blasting):** Attacks target a specific area rather than a single defense score.  
    * **General Area:** Targets within the blast radius take base damage. For every **5 points** the attack roll exceeds a **CR of 10**, the damage increases by **\+2 Dice**.

    * **Focused Blast Zone:** Targets in the center of the blast take **\+100% Damage Dice** if the attack roll exceeds a **CR of 15** by 10 or more.

* **Specialties:**   
  * **Squad Automatic Weapons (SAWs):** Firing belt-fed light machine guns to lay down suppressing fire on enemy tree lines.  
  * **Heavy Mounted Machine Guns:** Operating spade-gripped, tripod-mounted .50-caliber guns against light vehicles and infantry.  
  * **Unguided Rocket Launchers:** Shoulder-firing unguided RPGs and anti-tank rockets into bunkers and tank treads.  
  * **Smart Missile Systems:** Painting targets, locking thermal signatures, and firing fire-and-forget guided missile tubes.  
  * **Rotary Grenade Launchers:** Lobs of 40mm high-explosive, smoke, and tear-gas canisters downfield in rapid succession.  
  * **Vehicle-Mounted Cannons:** Operating auto-cannons (25mm-40mm) mounted in armored vehicle turrets using fire computers.  
  * **Rotary Miniguns:** Spooling electric gatling guns to unleash 4,000 rounds per minute, chewing through light fortifications.  
  * **Man-Portable Mortars:** Dropping indirect-fire mortar shells into tubes, setting baseplates, and adjusting quadrant sights.  
  * **Stationary Defense Emplacements:** Manning fixed bunker machine gun emplacements, managing ammunition belts and barrel swaps.  
  * **Anti-Air Kinetic Flak:** Tracking fast-moving atmospheric aircraft with high-angle, proximity-fused explosive flak cannons.

## 

## 

## **ADVANCED COMBAT SKILLS**

These skills govern the use of high-technology weaponry that utilizes directed energy rather than solid projectiles.

* **Mechanic:** **d20 \+ Skill Rank \+ Agility Modifier vs. Target's Defense**

### 

### **ENERGY**

* **Ability:** Agility  
* **Description:** Proficiency with personal, directed-energy weapons such as pistols and rifles. These weapons discharge various forms of lethal energy, from searing plasma to destabilizing sonic waves. Unlike ballistic weapons, energy weapons often carry secondary effects based on their energy type2222.

* **Mechanics:**  
  * **Attack Roll:** Standard ranged attack roll.  
  * **Critical Effects:** In addition to increased damage, specific energy types apply status effects on a Critical Hit3:

    * *Pyro (Plasma):* Burns target for ½ damage in the following rounds.  
    * *Cryo (Icer):* Freezes target (Slow/Snare effect).  
    * *Voltic (Electricity):* Shocks target (Stun effect).  
    * *Sonic:* Concussive force (Stun effect).  
    * *Corrosive:* Dissolves armor/structure (½ damage for next 2 rounds).  
    * *Grav:* Impact force causing Knockback.  
* **Specialties:**  
  * **Laser Pistols & Carbines:** Firing instant-hit coherent light beams that burn through flesh and thin sheet metal.  
  * **Plasma Rifles:** Hurling bolts of superheated gas that melt through composite plate and ignite secondary fires.  
  * **Particle Beam Carbines:** Firing high-velocity subatomic particle streams that deliver immense penetrating radiation trauma.  
  * **Sonic Stunners & Shatterguns:** Firing concussive acoustic shockwaves that knock targets unconscious or rupture organs.  
  * **Cryo-Projectors:** Spraying endothermic chemical streams that flash-freeze target limbs and shatter armor plates.  
  * **Arc & Voltic Rifles:** Discharging branching lightning bolts that jump across multiple metal-armored targets and stun.  
  * **Corrosive Spray Weapons:** Spraying pressurized acidic chemical streams that dissolve physical armor and melt hulls.  
  * **Grav-Pulse Blasters:** Firing high-density gravitational shockwaves that throw armored targets backward off feet.  
  * **Disruptor & Disintegrator Pistols:** Firing exotic matter-collapsing energy that breaks molecular bonds down to ash.  
  * **Thermal Microwave Lances:** Focusing invisible microwave radiation that cooks organic targets from the inside out behind cover.

### 

### **HEAVY ENERGY**

* **Ability:** Agility (or Intellect if integrated into vehicle/ship gunnery systems)  
* **Description:** The operation of massive, high-yield energy cannons. These weapons are typically vehicle-mounted, ship-mounted, or require a tripod setup due to their size and power consumption. They are designed to destroy armored vehicles, starships, and fortifications4444.

* **Mechanics:**  
  * **Attack Roll:** Standard ranged attack roll, often with range penalties reduced by targeting computers (Smart Links).  
  * **Area of Effect:** Many heavy energy weapons (like Particle Beams) strike in a line or an area blast rather than a single point.  
  * **Penetration:** Heavy energy weapons typically possess high Penetration values, ignoring significant amounts of Damage Resistance (DR).  
* **Specialties:**  
  * **Ship-to-Ship Laser Batteries:** Firing megawatt turrets to slice through starship hull plating and sensor domes.  
  * **Orbital Particle Lances:** Aligning spine-mounted spinal cannons to fire devastating lance strikes through the atmosphere.  
  * **Heavy Plasma Mortars:** Lobbing massive globes of burning thermonuclear plasma onto planetary siege fortresses.  
  * **Singularity Projectors:** Firing unstable microscopic gravitational vortexes that crush tanks and pull down walls.  
  * **Spinal Railguns & Coilguns:** Magnetically accelerating multi-ton slugs to relativistic speeds across solar distances.  
  * **Manned Fortress Energy Turrets:** Manning massive energy gun emplacements on planetary defense shields.  
  * **Tachyon Beam Cannons:** Firing high-energy faster-than-light particles that bypass conventional deflector shields.  
  * **Point-Defense Energy Arrays (PDC):** Tracking and vaporizing incoming torpedoes and starfighters with automated pulse lasers.  
  * **Macro-Ion Cannons:** Firing massive EMP and ion charges that disable starship power grids, engines, and shields without hull breach.  
  * **Coaxial Plasma Bombardment Cannons:** Directing planetary bombardment salvos from orbit to obliterate coastal cities.

# **\- METAPHYSICS & DISCIPLINES**

# **METAPHYSICS & DISCIPLINES**

**The Tangent System Framework**  
In the Tangent universe, "Meta" refers to any ability that defies the standard laws of physics—whether it be Arcane Magic, Psionics, Divine Intervention, or Super-Science. Regardless of the source (The Void, The Weave, The Quantum Field), the mechanics of harnessing this power remain consistent.

## 

## **CORE MECHANICS**

The Metaphysic system relies on a triad of skills: **Attune**, **Discipline**, and **Invocation**.

### 

### **ATTUNE (The Channel)**

Attribute: Varies (Intellect for Mages/Tech, Wisdom for Priests/Psychics, Charisma for Sorcerers/Fey)  
Type: Meta Skill  
Attune represents the character's ability to open a conduit to their power source and shape it safely. It is the "Accuracy" and "Control" stat of the meta system.

* **Setting the Save DC:** When you cast an Invocation that allows a Resistance Check (Reflex, Fortitude, or Will), the DC is determined by your Attune check.  
  * *DC Formula:* d20 \+ Attune Rank \+ Ability Mod  
  * *Passive DC:* 10 \+ Attune Rank \+ Ability Mod (Used when taking 10\)  
* **Overcoming Resistance:** If a target has Spell Resistance (SR) or Psionic Dampening, the Attune check must exceed the target's SR.  
* **Maintaining Focus:** Attune is used for Concentration checks when taking damage while sustaining an effect.  
* **Specialties:**  
  * **Conduit Channeling:** Opening your physical body and neural pathways to flow raw meta-energy without self-damage.  
  * **Concentration Under Stress:** Sustaining an active invocation while taking direct ballistic damage or running through fire.  
  * **Spell DC Augmentation:** Tightening the metaphysical weave to make your invocations vastly harder for targets to resist.  
  * **Overcoming Resistance (SR/PR):** Pushing raw energy through an enemy's innate spell resistance or psionic dampeners.  
  * **Runic Circle Scribing:** Drawing physical geometric runes on floors to anchor massive, long-duration rituals safely.  
  * **Metascience Calibration:** Tuning sensory implants and detection crystals to measure ambient magical currents accurately.  
  * **Counter-Magic Shaping:** Reading an enemy's incoming energy conduit and weaving an opposite wave to collapse their spell.  
  * **Ambient Energy Absorption:** Siphoning background radiation, ley line energy, or emotional resonance to fuel casting.  
  * **Reality Anchor Stabilization:** Holding your immediate 10-foot space rigid against planar incursions and reality warping.  
  * **Overcharging Invocations:** Pushing the conduit beyond safe redline limits to maximize damage dice at the risk of personal burnout.

### 

### **DISCIPLINE SKILLS (The Shape)**

Attribute: Varies (Key Ability)  
Type: Meta Skill  
Disciplines are broad categories of reality manipulation. A character must be **Awakened** (via Feature) to learn these. The Rank in a Discipline determines the **Scope** of the effect (Range, Area, Duration) and the "Stage" of power.  
**Skill Stages & Base Potency:**

* **Novice (Ranks 1-5):** Touch to Short Range.  
* **Trained (Ranks 6-10):** Medium Range. Gain \+1d6 Base Effect.  
* **Expert (Ranks 11-15):** Long Range. Gain \+2d6 Base Effect.  
* **Master (Ranks 16-19):** Extreme Range. Gain \+3d6 Base Effect.  
* **Pinnacle (Rank 20):** Line of Sight/Global. Gain \+4d6 Base Effect.

### 

### **INVOCATIONS (The Effect)**

Type: Specialization / Feature  
Invocations are specific "spells" or "techniques" learned within a Discipline.

* **Cost:** 1 Character Point (CP) per Rank.  
* **Potency:** Invocations add **1 die (usually d6) per Rank** to the effect (Damage, Healing, Shielding).  
* **Total Effect:** Base Discipline Stage Dice \+ Invocation Rank Dice.

# 

# 

# **THE DISCIPLINES**

## 

## **DIMENSION**

Focuses: Summoning, Teleportation  
Key Concept: The manipulation of space, time, and planar boundaries.

### 

### **SUMMONING FOCUS**

Calling forth creatures or objects from other coordinates or planes.

* **Mechanic:** Summoned items/creatures appear within Range.  
* **Duration:** 1 Round per Discipline Rank (Sustained). Spending 1 Karma extends this to 1 Minute per Rank.

**Invocations:**

* **Summon Creature:** Calls a creature to fight or aid.  
  * *Effect:* Summons a creature with a Build Point (BP) value equal to 10 \+ (Invocation Rank x 5).  
* **Create Object:** Manifests a temporary item from generic matter.  
  * *Effect:* Creates an object of Simple complexity (CR 0). Rank increases complexity/Tech Level.  
* **Banishment:** Forces a creature back to its home plane or a pocket dimension.  
  * *Effect:* Resisted by Will. On fail, target is removed for 1 round per Rank.  
* **Web:** Pulls strands of sticky dimensional matter to restrain foes.  
  * *Effect:* Area Entangle. Resisted by Reflex/Strength.

### 

### **TELEPORT FOCUS**

Relocation without traversing the intervening space.

* **Mechanic:** Difficulty is based on the Familiarity of the target location.  
* **Familiarity DC:** Very Familiar (5), Seen Once (15), Description Only (25), Blind (30).

**Invocations:**

* **Blink:** Short-range tactical hop.  
  * *Effect:* Move up to 10ft \+ (5ft per Rank) as a Move Action. Does not provoke Attacks of Opportunity.  
* **Gate:** Opens a stable portal for others to pass through.  
  * *Effect:* Sustained portal. Size 5ft x 5ft per Rank.  
* **Teleport Other:** Sends a target to another location.  
  * *Effect:* Touch attack to hit. Resisted by Will. Range determined by Discipline Rank.  
* **Spatial Distortion:** Warps space to increase distance/defense.  
  * *Effect:* Adds \+1 Defense per 2 Ranks against Ranged attacks (displacement).

## 

## 

## **ENERGY**

Focuses: Elemental, Force  
Key Concept: Manipulation of raw energy states and kinetic forces.

### 

### **ELEMENTAL FOCUS**

Conjuration of Pyro (Fire), Cryo (Ice), Voltic (Electric), Sonic, or Corrosive energy.

* **Damage:** 1d6 per Discipline Stage \+ 1d6 per Invocation Rank.  
* **Critical Effects:**  
  * *Pyro:* Burn (1/2 damage next round).  
  * *Cryo:* Freeze (Slow/Snare).  
  * *Voltic:* Shock (Stun).  
  * *Sonic:* Deafen/Stun.  
  * *Corrosive:* Dissolve (Armor DR reduction).

**Invocations:**

* **Blast:** Single target ranged attack.  
  * *Effect:* Standard Damage. High Accuracy (+2 to Attune check).  
* **Burst:** Radius explosion.  
  * *Effect:* Standard Damage in 5ft radius per Rank. Reflex Save for half.  
* **Line/Bolt:** Piercing line attack.  
  * *Effect:* Standard Damage in a 50ft \+ (10ft/Rank) line.  
* **Barrier:** Wall of damaging energy.  
  * *Effect:* Wall 10ft long per Rank. Deals damage to those crossing it.

### 

### **FORCE FOCUS**

Manipulation of pure kinetic energy and invisible constructs.

* **Damage:** 1d8 per Discipline Stage \+ 1d8 per Invocation Rank (Force Damage).

**Invocations:**

* **Telekinesis:** Move objects with the mind.  
  * *Effect:* Move weight 25lbs x Rank.  
* **Force Missile:** Homing shards of force.  
  * *Effect:* Always hits (no Attack roll needed). Lower damage die (d4s).  
* **Force Field:** Personal protection.  
  * *Effect:* Grants Temporary HP equal to 5 x Rank.  
* **Force Wave:** Cone of concussive force.  
  * *Effect:* Knocks targets Prone. Resisted by Fortitude/Might.

## 

## 

## **ENTROPY**

Focuses: Chaos, Order  
Key Concept: The cycle of decay and structure; Life and Death energy.

### 

### **CHAOS FOCUS**

Acceleration of decay, luck manipulation (bad luck), and negative energy.

* **Damage:** 2d6 per Discipline Stage \+ 1d6 per Invocation Rank. (High damage, but dangerous).  
* **Note:** Ignores hardness/DR of non-magical materials.

**Invocations:**

* **Wither/Touch of Death:** Melee touch attack.  
  * *Effect:* Necrotic damage. Heals Undead.  
* **Curse:** Inflicts Disadvantage.  
  * *Effect:* Target rolls at Disadvantage for 1 round per Rank. Resisted by Will.  
* **Corrosion:** Destroys physical armor.  
  * *Effect:* Reduces target DR by 1 per Rank.  
* **Fear:** Induces panic.  
  * *Effect:* Target flees for 1 round per Rank. Resisted by Will.

### 

### **ORDER FOCUS**

Restoring patterns, healing, and stabilization.

* **Healing:** 2d6 per Discipline Stage \+ 1d6 per Invocation Rank.

**Invocations:**

* **Cure:** Restores Health/Vitality.  
  * *Effect:* Touch range healing. Undead take damage.  
* **Stabilize:** Prevents death.  
  * *Effect:* Immediate Action. Target stops losing Health at Death's Door.  
* **Restoration:** Removes status effects (Poison, Disease, Stun).  
  * *Effect:* Difficulty based on the Severity of the ailment.  
* **Ward:** Protection against specific alignments/energy.  
  * *Effect:* \+2 Defense and Saves vs chosen source.

## 

## 

## **ILLUSION**

Focuses: Phantasm, Shadow  
Key Concept: Deception of the senses and the creation of quasi-real matter.

### 

### **PHANTASM FOCUS**

Pure sensory input. Only exists in the mind or as light/sound waves.

* **Mechanic:** Disbelief (Will Save) negates the effect entirely.

**Invocations:**

* **Invisibility:** Bends light around a subject.  
  * *Effect:* Subject is invisible. Attacking breaks effect. 1 min/Rank.  
* **Image:** Creates a visual/auditory scene.  
  * *Effect:* Stationary or scripted illusion. Complexity scales with Rank.  
* **Silence:** Dampens sound waves.  
  * *Effect:* Radius 10ft \+ 5ft/Rank. No sound can pass in or out.  
* **Disguise Self:** Alters perceived appearance.  
  * *Effect:* \+10 to Disguise checks.

### 

### **SHADOW FOCUS**

Infusing illusions with quasi-reality from the Shadow Plane.

* **Mechanic:** Even if disbelieved, Shadow illusions retain 20% reality (and damage).

**Invocations:**

* **Shadow Conjuration:** Mimics a Summoning or Evocation spell.  
  * *Effect:* Duplicates another spell at 50% power.  
* **Shadow Weapon:** Creates a semi-real weapon.  
  * *Effect:* Functions as a specific melee weapon. Deals Psychic/Cold damage.  
* **Obscuring Mist:** Creates heavy fog.  
  * *Effect:* Blocks Line of Sight. Can be solid/sticky (Shadow).

## 

## 

## **MATTER**

Focuses: Enhancement, Transmutation  
Key Concept: Altering the physical properties of existing matter.

### 

### **ENHANCEMENT FOCUS**

Improving the structural integrity or properties of an object or creature.

* **Mechanic:** Buffs and physical upgrades.

**Invocations:**

* **Hardening/Barkskin:** Toughens skin or armor.  
  * *Effect:* Adds \+1 DR per 3 Ranks to armor or natural skin.  
* **Keen Edge:** Sharpens a weapon.  
  * *Effect:* Increases Critical Threat range by 1\.  
* **Physical Boost:** Enhances physical attributes.  
  * *Effect:* Grants \+2 to Str, Dex, or Con for 1 min/Rank.  
* **Repair:** Fixes structural damage.  
  * *Effect:* Restores 1d10 Structure Points per Rank to objects/constructs.

### 

### **TRANSMUTATION FOCUS**

Changing one substance into another or altering state of matter.

* **Mechanic:** Permanent changes usually require high Ranks and Karma expenditure. Combat use is temporary.

**Invocations:**

* **Polymorph/Morph:** Changes biological form.  
  * *Effect:* Subject takes on physical stats of a Beast/Humanoid.  
* **Petrification:** Turns flesh to stone (or vice versa).  
  * *Effect:* Resisted by Fortitude. "Slow" effect on partial success.  
* **Liquid/Gas Form:** Alters caster's state.  
  * *Effect:* Can pass through cracks, resistant to physical damage.  
* **Matter Manipulation:** Shape stone, metal, or wood.  
  * *Effect:* Reshape 1 cubic foot per Rank of raw material.

## 

## 

## **MENTAL**

Focuses: Projection, Sense  
Key Concept: Telepathy, ESP, and mind-over-matter interactions.

### 

### **PROJECTION FOCUS**

Sending thoughts, commands, or psychic damage into another mind.

* **Range:** Extended. 100ft x Rank.

**Invocations:**

* **Telepathy:** Mental communication.  
  * *Effect:* communicate with any language-using creature.  
* **Domination/Command:** Force will upon another.  
  * *Effect:* Target obeys command. Resisted by Will.  
* **Mind Blast:** Overload neurons.  
  * *Effect:* Stun damage. Resisted by Will. 1d6 Psychic damage per Rank.  
* **Memory Edit:** Modify recollections.  
  * *Effect:* Alter last 1 minute per Rank of memories.

### 

### **SENSE FOCUS**

Receiving information from the universe or other minds.

* **Range:** Extended. 100ft x Rank.

**Invocations:**

* **Clairvoyance/Remote Viewing:** See/Hear at a distance.  
  * *Effect:* Project senses to a known location or person.  
* **Precognition:** Glimpse the future.  
  * *Effect:* Gain Advantage on Initiative or a specific Defense check.  
* **Mind Read:** Detect surface thoughts or deep probe.  
  * *Effect:* Resisted by Will.  
* **Detection:** Sense specific signatures (Life, Magic, Tech).  
  * *Effect:* Ping location of target type within range.

# 

# 

# **SKILL USE & MODIFIERS**

### **Focused Task (Concentration)**

Meta users can choose to **Focus** on their invocation.

* **Benefit:** \+2 to Attune/Discipline Check.  
* **Cost:** \-5 to Defense and Perception. Must remain stationary.

### 

### **Rushed vs. Meticulous Casting**

* **Rushed (Swift Action):** \-5 to Attune Check.  
* **Meticulous (Full Round \+):** \+2 to Attune Check per additional round spent channeling (Max \+6).

### 

### **Skill Synergy**

Using a complementary skill to boost the Meta effect.

* *Example:* Using **Anatomy (Knowledge)** to boost a **Wither (Entropy)** spell.  
* *Mechanic:* Roll Anatomy vs DC 10\. For every 5 points over 10, gain \+1 to the Attune/Damage roll.

# **SKILL GROUPS & TITLES**

# **SKILL GROUPS & TITLES**

Skills are categorized into five groups: Physical Skills, Mental Skills (further divided into Knowledge and Vocation categories), Social Skills (including Manipulation and Expression categories), Metafocus Skills (encompassing Metafocus Disciplines, Attune and Special Abilities), and Combat Skills (unarmed combat, defense and all weapons training).  
Depending on the skills level with specializations there are ranks, each with a unique title. These titles serve to highlight the character's proficiency and offer additional advantages.

## **Untrained (Skill Rank 0\)**

This represents the base level for a character with no formal training or practical experience in a particular skill. Their attempts to utilize the skill will be clumsy, inefficient, and often unsuccessful. This lack of expertise can manifest in various ways depending on the skill in question.

### **Physical Skills (Athletics, Acrobatics)**

An untrained character might struggle with basic maneuvers, be prone to stumbling or falling, and lack the finesse or technique to perform complex actions effectively.

### **Mental Skills (Medicine, Investigation)**

An untrained character might lack the knowledge or analytical ability to solve problems, draw accurate conclusions, or understand complex information related to the skill.

### **Social Skills (Persuasion, Deception)**

An untrained character might be awkward, easily flustered, or transparent in their attempts to influence or deceive others.

### **Technical Skills (Computers, Engineering)**

An untrained character might be unable to operate or repair even basic devices, and their attempts to interact with complex systems could lead to disastrous outcomes.

### **Combat Skills (Melee Weapons, Firearms)**

An untrained character might be clumsy and ineffective in combat, lacking the precision, timing, and tactical awareness to utilize weapons or defend themselves effectively.

*Overall, the "Untrained" skill level represents a starting point for characters in Tangent. It highlights the importance of skill development and encourages players to think creatively about how their characters can overcome challenges despite their initial lack of expertise.*

## **Novice (Skill Rank 1-5)**

A character at the Novice level possesses a foundational understanding of the skill. They have grasped the basic concepts and techniques, but their execution is still unrefined and inefficient. Think of them as an apprentice learning the ropes. They may be slow to start a task, require more time to complete it, and their work might lack the polish of a more experienced individual. However, they are eager to learn and improve. With practice and guidance, they can progress to higher levels of proficiency.

### **Physical Skills**

A novice swordsman might know the basic stances and strikes but lack the speed and precision of a trained warrior. A novice climber might be able to scale simple walls but struggle with more challenging terrain.

### **Mental Skills**

A novice medic might understand basic anatomy and first aid but lack the experience to diagnose complex illnesses or perform intricate procedures. A novice investigator might be able to gather basic clues but struggle to piece together complex mysteries.

### **Social Skills**

A novice diplomat might know the basic etiquette and protocols of negotiation but lack the finesse and subtlety to navigate complex political situations. A novice merchant might be able to haggle for simple deals but struggle to negotiate favorable terms in a competitive market.

### **Technical Skills**

A novice mechanic might be able to perform basic repairs and maintenance but lack the knowledge to troubleshoot complex malfunctions or modify existing systems. A novice computer programmer might be able to write simple code but struggle with more advanced algorithms or software architecture.

### **Combat Skills**

A novice archer might be able to hit stationary targets but struggle to track moving targets or adjust for wind and distance. A novice pilot might be able to fly a basic aircraft but struggle with advanced maneuvers or aerial combat.

*While Novice characters may not be the most skilled or efficient, they offer significant potential for growth and development. With experience and training, they can hone their abilities and progress to higher skill levels, becoming valuable assets to their team and the world around them.*

## **Trained (Skill Rank 6-10)**

At this level, the character demonstrates professional competence in the skill. They have a solid grasp of the fundamental techniques and can apply them effectively in various situations. Their work is of good quality, meeting the standards expected of a trained professional. They are reliable and efficient, able to complete tasks in a timely manner with minimal supervision.

### **Physical Skills**

A trained swordsman can execute complex maneuvers with speed and precision, adapting their fighting style to different opponents and situations. A trained climber can navigate challenging terrain with confidence, utilizing specialized equipment and techniques to overcome obstacles.

### **Mental Skills**

A trained medic can diagnose and treat a wide range of illnesses and injuries, utilizing their knowledge of anatomy, physiology, and pharmacology to provide effective care. A trained investigator can analyze complex crime scenes, identify patterns and inconsistencies, and draw accurate conclusions based on evidence.

### **Social Skills**

A trained diplomat can navigate complex social and political situations with grace and tact, utilizing their understanding of cultural norms, power dynamics, and negotiation strategies to achieve favorable outcomes. A trained merchant can negotiate complex deals, assess market trends, and build profitable relationships with suppliers and customers.

### **Technical Skills**

A trained mechanic can troubleshoot and repair complex machinery, utilizing diagnostic tools and their understanding of engineering principles to identify and resolve problems. A trained computer programmer can design and implement sophisticated software solutions, utilizing their knowledge of programming languages, algorithms, and data structures.

### **Combat Skills**

A trained archer can hit moving targets with accuracy, adjusting for distance, wind, and other environmental factors. A trained pilot can execute advanced maneuvers and engage in aerial combat with skill and confidence.

*Characters at the Trained skill level are valuable assets in any team or organization. They possess the knowledge, experience, and reliability to contribute effectively to a wide range of tasks and challenges. While they may not be masters of their craft, they are respected professionals who can be counted on to get the job done.*

## **Expert (Skill Rank 11-15)**

Characters who reach the Expert level in a skill are exceptionally proficient and knowledgeable. They can perform tasks with remarkable speed and efficiency, often completing them in a fraction of the time it would take someone less skilled. This level of expertise allows them to take on more complex and challenging tasks with confidence.

### **Skillful Professional**

One of the key benefits of being an Expert is the reduced penalty for rushing. In Tangent, when a character tries to complete a task quickly, they usually incur a penalty to their skill check, reflecting the increased risk of making mistakes. However, Experts have honed their skills to the point where they can work rapidly without sacrificing too much accuracy or precision. This means they can take swift action in time-sensitive situations or accomplish more in a shorter amount of time.   
Rush Penalty is reduced by 1 point per skill stage (-3 at Expert, \-4 at Master and \-5 at Pinnacle).

### **Physical Skills**

An expert swordsman can perform complex maneuvers flawlessly, adapting their fighting style on the fly to counter their opponent's moves. An expert climber can ascend a sheer cliff face with incredible speed and agility, using their knowledge of the rock and their own body to find the most efficient route.

### **Mental Skills**

An expert medic can quickly diagnose and treat even the most complex injuries and illnesses, drawing upon their vast knowledge and experience to make life-saving decisions. An expert investigator can analyze a crime scene in minutes, piecing together seemingly unrelated clues to form a coherent picture of the events.

### **Social Skills**

An expert diplomat can defuse tense negotiations, build rapport with even the most hostile individuals, and find creative solutions to seemingly intractable problems. An expert merchant can anticipate market fluctuations, negotiate complex trade deals, and build a vast network of contacts and allies.

### **Technical Skills**

An expert mechanic can disassemble and reassemble complex machinery blindfolded, diagnose and repair malfunctions with minimal tools, and even design and build their own custom devices. An expert computer programmer can write and debug code at lightning speed, optimize algorithms for maximum efficiency, and even develop their own programming languages.

### **Combat Skills**

An expert archer can hit multiple targets with pinpoint accuracy, even while moving or under duress. An expert pilot can perform breathtaking aerial maneuvers, outmaneuvering their opponents and pushing their aircraft to the limits of its capabilities.

*Characters at the Expert skill level are highly sought-after for their exceptional abilities. They are often leaders, innovators, and problem-solvers who can be relied upon to handle even the most difficult and demanding tasks. Their expertise not only makes them valuable assets but also inspires and motivates those around them.*

## **Master (Skill Rank 16-20)**

A character who attains the Master level in a skill is at the pinnacle of their profession. They possess an unparalleled understanding of the skill, its nuances, and its underlying principles. Their work is not only of exceptional quality but also bears the mark of their unique style and expertise.  They have a deep intuition for the task at hand, allowing them to anticipate challenges, identify creative solutions, and achieve results that others would deem impossible.

### **Aesthetics** 

One of the key benefits of being a Master is the increased bonus for meticulous work. In Tangent, when a character takes their time and focuses on precision and detail, they can gain a bonus to their skill check, reflecting the improved quality of their work. Masters, with their exceptional skill and experience, receive an even greater bonus for meticulous work, allowing them to produce truly outstanding results.   
Meticulous bonus increased by 1 point per skill stage (+3 at Expert, \+4 at Master and \+5 at Pinnacle).

### **Physical Skills**

A master swordsman moves with effortless grace and precision, their blade a blur as they anticipate and counter their opponent's every move. A master climber dances across impossible terrain, defying gravity and finding holds where none seem to exist.

### **Mental Skills**

A master medic possesses an almost intuitive understanding of the human body, able to diagnose rare diseases with a single touch and perform complex surgeries with unparalleled skill. A master investigator can unravel the most intricate mysteries, seeing patterns and connections that others miss and reconstructing events with uncanny accuracy.

### **Social Skills**

A master diplomat can broker peace between warring nations, charm even the most hardened cynics, and inspire loyalty and devotion in their followers. A master merchant can manipulate markets, forge alliances with powerful figures, and amass vast wealth and influence.

### **Technical Skills**

A master mechanic can build complex machines from scrap, repair ancient artifacts with modern tools, and even create entirely new technologies that push the boundaries of what is possible. A master computer programmer can write code that is both elegant and efficient, design complex software architectures, and even develop artificial intelligences that rival the human mind.

### **Combat Skills**

A master archer can hit a target from a mile away, split an arrow in mid-flight, and even shoot around corners with uncanny accuracy. A master pilot can perform seemingly impossible feats of aerial acrobatics, outmaneuvering entire squadrons and emerging unscathed from the most dangerous dogfights.

*Characters at the Master skill level are legendary figures, renowned for their exceptional abilities and accomplishments. They are often sought out as mentors, advisors, and leaders, and their expertise can shape the course of history. Their mastery not only inspires awe and admiration but also pushes the boundaries of what is possible within their chosen field.*

## **Grand Master (Skill Rank 21-25)**

A Grand Master in Tangent represents an exceptional level of skill mastery, surpassing even the impressive capabilities of a Master. These individuals are true virtuosos, possessing an almost intuitive understanding of their chosen skill and its underlying principles. Their expertise allows them to perform complex techniques and maneuvers with remarkable ease and precision, often appearing effortless to those less skilled.

### **Virtuoso**

May choose to take 10 in any situation. This likely guarantees success, even in the face of extremely difficult tasks or overwhelming odds.Their mastery is so complete that failure is almost unthinkable. They can perform their chosen skill specialization with effortless grace, precision, and creativity, achieving results that others can only dream of.

### **Physical Skills**

A grand master athlete exhibits superhuman strength, speed, and endurance, effortlessly conquering physical challenges. A grand master acrobat displays breathtaking grace and agility, defying gravity and performing impossible feats of balance and movement.

### **Mental Skills**

Medical grand masters intuitively understand the human body, diagnose rare diseases, perform complex surgeries, and develop groundbreaking treatments. Investigative grand masters excel at observation, analysis, and deduction, solving mysteries and bringing criminals to justice.

### **Social Skills**

A grand master of persuasion can sway anyone to their view, reading people, understanding their motivations, and tailoring arguments to their desires and fears. A grand master of deception can seamlessly blend into any social situation, manipulate others without their knowledge, and achieve their goals undetected.

### **Technical Skills**

A grand master hacker commands computer systems and networks, manipulating data and creating artificial intelligences with precision. A grand master engineer excels in design, construction, and innovation, pushing engineering boundaries with groundbreaking technologies.

### **Combat Skills**

Grand master swordsmen are highly skilled, can anticipate and counter opponents' attacks, disarm, and deliver lethal strikes. Grand master marksmen can hit targets at incredible distances, compensate for environmental factors, and shoot multiple targets in rapid succession.

## **Pinnacle (Skill Rank 26-30)**

This represents the absolute zenith of skill mastery and specialization, a level of expertise so rare and exceptional that it borders on the superhuman. Characters who achieve Pinnacle rank in a skill are considered living legends, their abilities the stuff of myths and stories. They possess an intuitive understanding of their craft, able to perform feats that seem impossible to others. With this level of Skill Metaphysic Special Abilities will likely be used to help define their capabilities. 

### **Advantage**

May choose to roll with advantage, meaning they roll twice and take the higher result. This significantly increases their chances of success, especially in high-stakes or challenging situations.

### **Physical Skills**

A pinnacle swordsman can deflect bullets with their blade, strike multiple opponents in the blink of an eye, and even anticipate attacks before they happen. A pinnacle climber can scale sheer cliffs without ropes or equipment, leaping impossible distances and finding footholds where none seem to exist.

### **Mental Skills**

A pinnacle medic can heal grievous wounds with a touch, diagnose rare diseases with a glance, and even extend the lifespan of the dying. A pinnacle investigator can solve the most baffling mysteries, uncovering hidden truths and exposing the most cunning criminals.

### **Social Skills**

A pinnacle diplomat can negotiate peace between bitter enemies, inspire entire nations to follow their vision, and manipulate events on a global scale. A pinnacle merchant can build a financial empire, control entire industries, and influence the fate of worlds.

### **Technical Skills**

A pinnacle mechanic can create devices that defy the laws of physics, repair ancient artifacts with futuristic technology, and even build machines that can think and feel. A pinnacle computer programmer can hack into the most secure systems, create artificial intelligences that exceed human capabilities, and with metaphysics even rewrite the fundamental code of reality.

### **Combat Skills**

A pinnacle archer can shoot a target through a keyhole from a hundred yards away, hit multiple targets with a single arrow, seemingly guide their arrows with their mind. A pinnacle pilot can perform impossible aerial maneuvers, dodge enemy fire with preternatural reflexes, and even fly through the heart of a storm unscathed.

*Pinnacle characters are rare and exceptional individuals who have dedicated their lives to mastering their chosen skill. They are the best of the best, and their abilities can change the world.*

# **SKILL SPECIALIZATION**

# **SKILL SPECIALIZATION**

Skill Specialization in Tangent enables players to enhance their characters' expertise within a specific skill. This feature allows characters to gain additional ranks in a specialized area of the skill, representing a deeper understanding and mastery of certain techniques or aspects. By choosing Skill Specialization, players can create more unique and specialized characters that better reflect their individual talents and interests. Additionally, it adds a layer of tactical depth to skill use, as players must carefully consider when to leverage their specialization and when to rely on their broader skill set.

## **Choosing a Specialization**

When a character takes the Skill Specialization feature, they must choose a specific area of focus within the skill. For example, a character with the "Melee Weapons" skill might specialize in "Swordfighting" or "Polearms," while a character with the "Medicine" skill might specialize in "Surgery" or "Herbalism."

## **Bonus Ranks**

The character gains a number of bonus ranks in their chosen specialization, equal to their current rank in the specialization. These bonus ranks can only be used for specialization related checks.

## **Limited Scope**

While Skill Specialization provides a significant advantage in the chosen area, it also narrows the character's focus.

## **Prerequisite**

To acquire a Skill Specialization, the character must already have a rank of 6 or higher (Trained level) in the chosen skill. This ensures a foundation of knowledge before specializing.

## **Cost**

Acquiring a Skill Specialization requires spending Build Points (BP), similar to other features in the system.

## **Independent Level**

The level of Specialization advances separately from the base skill rank. This means a character can continue to improve their specialization even if they don't invest further BP into increasing the overall skill rank.

## **Maximum Level**

Specializations have a maximum level of 10\. This cap ensures that even specialized characters retain some limitations and encourages diversity in skill development.

## **Bonus**

Adds the Specialization level to the character's skill check modifier. Providing greater benefits as the character becomes more specialized.

## **Exceeding the Maximum**

By adding the Specialization bonus to the base skill rank and other modifiers, a character can effectively exceed the normal maximum skill rank of 20\. This enables exceptional mastery in a chosen area and possibly reaching Pinnacle ranking of a skill.

## **Limited Use**

Only one Specialization can be applied to a single skill check. This prevents characters from stacking multiple specializations for an overwhelming advantage. However, Skill Synergy can still be used to combine different specializations for a more nuanced approach.

*Example*  
*A character with a rank of 8 in the "Computer" skill might acquire a Skill Specialization in "Hacking." This would represent their focused expertise in breaking into computer systems and bypassing security measures. The Specialization bonus would enhance their effectiveness in hacking-related skill checks, allowing them to attempt more complex tasks and potentially outperforming other characters with higher Computer skill ranks but lacking this specialization.*

# **LEARNING**

# **LEARNING**

The characters possess the ability to acquire new skills and features through dedicated study and practice. The learning process is designed to accommodate a range of preferences and learning styles.

For those who prefer self-directed education, Tangent offers a wealth of resources and materials to facilitate independent learning. Characters can immerse themselves in books, ancient scrolls, or online platforms to gain knowledge on specific subjects. This approach allows for a flexible learning pace and the freedom to explore areas of personal interest.

Alternatively, characters can seek guidance from experienced instructors who have mastered their chosen disciplines. These mentors provide personalized instruction, offering insights, techniques, and practical exercises to accelerate the learning process. Under the tutelage of these skilled masters, characters can refine their abilities and gain a deeper understanding of the subject matter.

The learning process in Tangent is not confined to formal lessons or structured classes. Characters can engage in hands-on activities, real-world projects, and collaborative endeavors to reinforce their knowledge and develop practical applications. This approach fosters a sense of curiosity and experimentation, encouraging characters to push their boundaries and explore innovative solutions.

Moreover, the learning process in Tangent is tailored to the individual needs and aspirations of each character. Whether they seek to enhance their combat abilities, hone their magical prowess, or delve into the intricacies of crafting and alchemy, the world of Tangent offers endless opportunities for growth and self-discovery.

As characters progress on their learning journey, they not only acquire new skills and features but also gain a sense of accomplishment and personal empowerment. The act of learning becomes a transformative experience, shaping their identities, motivations, and relationships within the Tangent universe. 

## **Study Period**

### **Time Frame**

To improve a skill, a character must commit to a study period. The length of this period depends on the desired skill level:

#### **Existing Skill**

1 day per skill level to gain a point.

#### **New Skill**

1d4+1 days to gain the first skill point.

## **Skill Check**

At the end of the study period, an Easy (CR 10\) Intellect check is made. Success indicates a skill point gain of 1 or 0.5 points, depending on the learning method.

### **Learning Methods**

Self-Taught  
Characters who learn independently gain 1 skill point per 2 study periods (0.5 points per period). This method is slower and will have a considerable amount of hands-on experience but doesn't require external resources.

### **Learning from Another**

Characters can learn from an instructor who is at least Trained (skill rank 6+) in the desired skill. This method provides a learning check bonus of \+1 per skill stage of the instructor and allows for gaining 1 skill point per study period.

### **Study Aids**

Manuals, videos, and other instructional materials can grant a bonus to the study period check, depending on the creator's competence (from \+1 for Novice to \+5 for Pinnacle).

### **Study Partners/Groups**

Learning with others can be beneficial. All members make an Easy Intellect check to stay on task. Each success grants a \+1 bonus to the study period check for all members and counts as "Learning from Another." Even group members who fail the task check still gain half the study period point.

## **Learning New Features**

### **Time**

Learning a new feature takes longer than learning a skill. It requires a week per study period when learning from another, and at least double that time if self-taught (if possible, depending on the feature).

### **Other Methods**

In some cases, features might be learned through alternative methods, such as completing specific quests, undergoing special training, or experiencing transformative events.

## **Additional Notes**

### **Technology**

Advanced technology, such as skill chips or virtual training programs, can significantly accelerate the learning process and provide additional benefits.

### **Character Development**

The learning mechanics encourage players to think about how their characters acquire new skills and features, adding depth and realism to their development.

### **GM Flexibility**

The GM has the final say on the availability of instructors, study materials, and the specific requirements for learning certain skills or features

# **SKILL CHECKS**

# **SKILL CHECKS**

In Tangent, when a character takes an action that relies on a particular skill, they must make a Skill Check to determine their success. This involves a simple but versatile process that takes into account both the character's competence in the skill and the difficulty of the task at hand. 

## **Adding Skill Rank**

The character's Skill Rank in the relevant skill is added to the d20 roll. This represents their level of training, experience, and expertise in that skill. A higher Skill Rank provides a larger bonus, increasing the character's chances of success.

## **Adding Ability Score**

The Ability Score associated with the skill is also added to the total. This reflects the character's natural aptitude and talent in the area covered by the skill. For example, a character with a high Dexterity score would have a bonus to their Lockpicking skill checks, as Dexterity governs fine motor skills and agility.

## **Adding Other Modifiers**

The GM may apply additional modifiers to the skill check based on various situational factors, such as:  
Environmental Conditions: Difficult terrain, poor lighting, extreme weather, or other environmental factors can make a task more challenging, resulting in a penalty to the skill check.

## **Tools and Equipment**

The use of specialized tools or equipment can sometimes provide a bonus to the skill check. For example, using a crowbar might grant a bonus to a Strength check for forcing open a door.

## **Assistance or Hindrance**

Other characters can help or hinder the character attempting the skill check, leading to bonuses or penalties.

## **Skill Synergy**

Using a different but related skill to aid in a check.

## **Special Abilities or Features**

Some characters might have special abilities or features that grant them bonuses or impose penalties on specific skill checks.

## **Rolling a d20**

The core mechanic of a Skill Check involves rolling a 20-sided die (d20). This introduces an element of chance, reflecting the unpredictability of real-world actions and the possibility of unexpected successes or failures.

### **ARBITRATION OF REALITY (SETTING DIFFICULTY)**

Reference: 01.08.06 Skill Use To adjudicate actions, the GM sets a Challenge Rating (CR) or Difficulty Class (DC).

* Simple (DC 0): Routine actions (Walking, breathing). Automatic Success.  
* Very Easy (DC 5): Minimal chance of failure (Climbing a ladder).  
* Easy (DC 10): Standard competency (Driving in traffic).  
* Average (DC 15): Professional standard (Repairing a common device). The baseline for "Trained" individuals.  
* Difficult (DC 20): Expert level (Hacking a secure server).  
* Very Difficult (DC 25): Master level (Tracking across stone).  
* Nearly Impossible (DC 30): Heroic feat (Leaping a canyon).  
* Miraculous (DC 35+): Defying probability (Persuading a zealot).  
  


  ### **The Time Factor (Adjudicating Patience vs. Haste): Reference: 01.08.09 Time**

* **Meticulous (+Bonus):** Characters who take extra time gain precision.  
  * \+50% Time: \+2 Bonus.  
  * \+100% Time: \+4 Bonus.  
  * \+200% Time: \+6 Bonus (Maximum).  
* **Rushed (-Penalty):** Characters forcing a result in less time suffer penalties.  
  * \-25% Time: \-2 Penalty.  
  * \-50% Time: \-4 Penalty.  
  * \-75% Time: \-8 Penalty.


  

  ### **Comparing to the Target Number**

  The final sum of the d20 roll, Skill Rank, Ability Score modifier, and any other modifiers is then compared to a target number set by the Game Master (GM). This target number, often referred to as the Difficulty Class (DC) or Challenge Rating (CR), represents the difficulty of the task being attempted. If the character's total equals or exceeds the target number, they succeed in the action. If their total falls below the target number, they fail.


  *Example: A character with a Stealth skill of 12 and a Dexterity modifier of \+3 attempts to sneak past a group of guards. The GM sets the DC for this Stealth check at 15, considering the alertness of the guards and the environmental conditions. The character rolls a 10 on their d20, adds their Stealth rank (+12) and their Dexterity modifier (+3), resulting in a total of 25\. Since 25 exceeds the DC of 15, the character successfully sneaks past the guards undetected.*

***Skill Checks are a fundamental mechanic in Tangent, used to resolve a wide range of actions and challenges throughout the game. They provide a structured and transparent way to determine success or failure, while also allowing for flexibility and customization based on the character's abilities, the difficulty of the task, and the GM's discretion. This system ensures that skill checks are both engaging and fair, reflecting the character's capabilities and the inherent risks and rewards of their actions.***

# **SKILL USE**

# **SKILL USE**

In Tangent, the success of actions and tasks attempted by characters is determined through Skill Checks. These checks involve rolling a d20 die and adding relevant modifiers to determine if the character succeeds in their endeavors.

## **Task**

CR (Challenge Rating)

#### **Simple**

0

#### **Very Easy**

5

#### **Easy**

10

#### **Average**

15

#### **Difficult**

20

#### **Very Difficult**

25

#### **Nearly Impossible**

30

#### **Miraculous**

35

## **Challenge Rating (CR)**

The core mechanic of Skill Checks revolves around the Challenge Rating (CR), a numerical value representing the difficulty of the task at hand.  The base CR for most Skill Checks is 10, signifying an average difficulty that a trained individual could reasonably accomplish. However, the GM can adjust the CR based on various factors, including:

### **Task Complexity**

More complex tasks naturally warrant a higher CR, reflecting the increased skill and effort required for success.

### **Environmental Factors**

Adverse conditions, such as poor lighting, extreme weather, or hazardous terrain, can elevate the CR, making the task more challenging.

### **Opposing Forces**

If the character's action is directly opposed by another character's skill or ability, the CR might be modified to reflect the competition.

### **Task Difficulty Table**

A general guideline for CRs and their corresponding task difficulties is provided

### **Crafting CRs**

Crafting tasks use CRs in a slightly different manner. In addition to indicating the overall difficulty, the CR also determines the degree of completion achieved with each check. This allows for incremental progress in crafting projects, with higher CRs representing more significant milestones.

## **Other Skill Use Mechanics:**

### **Taking 10**

In non-threatening or low-stress situations, characters can choose to take a fixed value of 10 for their Skill Check instead of rolling the d20. This represents a conservative approach, relying on consistent competence rather than risking a failed roll.

### **Taking 20**

In safe and low-stress environments, characters can opt to take a fixed value of 20 for their Skill Check, simulating multiple attempts and careful execution. However, this consumes significantly more time and resources.

### **Skill Synergy**

Characters can sometimes use multiple skills in conjunction to improve their chances of success. The GM determines which skills can synergize and the resulting bonus to the primary Skill Check.

### **Modifiers**

Various factors can influence the outcome of Skill Checks through modifiers. These can include the character's Ability Scores, Skill Ranks, situational bonuses or penalties, and the use of special features or abilities. The GM has the final say on applying modifiers and their effects.

# **SKILL SYNERGY**

# **SKILL SYNERGY**

Skill Synergy is a mechanic in Tangent that allows characters to utilize their existing skills to gain an advantage on other skill checks. This system encourages strategic thinking and rewards players for considering how their character's various skills can complement each other.

## **Using an Auxiliary Skill**

When attempting a primary skill check, a character can opt to use a different but relevant skill to boost their chances of success. This auxiliary skill should logically relate to the primary skill check, demonstrating a synergistic approach to problem-solving.

## **Bonus Calculation**

When a character possesses relevant practical experience that assists a primary check, they gain a flat synergy bonus based on their skill ranks. This replaces complex active division during gameplay.

* **Synergy Requirement:** To grant a synergy bonus, a secondary skill must have a clear, logical connection to the primary task, as approved by the GM.  
* **Trained Synergy (5 to 9 Ranks):** If the character has at least 5 ranks in the secondary skill, they grant a flat \+2 synergy bonus to the primary check.  
* **Expert Synergy (10+ Ranks):** If the character has 10 or more ranks in the secondary skill, they grant a flat \+4 synergy bonus to the primary check.

# **Example**

If a character makes a Reason (Intellect) check to decipher an ancient text and has 6 ranks in Linguistics, they add a flat \+2 synergy bonus to their Reason check.

# **ADVANTAGE**

# **ADVANTAGE**

In Tangent, Rolling with Advantage is a powerful mechanic that significantly increases the chances of success in various situations. When you have Advantage, you roll the d20 twice (or more, depending on the number of Advantage effects you have), and you choose the higher of the two rolls. This effectively boosts the probability of obtaining a high roll, thereby improving your odds of meeting or surpassing the Challenge Rating (CR) associated with the task or check.

Rolling with Advantage is particularly useful in situations where the odds are stacked against you. When you're facing a challenging encounter or a difficult skill check, having Advantage can tip the scales in your favor. It can also be helpful when you're trying to pull off a risky maneuver or when you need to make a crucial decision.

There are several ways to gain Advantage in Tangent. Some abilities and spells grant Advantage on specific rolls, while certain class features and feats can provide Advantage more broadly. Additionally, you can gain Advantage through environmental factors, such as flanking an enemy or having a clear line of sight.

Rolling with Advantage is a valuable tool that can help you overcome difficult challenges and achieve your goals in Tangent. Whether you're a seasoned adventurer or a new player, learning how to use Advantage effectively will make you a more formidable opponent.

## **Mechanics**

### **Multiple Rolls**

When rolling with Advantage, you roll the d20 die twice (or more, depending on the number of Advantage effects) and select the highest result. This effectively increases the probability of rolling a high number, improving your chances of meeting or exceeding the Challenge Rating (CR) for the task or check.

### **Stacking Effects**

Multiple sources of Advantage generally stack, meaning you get to roll an additional d20 for each Advantage effect. For example, if you have two Advantage effects, you would roll three d20s and choose the best result.

### **Countering Disadvantage**

Advantage directly counters Disadvantage. If you have both Advantage and Disadvantage on a roll, they cancel each other out, and you roll a single d20 as normal.

### **Karma Point**

You can spend a Karma Point to gain Advantage on a single roll. This is a valuable option when you need a crucial success and are willing to expend a limited resource.

## **Sources of Advantage**

### **Features**

Certain features, such as "Lucky" or "Improved Critical," grant Advantage on specific rolls or under certain conditions.

### **Situational Factors**

The GM may grant Advantage based on the specific circumstances of the situation, such as having the element of surprise, using superior tactics, or exploiting an opponent's weakness.

### **Environmental Factors**

Favorable environmental conditions, like elevated positioning or clear visibility, can sometimes provide Advantage.

### **Tactical Consideration**

#### **Important Rolls**

It's generally advisable to use Advantage on rolls where success is crucial or the consequences of failure are significant.

#### **Limited Resources**

If Advantage is gained from a limited resource, such as Karma Points, use it strategically and prioritize the most important rolls.

#### **Synergizing Effects**

Consider combining Advantage with other bonuses or modifiers to further increase your chances of success.

# **DISADVANTAGE**

# **DISADVANTAGE**

Rolling with Disadvantage presents a significant hurdle in Tangent, decreasing the likelihood of success in various situations.Rolling with Disadvantage in Tangent presents a significant hurdle for players, significantly decreasing the likelihood of success in various situations. When rolling with disadvantage, players roll two dice and must use the lower of the two results. This means that even if a player has high ability scores or modifiers, they may still have difficulty succeeding on certain checks.

Overall, rolling with disadvantage is a significant hurdle for players in Tangent. It can make it more difficult to succeed on checks, hit enemies in combat, and progress through the game.

## **Mechanics**

### **Multiple Rolls**

When rolling with Disadvantage, you roll the d20 die twice (or more, depending on the number of Disadvantage effects) and select the lowest result. This effectively increases the probability of rolling a low number, hindering your chances of meeting or exceeding the Challenge Rating (CR) for the task or check.

### **Stacking Effects**

Multiple sources of Disadvantage generally stack, meaning you get an additional d20 roll for each Disadvantage effect. For example, if you have two Disadvantage effects, you would roll three d20s and choose the worst result.

### **Countering Advantage**

Disadvantage directly counters Advantage. If you have both Advantage and Disadvantage on a roll, they cancel each other out, and you roll a single d20 as normal.

### **Karma Point**

You can spend a Karma Point to eliminate one of the extra dice in the roll, mitigating the impact of Disadvantage. This is a valuable option when you want to improve your chances of success and are willing to expend a limited resource.

## **Sources of Disadvantage**

### **Features**

Certain features, such as "Clumsy" or "Impaired," impose Disadvantage on specific rolls or under certain conditions.

### **Skills**

Low ranks in skills or attempting actions outside of your character's expertise can lead to Disadvantage.

### **Situational Factors**

The GM may impose Disadvantage based on the specific circumstances, such as being in a poorly lit area, using improvised tools, or facing a superior opponent.

### **Environmental Factors**

Unfavorable environmental conditions, like slippery surfaces, extreme weather, or distractions, can sometimes cause Disadvantage.

### **Tactical Considerations:**

Important Rolls  
Avoid Disadvantage on rolls where success is crucial or the consequences of failure are severe.

### **Limited Resources**

If you must roll with Disadvantage, consider using a Karma Point to improve your odds, especially if the outcome is vital.

### **Mitigating Factors**

Try to minimize the impact of Disadvantage by addressing its source, if possible. For example, move to a better-lit area, use proper tools, or seek assistance from a skilled ally.

# **TIME**

# **TIME**

In Tangent, the time a character dedicates to a skill check directly impacts their chances of success. This system introduces two primary modifiers:

## **Meticulous**

Characters who take their time to perform a skill meticulously receive a bonus to their skill check. This bonus reflects the increased precision and attention to detail afforded by a slower, more deliberate approach. For example, a character carefully examining a crime scene or meticulously picking a lock would benefit from this bonus.   
For each 50% increase in the base time to complete gains a \+2 to the check result.

\+50% time  
\+2

\+100% time  
\+4

\+150% time  
\+6

\+200% time  
\+8

## **Rushed**

Conversely, characters attempting to perform a skill hastily incur a penalty to their skill check. This penalty represents the heightened risk of errors and mistakes associated with a faster, less cautious approach. For instance, a character hurriedly trying to disarm a bomb or quickly hacking into a secure system would suffer this penalty.   
A 25% reduction in time incurs a \-2 penalty (doubling for each additional 25% reduction, with a minimum of ¼ the typical time at \-8 penalty).

3/4 time  
\-2

1/2 time  
\-4

1/4 time  
\-8

The magnitude of these modifiers scales with the degree of meticulousness or haste. Taking more time for meticulous execution yields a larger bonus, while rushing a task incurs a greater penalty. This system encourages strategic decision-making, where characters must weigh the benefits of a potential bonus against the risks of a penalty based on the urgency and complexity of the situation.

## **Time Factors for Unskilled Use**

When a character attempts to use a skill in which they are Untrained (skill rank 0), the penalties for Rushed actions are increased, and the bonuses for Meticulous actions are reduced (either is 1 step worse). This reflects the character's lack of familiarity and expertise, making it harder for them to perform the skill effectively under time pressure or with increased care.

*Examples*  
*A team of investigators is examining a complex crime scene. The character with the highest Investigation skill wants to ensure they don't miss any crucial clues. They decide to take their time and meticulously comb through the scene. The GM sets the base time for a thorough investigation at 2 hours. The character spends 6 hours meticulously analyzing every detail. As a result, they receive a \+6 bonus on their Intellect (Investigation) check. This reflects the increased likelihood of uncovering hidden evidence and making accurate deductions due to the meticulous approach.*

*A team of adventurers is infiltrating a high-security facility. They need to bypass a locked door quickly to avoid being detected by patrolling guards. The character with the highest Computers skill decides to rush the hacking process.  The GM determines the base time for hacking the door is 3 minutes.  The character tries the hack in 1 minute (less than half the base time).  As a result, they incur the maximum \-8 penalty on their Computers (Hacking) check. This reflects the increased likelihood of making mistakes or triggering alarms due to the rushed approach.*

# **FOCUSED TASK**

# **FOCUSED TASK**

In Tangent, characters have the option to enter a state of intense focus when undertaking a specific task. This represents a heightened state of concentration and dedication where the character directs their complete attention to the task, potentially at the cost of their awareness of what's happening around them or their ability to handle multiple things at once.  It's a high-risk, high-reward tactic that can be used strategically to increase the chances of success in critical situations.

## **\+2 Bonus**

Focusing on a task grants a \+2 bonus to the Skill Check associated with that task. This bonus reflects the character's increased concentration and attention to detail, allowing them to perform the task with greater precision and effectiveness.

## **\-5 Penalty**

While focused, the character suffers a \-5 penalty to all other Skill Checks. This penalty represents the character's reduced awareness of their surroundings and decreased ability to react to unexpected events or perform actions unrelated to the focused task.

## **Duration**

The \+2 bonus and \-5 penalty remain in effect for the entire duration of the task. This means that once the character decides to focus, they must commit to it until the task is completed or they choose to break their focus.

*Examples*  
*A hacker focusing on bypassing a complex security system would receive a \+2 bonus to their Computers (Hacking) Skill Check, but would suffer a \-5 penalty to their Alertness or Perception checks while doing so.*

*A surgeon focusing on a delicate operation would gain a \+2 bonus to their Medicine (Surgery) Skill Check, but would be less likely to notice distractions or interruptions in the operating room.*

*A sniper focusing on lining up the perfect shot would receive a \+2 bonus to their Firearms (Sniper) Skill Check, but would be more vulnerable to flanking attacks or sudden changes in the environment.*

# **TRANSPOSED SKILL BASE**

# **TRANSPOSED SKILL BASE**

Skills are generally associated with specific Ability Scores that represent the primary attribute governing their use. However, certain character concepts might call for a different Ability Score to be used for a particular skill, reflecting unique traits, abilities, or circumstances. This is known as a Transposed Skill Base.

## **Permanent or Situational**

Transposed Skill Bases can be either permanent, reflecting an inherent aspect of the character's nature, or situational, depending on specific circumstances or the use of certain abilities.

## **GM Approval**

Any transposition of a skill's base Ability Score requires GM approval. The GM will consider the character's concept, the rationale behind the change, and the potential impact on game balance before allowing it.

*Examples*  
*A character with cybernetic enhancements might use Intellect instead of Agility for their Piloting skill, reflecting the use of advanced computer interfaces and calculations.*

*A character with strong psychic abilities might use Wisdom instead of Charisma for their Persuasion skill, drawing upon their mental powers to influence others.*

*A character who relies on agility and finesse in combat might use Agility instead of Strength for their Melee Weapons skill, reflecting their fighting style and weapon choices.*

## **Benefits**

### **Character Customization**

Transposed Skill Bases allow for greater flexibility in character creation, enabling players to build characters with unique abilities and play styles that might not fit neatly into the standard skill-ability associations.

### **Narrative Depth**

They can reflect a character's unique background, training, or physical/mental attributes, adding depth and realism to their portrayal.

### **Tactical Advantages**

In some cases, transposing a skill's base Ability Score can provide tactical advantages, allowing characters to leverage their strengths in unexpected ways or overcome limitations imposed by their primary attributes.

## **Considerations**

### **Game Balance**

The GM must carefully consider the potential impact of Transposed Skill Bases on game balance, ensuring that they don't create overly powerful or unbalanced characters.

### **Clarity and Consistency**

Clear communication between the player and GM is essential to ensure that the transposition is understood and applied consistently throughout the game.

### **Character Concept**

Transposed Skill Bases should align with the character's overall concept and make sense within the context of the game world and its rules.



## Game Mechanics Rules
```
See full canonical text in 1.07 SKILLS.md
```

## Gameplay Instructions
Refer to 1.07 SKILLS.md in the game rules library for complete architectural tables and system parameters.

## Designer Notes
Canonical Tangent SF RP rulebook reference from 1.07 SKILLS.md.
