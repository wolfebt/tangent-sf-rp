...
export const CampaignOpsWidget = ({ onOpenVttOptionsDrawer, onShowOverview }) => {
  ...
  return (
    <div 
      ...
      className="bg-slate-900/30 hover:bg-slate-900/80 backdrop-blur-md p-2.5 sm:p-3 rounded-xl border border-cyan-500/40 hover:border-cyan-400/90 flex flex-col justify-between relative overflow-hidden group transition-all duration-200 shadow-[0_0_15px_rgba(34,211,238,0.1)] cursor-pointer select-none"
    >
      ...
      <div>
        ...
        <div className="mt-1.5 space-y-1">
          <div>
            <span className="text-[8px] font-mono uppercase tracking-wider text-slate-500">Tactical Deployment</span>
            <h2 className="text-xs sm:text-sm font-bold text-white tracking-wide truncate mt-0 font-mono">
          ...
          <div className="space-y-0.5 text-xs">
            ...
            <div className="flex items-center justify-between text-[9px] font-mono text-slate-400 pt-0.5 border-t border-slate-800/60">
              <span className="flex items-center gap-1">
                <Map size={10} className="text-cyan-400" /> {mapCount} {mapCount === 1 ? 'Map' : 'Maps'}
              </span>
              <span className="flex items-center gap-1">
                <Layers size={10} className="text-purple-400" /> {scenarioCount} {scenarioCount === 1 ? 'Scenario' : 'Scenarios'}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="mt-2 pt-1.5 border-t border-slate-800/80 space-y-1">
        <button
          ...
          className="w-full py-1.5 px-2.5 bg-gradient-to-r from-cyan-600 to-cyan-500 hover:from-cyan-500 hover:to-cyan-400 text-white font-bold text-[10px] font-mono uppercase tracking-wider rounded-md flex items-center justify-center gap-1.5 transition-all shadow-[0_0_15px_rgba(34,211,238,0.3)] hover:shadow-[0_0_20px_rgba(34,211,238,0.5)] active:scale-98"
        >
          <Play size={11} fill="currentColor" /> Launch Tactical VTT
        </button>
        ...
        <div className="flex items-center gap-1.5">
          ...
          >
            <Sliders size={10} className="text-cyan-400" /> VTT Options
          </button>
          ...
          >
            {copied ? <Check size={10} className="text-emerald-400" /> : <Eye size={10} className="text-emerald-400" />}
