...
export const GameSquadsWidget = ({ onOpenSquadsDrawer }) => {
  ...
  return (
    <div 
      ...
      className="bg-slate-900/30 hover:bg-slate-900/80 backdrop-blur-md p-3.5 sm:p-4 rounded-xl border border-emerald-500/40 hover:border-emerald-400/90 flex flex-col justify-between relative overflow-hidden group transition-all duration-200 shadow-[0_0_15px_rgba(16,185,129,0.1)] cursor-pointer select-none"
    >
      {/* Ambient Glow */}
      ...
      <div>
        ...
        <div className="flex items-center justify-between pb-2 border-b border-slate-800">
          ...
          {inviteCount > 0 ? (
            <span className="px-2 py-0.5 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-300 font-mono text-[9px] font-bold flex items-center gap-1 animate-pulse">
              <UserPlus size={10} />
        ...
        <div className="mt-2.5 space-y-1.5">
          {targetGroup ? (
            <>
              <div>
                <span className="text-[8.5px] font-mono uppercase tracking-wider text-slate-500">Active Fireteam</span>
                <div className="flex items-center justify-between mt-0.5">
                  <h2 className="text-sm sm:text-base font-bold text-white tracking-wide truncate font-mono">
                    {targetGroup.name}
                  </h2>
                  <span className={`px-1.5 py-0.2 rounded text-[8.5px] font-mono border uppercase ${
                  ...
                  {membersList.slice(0, 5).map((m, idx) => {
                    ...
                    return (
                      <div
                        ...
                        className="w-6 h-6 rounded-lg bg-slate-800 border border-slate-700 flex items-center justify-center text-[10px] font-bold text-cyan-300 font-mono"
                      >
                        {initial}
                      </div>
                    );
            ...
            <div className="py-2 text-center space-y-1">
              <p className="text-xs text-slate-400 font-mono">NO ACTIVE SQUAD</p>
      ...
      <div className="mt-3 pt-2 border-t border-slate-800/80 space-y-1.5">
        <button
          ...
          className="w-full py-2 px-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-[11px] font-mono uppercase tracking-wider rounded-lg flex items-center justify-center gap-1.5 transition-all shadow-[0_0_15px_rgba(16,185,129,0.25)] hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] active:scale-98"
        >
          <Users size={12} />
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="py-1.5 px-2.5 bg-slate-800/90 hover:bg-slate-700 text-emerald-300 font-bold text-[10px] font-mono uppercase tracking-wider rounded-lg border border-slate-700 hover:border-emerald-500/50 transition-colors flex items-center gap-1"
          >
