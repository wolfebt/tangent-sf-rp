...
export const PersonaLogModal: React.FC<PersonaLogModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 font-sans select-none">
      <div className="bg-[#0b1019] border border-cyan-500/50 rounded-2xl shadow-[0_0_50px_rgba(6,182,212,0.25)] w-full max-w-3xl overflow-hidden flex flex-col max-h-[85vh] animate-in fade-in zoom-in duration-150 text-slate-200">
        
        {/* Header */}
        <div className="px-5 py-3.5 border-b border-slate-800 bg-slate-950/90 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-amber-500/20 border border-amber-500/50 flex items-center justify-center text-amber-300 shadow-[0_0_15px_rgba(245,158,11,0.2)]">
              <Activity size={18} className="animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-sm font-bold font-mono text-white tracking-wide">
                  OPERATIVE ACTION TELEMETRY: {personaName}
                </h2>
                <span className="px-2 py-0.2 rounded-full bg-cyan-950 border border-cyan-500/40 text-[9px] font-mono text-cyan-300 font-bold">
                  TL{techLevel}
                </span>
                <span className="px-2 py-0.2 rounded-full bg-amber-950/80 border border-amber-500/40 text-[9px] font-mono text-amber-300 font-bold uppercase">
                  GM BLACKBOX
                </span>
              </div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleCopy}
              className="px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-cyan-500/50 text-slate-300 hover:text-white text-xs font-mono font-bold flex items-center gap-1.5 transition-all cursor-pointer"
        ...
        {/* Filter Toolbar */}
        <div className="px-5 py-2.5 bg-slate-950/60 border-b border-slate-800/80 flex items-center justify-between gap-3 text-xs font-mono shrink-0">
        ...
        {/* Telemetry Stream */}
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5 font-mono text-xs no-scrollbar bg-slate-950/40">
