...
export const DBMTableView = ({
  ...
  const facetCounts = useMemo(() => {
    ...
    currentItems.forEach(i => {
      ...
      types.forEach(t => {
        if (!counts.types[t]) counts.types[t] = 0;
        counts.types[t]++;
      });

      // Subtypes
      const valSub = i.subtype || i.discipline || i.society || i.aspect || i.aspect_subtype;
      const subs = Array.isArray(valSub) ? valSub : (valSub ? [valSub] : []);
      subs.forEach(s => {
        if (!counts.subtypes[s]) counts.subtypes[s] = 0;
        counts.subtypes[s]++;
      });

      // TLs
      const tl = i.tech_level !== undefined ? i.tech_level : (i.tl !== undefined ? i.tl : i.techLevel);
      if (tl !== undefined && tl !== null) {
        if (!counts.tls[tl]) counts.tls[tl] = 0;
        counts.tls[tl]++;
      }

      // MLs
      const ml = i.meta_level !== undefined ? i.meta_level : (i.ml !== undefined ? i.ml : i.metaLevel);
      if (ml !== undefined && ml !== null) {
        if (!counts.mls[ml]) counts.mls[ml] = 0;
        counts.mls[ml]++;
      }
      ...
      tags.forEach(t => {
        if (!counts.tags[t]) counts.tags[t] = 0;
        counts.tags[t]++;
      });

      // Lineages
      const p = (i.parent_species || '').toLowerCase();
      SPECIES_LINEAGES.forEach(lin => {
        const shortName = lin.name.split(' ')[0].replace(/[^a-zA-Z]/g, '');
        if (p.includes(shortName.toLowerCase()) || p === lin.id.toLowerCase()) {
           if (!counts.lineages[lin.id]) counts.lineages[lin.id] = 0;
           counts.lineages[lin.id]++;
        }
      });
    });
      ...
      {currentKey === 'species' && (
        <div className="flex flex-wrap items-center gap-1.5 mb-2 pb-2 border-b border-purple-900/40 shrink-0 bg-purple-950/20 p-2 rounded-lg border border-purple-800/30">
          ...
          {SPECIES_LINEAGES.map(lin => {
            ...
            const count = facetCounts.lineages[lin.id] || 0;
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {availableSubtypes
                        ...
                        .map(s => {
                          const count = facetCounts.subtypes[s] || 0;
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {availableTLs.map(tl => {
                        ...
                        const count = facetCounts.tls[tl] || 0;
                        return (
                          <button
                            key={tl}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {availableMLs.map(ml => {
                        ...
                        const count = facetCounts.mls[ml] || 0;
                        return (
                          <button
                            key={ml}
