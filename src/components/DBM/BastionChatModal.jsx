...
export const BastionChatModal = ({
  ...
  const [isDocked, setIsDocked] = useState(() => {
    try {
      const saved = localStorage.getItem('bastion_dbm_dock_mode');
  ...
  const toggleDock = () => {
    setIsDocked(prev => {
      const next = !prev;
      try {
        localStorage.setItem('bastion_dbm_dock_mode', JSON.stringify(next));
      } catch (e) {}
      return next;
    });
  };
              ...
              <div className="bg-slate-900/90 border border-cyan-900/50 rounded-lg p-3">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={selectBlankOnly} className="text-[9px] text-cyan-400 hover:text-cyan-300 underline font-bold uppercase">
                      + Blank Only
                    </button>
                    <span className="text-slate-600">|</span>
                    <button type="button" onClick={clearAllFields} className="text-[9px] text-slate-400 hover:text-slate-200 underline font-bold uppercase">
                      Clear
                    </button>
                  </div>
                </div>
    ...
    return (
      <>
        ...
        <div 
          ...
          onClick={onClose} 
        />
        <div className="fixed top-[52px] bottom-0 right-0 z-50 w-80 sm:w-[440px] bg-[#0d1117]/95 border-l border-cyan-500/50 shadow-[-10px_0_30px_rgba(0,0,0,0.8)] flex flex-col font-sans backdrop-blur-md">
          {renderInnerContent()}
        </div>
      </>
    );
