...
export const RosterModal = ({
  ...
  // Helper to extract string value from identity fields (which can be string or object)
  ...
  const filteredRoster = personaRoster.filter((char) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-3 sm:p-6">
      <div className="bg-slate-900 border border-cyan-500/50 rounded-xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden relative">
        ...
        <div className="bg-slate-950/80 p-3 px-6 border-b border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
          <div className="relative w-full sm:w-80">
            <span className="absolute left-3 top-2.5 text-slate-500 text-xs">🔍</span>
            <input
              type="text"
              placeholder="Filter by name, species, faction, origin, occupation..."
              value={searchQuery}
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto text-xs">
            <span className="text-[10px] text-slate-400 uppercase font-mono mr-1">View Layout:</span>
            <button
              onClick={() => setViewMode('card')}
              className={`px-3 py-1 rounded font-bold uppercase text-[11px] transition-colors flex items-center gap-1 ${
                viewMode === 'card'
                  ? 'bg-cyan-600 text-white border border-cyan-400 shadow'
                      ...
                      <div className="mt-3 p-2.5 bg-slate-900/60 border border-slate-800/80 rounded-lg">
                        ...
                        {isEditingThisNote ? (
                          ...
                          <p className="text-[11px] text-slate-300 font-sans italic line-clamp-3 leading-relaxed">
                            {noteContent || <span className="text-slate-600 not-italic">No notes logged for this operative. Click Edit Note to add notes.</span>}
                          </p>
                        )}
                      </div>
                    </div>
