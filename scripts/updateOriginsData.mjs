...
const originsDataPath = 'd:/_ Data/Tangent SF RP/TANGENT SF RP react project/src/data/originsData.js';
const omnicortexOriginsDir = 'd:/_ Data/Tangent SF RP/TANGENT SF RP react project/src/data/omnicortex/origins';

const ALL_11_ORIGINS = [
  {
...
export const DEFAULT_ORIGINS = ${JSON.stringify(ALL_11_ORIGINS, null, 2)};

export default DEFAULT_ORIGINS;
`;

fs.writeFileSync(originsDataPath, fileContent, 'utf-8');
console.log('Successfully written originsData.js with 11 canonical origins!');

// Also ensure the 11 markdown files in omnicortex/origins are updated
ALL_11_ORIGINS.forEach(origin => {
  const mdFileName = `${origin.id}.md`;
  const mdFilePath = path.join(omnicortexOriginsDir, mdFileName);

  const mdContent = `---
id: ${origin.id}
name: ${origin.name}
category: origins
habitat: "${origin.habitat}"
skill_points: 20
society_skills:
${origin.society_skills.map(s => `  - "${s}"`).join('\n')}
archetypes:
${origin.archetypes.map(a => `  - "${a}"`).join('\n')}
traits:
${origin.traits.map(t => `  - "${t}"`).join('\n')}
description: >-
...
${origin.traits_detail.map(td => `### ${td.name}
${td.description}  
*Bonus*: **${td.mechanic}**
`).join('\n')}
...
console.log('Successfully updated all 11 markdown files in omnicortex/origins!');
