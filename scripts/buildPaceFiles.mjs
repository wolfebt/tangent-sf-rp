...
const paces = [
  ...
  { id: 'movement-running', name: 'Ground: Running Pace', mode: 'Ground', mult: '4x Base (5x with Runner)', speed: 120, desc: 'Fast running pace requiring Athletics check (CR 10+) each minute to avoid fatigue.', pen: '-4 to subtle actions', check: 'Athletics CR 10+' },
  { id: 'movement-sprinting', name: 'Ground: Sprinting Pace', mode: 'Ground', mult: '6x Base (7x with Runner)', speed: 180, desc: 'Maximum land sprint requiring demanding Athletics check (CR 15+) each minute to avoid fatigue.', pen: '-8 to subtle actions', check: 'Athletics CR 15+' },
  ...
  { id: 'movement-surge', name: 'Flying: Surge / Soar Pace', mode: 'Flying', mult: '4x Flight (8x Walk, 5x with Soar)', speed: 240, desc: 'Maximum aerial surge requiring Acrobatics check (CR 10+) each minute to avoid fatigue.', pen: '-4 to subtle actions', check: 'Acrobatics CR 10+' },
  { id: 'movement-diving', name: 'Flying: Diving Pace', mode: 'Flying', mult: '2x Current Speed (9x with Soar)', speed: 480, desc: 'High-speed descent maneuver for tactical engagement.', pen: '-4 to actions', check: 'Acrobatics CR 15+' },
  { id: 'movement-gliding', name: 'Flying: Gliding Maneuver', mode: 'Flying', mult: '1x Flight (drops 1ft per 5ft horiz)', speed: 60, desc: 'Controlled descent using aerodynamic lift. Grants +2 bonus to actions.', pen: 'None (+2 to actions)', check: 'Acrobatics CR 10+' },
  { id: 'movement-hover-descent', name: 'Flying: Hover & Controlled Descent', mode: 'Flying', mult: '1/2x Flight or less', speed: 30, desc: 'Stationary or slow descent enabling precise observation and stable targeting.', pen: 'None', check: 'Acrobatics CR 15+' },
  ...
  { id: 'movement-glide-swim', name: 'Swimming: Glide Pace', mode: 'Swimming', mult: '2x Swim (1x Walk)', speed: 30, desc: 'Hurried swimming pace with a -2 penalty to subtle actions.', pen: '-2 to subtle actions', check: 'Athletics (Swimming) CR 10+' },
  { id: 'movement-stroke', name: 'Swimming: Stroke Pace', mode: 'Swimming', mult: '4x Swim (2x Walk)', speed: 60, desc: 'Fast power-stroke swimming pace requiring Athletics (Swimming) check (CR 15+) each minute.', pen: '-4 to subtle actions', check: 'Athletics (Swimming) CR 15+' },
  { id: 'movement-treading', name: 'Swimming: Treading Pace', mode: 'Swimming', mult: '1/2x Swim or less', speed: 7.5, desc: 'Stationary or slow treading water to conserve stamina. Possible +2 bonus to actions.', pen: 'None (+2 to actions)', check: 'Athletics (Swimming) CR 5+' },

  // Climbing paces
  { id: 'movement-climb', name: 'Climbing: Standard Climb Pace', mode: 'Climbing', mult: 'Surface Dependent (1/2, 1/4, 1/10 Walk)', speed: 15, desc: 'Standard vertical ascent/descent across Easy (CR 10), Moderate (CR 15), or Difficult (CR 20) surfaces.', pen: 'Risk of falling', check: 'Athletics (Climbing) vs Surface DC' },
  ...
  { id: 'movement-fast-descent', name: 'Climbing: Fast Descent Pace', mode: 'Climbing', mult: '4x Base Walk', speed: 120, desc: 'Rapid controlled vertical descent without injury.', pen: '-4 to actions', check: 'CR 20 Athletics (Climbing) or -10 penalty' },
...
- **Glide (2x Swim / 1x Walk)**: 30 ft/rd. -2 penalty to subtle actions. Athletics (Swimming) CR 10+.
- **Stroke (4x Swim / 2x Walk)**: 60 ft/rd. -4 penalty to subtle actions. Athletics (Swimming) CR 15+.
- **Treading (1/2 Swim or less)**: 7.5 ft/rd. +2 bonus to actions. Athletics (Swimming) CR 5+.
`);
...
- **Easy (CR 10+)**: 15 ft / rd (1/2 base walk).
- **Moderate (CR 15+)**: 7.5 ft / rd (1/4 base walk).
- **Difficult (CR 20+)**: 3 ft / rd (1/10 base walk).
...
- **Fast Descent (4x Walk)**: 120 ft/rd. -4 penalty. CR 20 Athletics (Climbing) or -10 penalty.
`);
