 ...
 * @description Stage 2.3: Infinite canvas spatial hashing and aggressive culling.
 ...
 * bypassing the GPU entirely to maintain 144hz during Mega Colossal engagements.
 */
...
export class FrustumChunkManager {
  ...
  /**
   * Updates a sprite's chunk location if it has moved (e.g., a token moving across the grid).
   */
  ...
  /**
   * Called every frame or whenever the camera pans/zooms. 
  ...
  public updateCulling(viewportBounds: Rectangle) {
    ...
    for (const activeChunk of this.activeVisibleChunks) {
      if (!newlyVisibleChunks.has(activeChunk)) {
        ...
        if (spritesToHide) {
          spritesToHide.forEach(sprite => sprite.renderable = false);
        }
      }
    }
    ...
    for (const newChunk of newlyVisibleChunks) {
      if (!this.activeVisibleChunks.has(newChunk)) {
        ...
        if (spritesToShow) {
          spritesToShow.forEach(sprite => sprite.renderable = true);
        }
      }
    }

    // 4. Update the cache for the next frame
