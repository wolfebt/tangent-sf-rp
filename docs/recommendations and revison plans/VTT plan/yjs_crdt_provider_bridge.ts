...
// We define a unique byte prefix to differentiate Yjs binary payloads from our custom Telemetry payloads
...
export class YjsProviderBridge {
  ...
  public characterSheets: Y.Map<any>;
  ...
  constructor(room: Room) {
    ...
    // Initialize the Yjs document with aggressive garbage collection enabled by default
    // This prevents the document history from ballooning indefinitely over a long campaign
    this.doc = new Y.Doc({ gc: true });
    
    // Map the CRDT structures
    this.characterSheets = this.doc.getMap('characters');
  ...
  private setupLiveKitListeners() {
    this.room.on(RoomEvent.DataReceived, (payload: Uint8Array, participant?: RemoteParticipant, kind?: DataPacket_Kind) => {
      // Yjs updates MUST be reliable. We ignore LOSSY (unreliable) packets which are for cursor telemetry.
  ...
  private setupYjsListeners() {
    // Listen for any local modifications to the Y.Doc (e.g., a player editing their HP)
    this.doc.on('update', (update: Uint8Array, origin: any) => {
      // If the origin is 'network', it means we just received this from a peer. 
      // We only broadcast local changes to avoid infinite echo loops.
      if (origin !== 'network' && this.isConnected) {
  ...
  private broadcastYjsUpdate(update: Uint8Array) {
    // Prepend our custom byte identifier to the payload
    const payload = new Uint8Array(update.length + 1);
    payload.set([PAYLOAD_PREFIX_YJS], 0);
    payload.set(update, 1);

    try {
      // Publish via reliable channel to ensure guaranteed, ordered delivery
      this.room.localParticipant.publishData(payload, DataPacket_Kind.RELIABLE);
    } catch (error) {
  ...
  private broadcastStateVector() {
    // Extract the state vector (a tiny fingerprint of our current document history)
  ...
  private decodeAndApplyYjsPayload(payload: Uint8Array) {
    ...
    // Check if this is a Yjs packet
    if (type === PAYLOAD_PREFIX_YJS) {
      const updateData = payload.slice(1);
      
      try {
        // Yjs magically merges the remote update into our local document mathematically.
        // We pass 'network' as the origin to prevent our listener from bouncing it back.
